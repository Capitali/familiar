...
- [x] T8. R4: triage `crates/mesh` unwrap/expect on network-input paths (brief ingestion, worldview parsing, observation batch parsing, TLS/handshake)
...
- [x] T15. R10: iOS face recognition — verify current Vision API against Apple docs, then implement embedding/match
- [x] T16. R10: confirm-before-keep UI + interactive identification fallback
- [x] T17. R10: macOS equivalent in FamiliarMac.app
...
- [x] T21. iOS: build, install to Aphelion (iPhone) + Codex (iPad) — confirm before device install
- [x] T22. watchOS: build, install to paired watch — confirm before device install
- [x] T23. Lighthouse VPS: redeploy with updated mesh/kernel code — confirm before remote deploy
- [x] T24. TestFlight/App Store submission, if desired — confirm explicitly, separate from device sideload
