...
- [ ] T4. R1: `ActionKind::Microphone/::Location/::Motion/::NetworkDiscovery` + matching `Boundary`/`CapabilityScope` fields, fail-closed, tested
- [ ] T5. R1: extend with a distinct biometric-recognition gate (`allow_face_recognition` or similar) for R10's "new consent toggle distinct from plain camera" requirement — not in original R1 list, added here since R10 depends on it
