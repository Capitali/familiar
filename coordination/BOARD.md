...
- next: `fleet status` as a captain-facing digest (per-ship P&L from deliveries.jsonl + trade receipts); the pooled treasury as a real ledger once the exchange grows fleet accounts (metal#62); a captain surface in the Familiar app (Partner ring card per ship).
