...
- notes: **What is ALREADY built** (checked in the tree 2026-08-24, so nobody rebuilds it): iOS/iPadOS runs a Bonjour/mDNS survey over **27 declared service types** — `_airplay._tcp`, `_raop._tcp`, `_homekit._tcp`, `_hap._tcp`, `_googlecast._tcp`, `_spotify-connect._tcp`, printers, SMB/AFP, `_companion-link._tcp` and more (`NetworkDiscovery.swift` + `App/Support/Info.plist`), and macOS mirrors it (`MacSensing.swift`, ~25 types). `NSLocalNetworkUsageDescription` is written and honest. **The iPad is the same target as the iPhone, so iPadOS already has this** — Ian's "ios and ipad os should be doing discovery" is, for Bonjour, already true
