...
- status: claimed, **BUILDING** — ADR-0044 ACCEPTED (Ian, 2026-08-21: "start ADR-0044 (use codex partner if needed)"). Rung 2: companion:claude-opus builds human-authored ClassDefs, the availability compiler, allowlist serializer, and `discover_classes`. **Rung 3 design CLAIMED by companion:codex**: grant object + typed partner-act ledger for `request_grant`/`propose`; no observe/invoke and no gate opens.
