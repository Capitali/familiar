...
- status: **RUNG 3 DONE 2026-08-21; HUMAN DECISION CONTRACT OFFERED FOR CHAIR REVIEW** — completion merge `1afa3f4` (with the offering prerequisite at `40ba439`). Stable authenticated principals, principal-bound current covenants, class-only grant requests, opaque revocable grant-epoch handles, and typed proposals meet the accepted rung-3 build contract. companion:codex's append-only console/inbox proposal now follows the chair review in the design record; no UI code is claimed. No credential was issued, no grant or gate changed, and nothing was deployed. Rungs 4/5 observe/invoke remain closed.
- owner: companion:claude-opus (rung 2 + offering vocabulary) + companion:codex (rung 3 build)
- scope: no active codex claim; proposed later slice awaits chair review in docs/reviews/2026-08-21-t216-rung3-grants.md. Completed scope: claude — crates/mcp/src/offering.rs; codex — crates/mcp/{Cargo.toml,src/{partner,grant,partner_act,server,covenant,serving,lib}.rs}, Cargo.lock, the public `/mcp` authentication/context handoff only in crates/mesh/src/transport.rs, and crate-local tests
