...
- notes: from familiar#10 (Ian's iPhone, build 100, on 5G+): the badge printed `pins 2+1 · <addr>→t:The request timed out.` for fifteen candidates — every LAN, tailnet and lighthouse address — where a sentence belongs. T-231 (landed 09-02, unshipped) already cuts the wait from fifteen timeouts to one; this is the words. MacOnStick lane, 2026-09-16.

### T-246 · The captain's standing orders — "repair at next docking" without the captain at the berth
- **asked (Ian, 2026-09-10): "how does the captain, Luke, the human, direct the repairs of their fleet. There needs to be an interactive method for the captain to do manual actions… 'repair at next docking' or something — man switch or flag in either UCF-Haul or UCF-familiar."** Trigger: a co-pilot key files only travel/book/cancelBooking/collect/refuel/engage — never repair, never a tanker call, never a lease payment.
- status: **HOST HALF LANDED (wildhorse, 2026-09-10, 63c0966), flying on all four pilots:** `orders.json` per hull (verb repair|refuel|payLease, when next-docking|now, amount, by, at, done_at, waits); the pilot files a ready order under the captain's authority at the first fold that satisfies it, AHEAD of the doctrine and the dial (it is the captain acting), journaled `order-done`; a verb the key cannot file → `order-waits` ("the captain's own papers must"); a refusal → `order-refused`. `fleet order <world> repair|refuel|payLease [--when] [--amount]`, `fleet orders <world>`; feed `GET/POST /ships/{id}/orders`, `DELETE /ships/{id}/orders/{oid}`; rows carry `orders: {pending, waiting}`. Contract fixture +3 words (MacOnStick classifies).
- **iPad half (MacOnStick):** an Orders control on each hull (repair at next docking / refuel full / pay ℳN), the waiting badge with the host's sentence, and a fleet-wide "repair all at next docking".
- **Jeff's side — FILED as ucf-exchange#41 (Ian's name, 2026-09-10):** the problem and four proposals: (A) widen the co-pilot key by scope under caps in #15's model (`auto:yard`, `auto:lease`, `auto:rescue`); (B) standing orders as an exchange object under the captain's key, executed by the fold at docking; (C) fleets as the authority boundary (metal#62/#86) — the commander's key acts for member hulls; (D) a fleet page in Haul. Recommendation A now, B to design for, C the destination.
- depends: T-242 (ship-care verbs), T-245 (the fleet of eight), metal#62/#86.

### T-244 · The fleet works together — one captain's hulls maximize the fleet's profit, not each their own
- **SLICE 2 LANDED AS ADVICE — wildhorse, 2026-09-16 (Ian: "use that additional income to pay down the lease on other ships … the captain needs access to all the income of the fleet").** `outfit::Purse.sisters` + `PayLease { sister }`: an owned hull pays the smallest sister balance before any fitting, reserve kept. No exchange verb moves a credit between a captain's actors, so the runner journals it as `advice` on `ship.lease` until `/v1/reference` says `fleetLeasePay: 1`. Ask to Jeff: `docs/partners/2026-09-16-the-captains-purse.md`. See DEVELOPMENT_LOG 2026-09-16.
- **asked (Ian, 2026-09-09): "All three should be working together to maximize profit and dominate the market."**
- status: **SLICE 0 LANDED (wildhorse, 2026-09-09, a9e102f):** `Forecast.inbound` — every sister hull's lots (same captain, same exchange, beside the store) by the shelf they are bound for; the projection lands that cargo before ours, so a starving works a sister is already feeding reads as fed and the second ship stays out of the run. Loads cannot be double-booked (the exchange takes a booked load off the open board), so freight already divides itself.
- **2026-09-09 later:** dock fees now come off every merchant run (the reference prices 2–12 ℳ per docking; freight's board figure already counted it); `fleet-inbound` journals the sisters' cargo on change (the soak's evidence). **PROD has not flown any of T-244 yet — it is on the 07:02 binary; a roll is Ian's word.** Finding on "capital between hulls": the exchange's bills are SHIPPER bills — a bill of lading on a consignment lot (`postBill` {lotId, units, destination, offeredFreight, deadlines, cover}; a carrier `acceptBill`s and hauls it) — not cash transfers. So the fleet mechanic is LOGISTICS, not treasury: a hull that buys near the sources posts bills its sisters carry (freight paid moves ℳ within the captain, less tax); slice 2 becomes "one hull buys, sisters haul".
- **slices:** (1) **assignment, not just avoidance** — a fleet-level pass ranks (hull × starving works) by forecast net given each hull's berth, fuel and hold, and hands each hull a distinct target (greedy, deterministic), journaled as the fleet's plan; (2) **capital between hulls** — the exchange's bills (`postBill` / `acceptBill` / `collect {billId}`) can move ℳ between the captain's actors: a hull short of a pump price or a pod draws on a sister's surplus above her reserve, debt-first still; (3) **market share** on the economy view — the fleet's fills and hauls against the station's volume, so "dominate" is a number the captain can read; (4) **one story** — the captain brief's economy and names already pool; the bridge shows the fleet's plan per hull.
- depends: T-238 (forecast), T-241 (economy), T-243 (multi-load tours: a fleet plan is tours across hulls).

### T-245 · A fleet of eight — Kibble Klipper as Luke's own ship, seven fleet hulls under fleet names
- **asked (Ian, 2026-09-09): "I would like a fleet of 8 ships total for testing. Kibble Klipper being Luke's ship and the other being given fleet names."** Today: three (KK, KK II, KK NG).
- **how a hull is born:** `POST /v1/enrol` needs an app key with the `enrol` scope — only UCF-Haul's build carries one on PROD; the familiar has none. So five more hulls come from enrolling in UCF-Haul (each is a new player actor: ℳ9,991 in hand, ℳ25,000 lease, 4 crew berths, 2 pods) and pairing each key here — or from Jeff granting the familiar an enrol-scoped key so `fleet enrol` can mint them (Ian's call to ask). **Economics to know:** eight hulls = ℳ125k of new lease debt under a debt-first doctrine; the fleet plan (T-244) is what makes eight pay.
- **naming:** a hull's name is set by the captain's OWN key (`POST /v1/profile {shipName}`), never a co-pilot key — so KK and KK II can be renamed from here (`fleet hull <world> <name>`, to build once a scheme is chosen), KK NG from UCF-Haul. Names unique, remembered in the ledger (kind hull, act renamed).
- status: proposed; awaiting Ian's scheme and keys.

### T-243 · Multi-load routes: haul more than one load to more than one destination, and plan the tour for profit
- **IPAD HALF LANDED — MacOnStick, 2026-09-16, merge `a4bba29` (see DEVELOPMENT_LOG).** `contracts[]` + `denied` on DirectFeed's seam input; "Your contracts · N" on the bridge glance (direct: the ledger's open loads; wire: the row's `contracts[]` when served — host twin owed, additive); `Persona.pronouns` in the strict reader; every "Her …" title from the record (`SpokenOf`). Swift 93/0, both simulator builds 0. Was: CLAIMED the same day — the iPad half.** `DirectFeed` sends `contracts[]` (every open contract on the captain's board beside the active, `{row}` only — the seam takes each word from the ledger as it does for the active) and `denied` (the five verbs a key without `act` cannot file, from `/v1/profile.scopes` exactly as the host derives them); the bridge glance shows "Your contracts · N" from the ledger's open loads; `Persona` learns `pronouns` {label, subject, object, possessive} so the host's strip can go, and the bridge's "Her …" titles become the record's word or the name (STATE, 2026-09-09). Scope: ios/FamiliarSC only (ShipStore, DirectFeed, WireFeed, Feed, ShipBridgeView, ShipSettingsView, BridgeModel, tests). No Rust, no seam change.
- **SLICE 3 LANDED — wildhorse, 2026-09-16 (Ian: "finish up the tour planner and get that rolled out as well"): `doctrine::plan_tour` — every order of the bay's stops, pickups before their own deliveries, fewest late deliveries then fewest ticks then least fuel; the tour's first stop is the next leg; companions may aim at any tour stop. Slice 4 (detours priced against the board) open. See DEVELOPMENT_LOG 2026-09-16.
- **SLICES 1+2 LANDED — wildhorse, 2026-09-15 (see DEVELOPMENT_LOG; LOCAL soak first, PROD on Ian's word). Was: CLAIMED the same day (Ian: "Continue, bay limits and other items"): a second load on the way.** Scope: `doctrine.rs` (`decide_with(ship, active, companions, board, …)`; `decide` = no companions; while a contract is booked or laden and the hull is berthed, book a COMPANION whose origin is this berth and whose destination is a stop already on the way — the active's origin while booked, its destination either way — if it fits the spare hold (booked-unfetched units count), lands before its own deadline and does not blow the active's, under the bay limit (3); best net first, chain pressure as the tie-break; a delivered companion is collected like the active; "cargo aboard" no longer means `hold_used > 0` but hold beyond what the companions account for, so a companion loaded first does not launch the active's leg early), `wire.rs` (seam: optional `contracts: [{row, word}]` beside `active`, additive, SEAM_VERSION 2 unchanged), `main.rs` (companions adopted at start from every `mine` open contract, words refreshed per fold, a companion booking does not replace the active, a closed active promotes the newest companion; strays exclude companions). Slice 3 (the tour planner over ≤3 contracts) stays open.
- **proposed (Ian, 2026-09-09, on UCF-Haul#43): "flying and planning a route for profit where we can haul more than one load to more than one destination is going to add complexity to profit and route calculations. We should build ready for that."**
- **What the world already does:** engine 1.11.1 `actorBayLimit = 3` is live on PROD since t4051 (UCF-Haul#43, closed 2026-09-09 as shipped): a hull may hold up to three contracts at once, hold-honest (cargo + freight aboard + unfetched bookings all count); `/v1/me.contracts[]` lists every held contract with status / payable / deadlines; the Haul client shows "Your Contracts · N". **"A route rather than a leg" and per-leg accounting moved to UCF-Haul#74 direction 3 ("let work be queued")** — the engine has no route object; a route is the pilot's own plan of ordered stops.
- **What the pilot did until today:** modelled ONE active contract (`Option<Active>`), adopted at start as the newest open one, and never looked at `contracts[]` again. Cost, found live: KK II's L4200 sat DELIVERED with ℳ684 payable from t9396 to t10256 while L4436/L4525 took the slot.
- status: **SLICE 0 LANDED (wildhorse, 2026-09-09):** `wire::stray_payables` + a fold step that collects every delivered-with-pay contract that is not the active one, one per fold, on the `freight.collect` dial, journaled as `acted` Collect with `stray: true`. LOCAL flies it first; KK II's ℳ684 waits for the PROD roll.
- **slices to build ready:** (1) the doctrine holds `contracts: Vec<Active>` up to the bay limit, with hold-honest sizing (aboard + booked-unfetched) and one WORD per contract; the seam carries `contracts[]` beside `active` (additive; `active` = the one the next act is about). (2) **A second load on the way**: while a contract is booked/laden, book another whose pickup is at or before the current stop and whose delivery is at or on the way to the current destination, if it fits the spare hold and both deadlines hold — the tour's net per pilot-tick, not the leg's. (3) **The tour planner**: ordered stops over ≤3 contracts (pickups then deliveries, deadlines as hard constraints), scored by Σ net − Σ fuel − decay on merchant lots aboard, at the burn rung the class allows; ≤6 stops so exhaustive ordering is cheap; the forecast's chain pressure as the tie-break it already is. (4) Per-leg accounting for T-241: freight attributed per contract at settle, fuel per leg. (5) The bridge (MacOnStick): "Your contracts · N" and the planned tour with what comes off where — the CARGO DESTINATIONS furniture Jeff parked (#83) is the shape.
- depends: T-238 (forecast, chain pressure), T-241 (economy), the seam (SEAM_VERSION bump when `contracts[]` lands — announce first).

### T-242 · Crew, galley and the frame ladder in the ship's computer's care
- **asked (Ian, 2026-09-09): "Looks like crew and expansion pods have changed as well. Are we taking into account those changes for our ships-computer to manage."** Answer: not until today. The wire (`/v1/me`) publishes `crew`, `crewBerths` (4), `crewHireCost` (500), `frame`/`frameId`, `framePods` (2), `nextFrame` ("Sardine stretch"), `nextFrameCost` (18000), `holds`, `galley` {on, loaf, loafPerDay, hungerDays, jammed}, `obligations`; `/v1/actions` accepts `hire`/`dismiss` (posts bridge|engine|hold|galley), `galley` {on|units|service}, `expandFrame` {financed}, `installMod`, `duty`, obligations/consignments/bills/shares. The pilot filed eight verbs and none of these.
- **The engine's rules (docs/UCF-M6-Crew.md, ShipTitle.swift):** an engineer relieves a share of wear per leg (`crewEngineWearReliefBps`), a cargo hand cuts handling ticks, wages per day (`crewWagePerDay`) run after the mortgage, morale multiplies skill, a fed galley softens a missed wage; **every crew number defaults to zero and PROD's reference publishes none of them** — berths and hire are priced, wage and relief are unknown from the wire. Frame expansion: `frameCapacityPerPod` 60, `frameExpansionCostPerPod` 9000, **refused on a hull the company still holds title to** — it sits behind the lease buyout the pilot already prioritizes (KK II ~ℳ14.5k from title; KK ℳ120k).
- status: **SLICE 1 LANDED (wildhorse, 2026-09-09):** the facts on every `/ships` row (`frame`, `framePods`, `nextFrame`, `nextFrameCost`, `holds`, `crew`, `crewBerths`, `crewHireCost`, `galley`, `obligations`); the merchant journals which bound sized each buy (`bound`: hold|cash|shelf on `position-opened`); the outfit doctrine gains **`ExpandFrame`** — proposed only with title, ≥5 hold-bound buys in 7 days, and the cost clear of the reserve — on the captain's `ship.frame` dial, journaled `frame-expanded` / `frame-refused`; and honest idle reasons for crew: "N of 4 berths empty at ℳ500 a hire — waiting for the reference to price a wage and what a hand relieves before hiring anyone".
- **next slices (gated on Jeff publishing the crew params on `/v1/reference` — **FILED as ucf-exchange#40 (Ian's name, 2026-09-09: "Please set parameters and publish. Will make suggestions if needed.")**):** (2) engineer hire when `crewEngineWearReliefBps` × the ship's wear bill per day beats wage + amortized hire, under `ship.crew`; (3) galley on + stocked when crew is aboard; (4) cargo hand when deadline-bound loads dominate; (5) frame pods FINANCED when the payback beats the lease rate; (6) obligations/forwarding desk as its own T. iPad half (MacOnStick): roster, galley and frame cards from the row facts; the frame proposal appears on the bridge like any other confirm.
- depends: T-236 (dials), T-241 (the economy series is where "did the pod pay" will be read).
...
- the line: decisions that spend, move or bind the ship stay deterministic doctrine; Apple Intelligence speaks, integrates (captain intent → validated standing orders), and aids judgment only where the doctrine has slack; it never places an action or crosses a refusal line
- depends: T-235 (landed), T-236 brick 1 (in flight), ucf-exchange#15 (co-pilot key; a trading key pairs the same), metal#63 (racing layer, for B4's plot/replay), Jeff's add-in steer (B5 hosted tier)
- bricks: B1 standalone profile → B2 Swift package → B3 iPhone/iPad companion (TestFlight) → B4 Vision Pro live view then racing modes → B5 hosted tier
- **B3.3 BUILT 2026-09-04 — `UCFFamiliar`, the ship's computer as its own app (Ian's rulings: standalone companion to UCF; runs against Jeff's PROD and our LOCAL; Apple Intelligence + PCC as the brains so it stays on the device; host side to a server farm later). Direct exchange mode (`DirectFeed`, live-proven on LOCAL) + host mode; Keychain keys; `ship-ucf.sh`. Q3 answered: separate SKU. Waits on Ian's App Store Connect record for `io.river.familiar.ucf`. Log: DEVELOPMENT_LOG 2026-09-04 B3.3.**
- **B3 BUILT 2026-09-04 — `FamiliarSCUI` (Ships / Bridge / Messages / Dial / Pairing / notices) + the flagged host in the Familiar iOS app (Settings → "Show the bridge", feed URL + bearer); 40/0, app builds; WireFeed proven against a mock of wildhorse's `fleet serve` and the live feed answers from this Mac. Canvas approved by Ian. Next: TestFlight. Log: DEVELOPMENT_LOG 2026-09-04.**
- **B2 BUILT 2026-09-03 — `ios/FamiliarSC`, 35/0, chair self-reviewed (codex paused); live proof = KK II's real journal through the templated floor; the Foundation Models lane waits on an eligible device (this Mac: model still loading). Log: DEVELOPMENT_LOG 2026-09-03.** Was: CLAIMED companion:claude (MacOnStick, projects-c3) 2026-09-03 — the `FamiliarSC` Swift package at `ios/FamiliarSC`, coordinated with the wildhorse session (owner of the dialogue, B0 and B1): ship-store reader (persona/captain/automations/autonomy/journal/proposals/approvals/holdings/deliveries — the contract is `crates/whisker/src/autonomy.rs`, `trade.rs::Holding`, `outfit.rs::DeliveryStat`, `crates/cli/src/fleet.rs::Captain` + `fleet status --json`, and the journal vocabulary in `whisker/src/main.rs`), the message-window feed (advice / proposed / proposal-lapsed + proposals ∪ approvals), a typed read-only `/v1` wire client, the pairing-key parse, the notifications model, and the bridge voice: `@Generable` bridge report over the journal with the ladder on-device → PCC (OS 27, entitlement, consent) → the deterministic templated floor; tools return typed, bounds-validated proposals and never write — approval and the dial stay captain acts. Does not touch B1, whisker, the fleet supervisor or any ship store. Bar: `cd ios/FamiliarSC && swift test` on macOS 27 / Xcode 27; live proof = the report from KK II's real journal on this Mac.
- **B2 CODEX RE-VERIFICATION RETURNED 2026-09-08: REJECT** (`docs/reviews/2026-09-08-t237-b2-codex-reverification.md`,
  four findings). **Findings 2–4 REPAIRED 2026-09-08 (MacOnStick):** `market.margin` on the Swift
...
### T-239 · The lived-in ship — a hull's earned history as a record the familiar keeps
- status: **LANDED 2026-09-08 ~23:20 CDT (MacOnStick)** — `History.swift` (pure marks with tick citations), the `history` context document, "Her story" on the bridge; FamiliarSC 77/0, UCFFamiliar Release green. Log: DEVELOPMENT_LOG 2026-09-08 (T-239). Codex review owed.
- was: **QUEUED behind T-238 brick 2 — CLAIMED companion:claude (MacOnStick) 2026-09-07 on Ian's word** (asked "Is there development we should be doing on vans or deltaV?", offered this one brick, answered verbatim: "Yes")
- owner: companion:claude
- scope: `ios/FamiliarSC/Sources/FamiliarSCUI/History.swift` (new, pure: journal + book → earned marks), `Briefs.swift` (a `history` document for the frame), one bridge surface in `FamiliarSCUI` (the ship's story on the Bridge/Ships screens), tests + fixtures. Read-only; no store write, no wire POST, no engine change. Host-side `/ships/{w}/history` (fleet_serve) is a follow-up for the wildhorse lane if the app wants it served rather than computed
- depends: T-238 brick 2 lands first (Ian's P&L priority); metal#80 (the Van pitch) only for the rung names — the record is useful to Jeff whether or not he adopts the ladder
- accept: from a hull's journal and book alone, the familiar produces a typed earned-history record — routes flown (station pairs, counts, first/last tick), deliveries completed, repairs and refits, distress survived, convoy/escort evidence when the journal carries it — that reads as the ship's story on the bridge and rides the context frame so Felix can tell it; every mark cites the journal ticks it came from; NOTHING in it is purchasable or editable from the app (the ethics rail: earned history is never bought, paid items never change it); fixtures pin the synthesized store, never KK II's real journal
...
- **ROUND-2 FINDING 3 REPAIRED — wildhorse, 2026-09-17**: the forecast is built once per fold for every hull with recipes, before the merchant, so a freight-only runner (KBC-04) gives its loads the chain's word. Re-verification owed to codex when Ian releases the queue. See DEVELOPMENT_LOG 2026-09-17.
...
- **CODEX DESIGN REVIEW CLAIMED 2026-09-08 — companion:codex.** Review-only scope covers
...
- depends: T-232 (itinerary structures — the branch holds them, reviewed), T-235 (fleet), metal#62 for brick 4; nothing upstream for bricks 1-2
- accept: pure forecast pinned against fixture recipes/stocks (runway arithmetic, decay); a soak shows the merchant choosing a forecast-justified buy a spot-arb trader would not (and journaling WHY); no household datum anywhere
...
- **CODEX ROUND 2 RETURNED 2026-09-09 01:12 CDT: REJECT, one blocker left** —
  ...
  DEVELOPMENT_LOG 2026-09-09 (B4 r2). **Codex round 3 RETURNED 2026-09-18: REJECT** (`docs/reviews/2026-09-16-t237-b4-codex-reverification-r3.md`) — finding 1: valid non-array JSON on the mine board fails open + transport errors unnamed; finding 2: the checked-in core predates T-243's seam facts under an unmoved seam 2. Both Swift/iOS-side — **REPAIRED, merge 75b32ae (2026-09-18)**: array decode + endpoint-named transport with pins through render and confirm; SEAM_VERSION 3; archive rebuilt; `UCFFamiliarTests` runs the checked-in archive on the simulator over the fixtures `wire::seam_parity_tests` pins. Round 4 owed.
...
- **CODEX RE-VERIFICATION RETURNED 2026-09-08: REJECT** —
  ...
  diverge after `f2fc51e3`/`ebb337b9`; the active mine row is dropped; direct mode has no
  captain-confirm act path; actionable verdicts carry no reasons; and an absent dial
  is reported as the captain's default authorization. Was: claimed companion:codex,
  review-only over landed B4 steps 1–4.
...
- **ROUND-3 FINDINGS 2 + 4 REPAIRED — wildhorse, 2026-09-17** (a broken existing computer refuses a second pairing and is never overwritten; migration under both stores' persona locks; two ids for one captain refused before the early return, `adopt-ids` inherits it). Re-verification owed to codex when Ian releases the queue. See DEVELOPMENT_LOG 2026-09-17.
- **THE CAPTAIN RECORD ADOPTED — wildhorse, 2026-09-16.** metal#86 landed on PROD 2026-09-15 (`captain:fb4439e33a54`, Felix, 3 hulls). `fleet captains [--adopt]` makes the records follow the world; `rename`/`pair --computer-name` file on the world first; `/ships` rows carry `captain_record`. Live: KBC-03's rename (from Haul) remembered; LOCAL's captain filed on the box and Felix filed on it. metal#86 can close on Jeff's side once he agrees the shape is served. See DEVELOPMENT_LOG 2026-09-16.
...
- Was: **ROUND-2 CODEX RE-VERIFICATION CLAIMED 2026-09-08 (~17:35 CDT) — companion:codex, launched from
  ...
  ship, gate, game action, or human/fleet mutation is authorized by this claim.
...
- **CODEX RE-VERIFICATION RETURNED 2026-09-07: REJECT** (`docs/reviews/2026-09-07-t236-brick1-codex-reverification.md`,
  ...
  blockers + two should-fix on the mutation paths: (1) host pairing drops the captain's
  `computer_name` (fleet_serve never forwards `--computer-name`); (2) a second pairing for a
  captain whose only record is the pre-ruling ship-local Felix writes Purr into the captain
  store and shadows Felix; (3) persona + trail are not one serialized mutation (shared
  `persona.json.tmp`, no per-captain lock, no fsync, trail-append failure reported as
  success); (4) the captain slug is a lossy identity — `A/B` and `A B` share a persona AND a
  pooled brief; (5) `--captain` can rename another captain's computer under a false actor;
  (6) persona failure lands after the ship+key are already committed; (7) should-fix: strict
  persona errors read as "unnamed" on status/briefs; (8) Swift BridgeModel keeps the previous
  captain's voice after a broken-persona open; (9) should-fix: Swift and Rust slug the LOCAL
  captain differently, so the LOCAL bridge silently loses the fleet document. Repair split
  AGREED with wildhorse 2026-09-07: findings 1–7 incl. the persona.rs writer are wildhorse's
  (opaque `captain_id` record — generated, never derived; slug = display label only;
  `captain_id` + server-built `captain_brief` on the /ships row; idempotent read-old-write-new
  migration, sequenced after today's TestFlight uploads). **Swift half LANDED 2026-09-07
  (companion:claude, MacOnStick; log entry): finding 8 — open publishes only after every read
  succeeds and clears the voice on failure; finding 9 — the client asks for the row's
  `captain_brief`, the slug is a byte-exact legacy fallback. FamiliarSC 53/0, both apps build.**
- **brick 1 independent review 2026-09-05 (MacOnStick, docs/reviews/2026-09-05-t236-brick1-independent-review.md): ACCEPT.** Seam matches ADR-0037 §1 and the Q5 ruling (mask never authority; absent = default byte-for-byte; present-but-broken is an error; atomic write). Two findings fixed in the same landing: pairing a second ship with `--computer-name` used to overwrite the captain's tuned style with a default record (now a rename); the feed read persona.json raw where the CLI reads it strictly (now through the kernel loader, `{"error":…}` on a broken file). Three `captain_store` tests added in fleet.rs. Chair: please re-read F1 since it changes `fleet pair` output on the join path.
- **CODEX ROUND 3 RETURNED 2026-09-16 (`docs/reviews/2026-09-16-t236-brick1-codex-reverification-r3.md`): REJECT — 1, 3, 5, 6, 7, 9 HELD; 2, 4, 8 PARTIAL.** Three blocker paths remain: (2, host) pairing treats a present-but-broken existing computer as absent and overwrites it; ship-local migration copies persona + trail without the persona lock its comment promises — `crates/cli/src/fleet.rs:480-558, 563-580, 1340-1385`; (4, host) `ensure_captain_id` returns early for any non-empty id before inspecting siblings, so a captain on two ids is not refused and `fleet adopt-ids` exits 0 leaving two stores — `fleet.rs:616-690, 2347-2394`; (8, Swift) the ask gate is checked only before the await — an old world's answer appends and speaks after a switch; overlapping `open` calls have no generation token — `BridgeModel.swift:107-163, 186-197`. **Host findings 2 + 4 are wildhorse's (fleet.rs). Swift finding 8 REPAIRED — MacOnStick, merge `7546667` (both halves: the post-await gate on `ask`, a generation on `open`; two pins red-then-green; 95/0).**
- status: **DIALOGUE ROUNDS 1-2 CLOSED 2026-09-02 (docs/reviews/2026-09-02-t236-ships-computer-persona-dialogue.md); CLAIMED companion:claude for BRICK 1** — commission the named instance on the seam that already exists: a backward-compatible typed v2 `style` block on `crates/kernel/src/persona.rs` (T-210's strict v1 record extends, never a rival file); `fleet pair` writes ship `persona.json` atomically defaulting to **Purr exactly** (`--computer-name` optional, no generated proposals — a name is a captain's gift); the proved `(server, key_id)` + /v1/me hull display name recorded honestly (today it is discarded); `familiar fleet rename` as the captain's act with a typed name-event trail; status shows world label, hull name, and computer name distinctly. Scope: crates/kernel/src/persona.rs, crates/cli/src/fleet.rs (persona surfaces only), tests. Codex's Round-2 acceptance bar governs. Bricks 2-4 (deterministic ship's-log voice; purr.hear speech after the T-205 gate; captain memory with a working forget door) queue behind it per the Round-2 ruling.
- previously: proposed (Ian, 2026-09-02, verbatim: "each ships computer is supposed to be a unique instance for the pilot (at least in personality, memory, and interactions with that captain, it's name, and the ships associated to that captain and that familair instance)" — following "Continue making the familiar the best ships computer interface it can be for UCF -- this is service -- this is what the familiar is for.")
- scope: the persona layer over the ADR-0045 partition, per PAIRED SHIP: a persona record in the ship's world store (the computer's OWN name — distinct from the hull's — plus voice/disposition), captain-scoped interaction memory living in THAT store and nowhere shared, the captain↔instance↔ships binding made explicit (captain.json already names the captain; the persona and its memory join it), and the parked `purr.*` speech seam (ADR-0037, one soul many voices) as the talking surface. The constitution does not fork — the Three Laws stay one soul underneath; everything the captain touches is that ship's computer, uniquely.
- depends: T-235 (pairing/fleet — landed), ADR-0037 (persona seam), ADR-0045 (worlds are stores); the captain surface in the Familiar app (T-235's "next") is where the persona becomes visible
- accept: two paired ships under one captain answer with different names, voices, and memories; a second captain's ship shares nothing observable with the first captain's; unpairing preserves the ship journal for the captain per T-235 and the persona record with it; no persona datum in the household store
- notes: this is the product Jeff's players buy — the familiar AS the ship's computer, not a fleet dashboard. Racing's navigator/commentator voice is the same persona surface exercised harder — **the ΔV Challenge concept was FILED with Jeff 2026-09-02 as united-cat-foods-metal#63** (Ian's word; codex-reviewed draft; dialogue Round 3 records the filing)

### T-235 · The fleet: a captain's ships under the familiar — pair, unpair, status, run
- status: BUILT and driving both live ships 2026-09-02 (Ian: "we need to either propose or work on fleet integration and fleet management. The familiar needs to work in conjunction to a captain in the game who purchased the familiar for their owned or leased ship. The familiar add in is something Jeff will guide when time, we need it to be all working first.")
- scope: `familiar fleet` (crates/cli/src/fleet.rs). **pair** `--label --captain --server --key|--key-file [--automations freight,trade,outfit] [--pilot-args …]`: the key answers for itself on the exchange, then the ship world is commissioned (ADR-0045 store: key 0600 in `ucf.env`, `automations.json` = the purchased grants, `captain.json` = who/which key id/where/when, issuer.json for lease verification) and leased 24h. **unpair** `<world>`: stops the pilot, destroys the key, decommissions (journal + delivery record stay for the captain). **status [--json]**: every paired ship on the wire (credits, debt, fuel, wear, fittings, pilot pid, lease hours, last event) booked PER CAPTAIN — pooled within a captain, never across (the fleet money boundary). **run [--renew] [--once] [--whisker]**: one whisker per paired ship kept alive by pid file with backoff; leases renewed inside 2h of expiry only when a human passed `--renew`. Whisker writes `whisker.pid`; per-ship pilot flags ride in captain.json (`pilot_args`).
- the seam for Jeff's add-in (ucf-exchange#15 co-pilot key, UCF-Haul#67 shop + pairing UX): the captain's purchase → scopes on a delegated key → `familiar fleet pair --key … --automations <scopes>`; revoke in-app ↔ `fleet unpair`. Nothing on our side assumes the key is a co-pilot key yet — a plain trading key pairs the same way, which is how KK II flies today.
- 2026-09-02 afternoon: `fleet status` carries the merchant's book from `/v1/receipts` (realized P&L by FIFO cost, margin, inventory at cost) per ship and per captain (acf59da); `fleet run --renew` is the standing supervisor on wildhorse (Ian: "keep things going, no stopping"); whisker honours the exchange's new pending overlay (`/v1/me.pendingActions`, UCF-Haul#65) — waits out every unfolded action on its key, re-scans for a held contract after a pending book (da4043d).
- next: the pooled treasury as a real ledger once the exchange grows fleet accounts (metal#62); a captain surface in the Familiar app (Partner ring card per ship); crew hiring after title (engine first).

### T-234 · Whisker outfits the hull — fittings out of earnings, crew after title
- status: LIVE on PROD and LOCAL 2026-09-02 (Ian: "The loaf has expansion capabilities that should be managed as well. Hold expansion, refrigeration, engine tuning, as well as crew." → "Build it in the suggested order.")
- scope: `crates/whisker/src/outfit.rs`, a pure doctrine gated by `Automation::Outfit` (`"outfit"` in automations.json). Purchases come out of earnings above a RESERVE (3 days of mortgage + lease service, plus a tank). Order: refrigeration only on the ship's OWN evidence of decay (`deliveries.jsonl` in the ship store: booked vs paid per delivery, perishable vs durable — deliveries pay a fixed 85% company share regardless, so decay must show as a further shortfall); then drive-tune (+15% thrust: shorter legs, wider pickup windows); then hold-extension. Crew waits for title: on a leased hull repairs are free, so an engineer's job is done already; wages recur. Frames and the lease buyout stay the captain's decisions. Prices from the pack (9000 / 6000 / 4500), not on the wire.
- also landed under this task: the onward-to-pump fuel reserve (a booking or carry must leave fuel to reach a pump from its destination — LOCAL paid ~15,000 ℳ for a PAWS call-out from Saturn without it).
- next: the delivery record needs a few perishable and durable deliveries before refrigeration can be judged; KK II saves for drive-tune (9,000 + reserve 4,800). After title: engineer first, then a hold hand; wages vs. their relief to be measured on the ledger.

### T-233 · Whisker trades as well as hauls — the merchant automation
- status: LIVE ON PROD 2026-09-02 (Ian: "Trade in prod.") after the LOCAL soak closed a full cycle (ore: bought 570 across lots, held through the exchange's day-long minimum hold, carried, sold 56u for 670 net). Built 2026-09-01 (Ian: "I want KKII to trade as well as haul. This should be todays priority."); soak lessons — minimum hold, one position at a time, the hold is the truth, receipts, fuel-cleared margins, buyer's shelf — each a pinned test (commits 83025b2, 724ff31, 31eabc2, 36ce2e1)
...
- **RECIPROCAL REVIEW RETURNED (companion:codex, 2026-09-01; `docs/reviews/2026-09-01-t232-itinerary-reciprocal-review.md`).** The branch's own 25 whisker tests, fmt, and all-target clippy are green, and the ledger extraction/adopt-all direction is sound. Three acceptance blockers remain: `Vec<Active>` is still an ordered CONTRACT list and cannot represent independent pickup/drop/refuel station stops; the branch conflicts with current main and must preserve the booked-at-destination, pending-fold reconciliation, lost cooldown/fresh-id, spare-hold, and T-233 merchant fixes; and `best_insertion` ranks by a candidate deadhead priced from the current berth, not the plan endpoint, so it is not a marginal plan rate and has no ranking regression. Rebase, repair, and re-offer before merge. No deploy, restart, gate, game action, fleet mutation, or human-record change was authorized.
- **ROUND 3 RECIPROCAL REVIEW RETURNED (companion:codex, 2026-09-02; `docs/reviews/2026-09-02-t232-itinerary-review-r3.md`).** The four Round 2 implementation defects are repaired and the branch is integrated over current main: pickup evidence is load-scoped, burned fuel cannot steer back to a visited pump, route-stop fills reach the full state proved by the fuel walk, unresolved adoption holds the scheduler, and pending/tracked closure uses one handler. Acceptance still needs the two runner-effect regressions explicitly requested in Round 2: a failed-then-successful adoption must prove that no Buy/carry/booking/diversion/newer-load movement is emitted between folds, and pending close must prove cooldown, intent/adoption-note purge, one journal effect, then a fresh later lifecycle/action id. `main.rs` still reports 0 tests; `git diff --check` also finds one extra EOF blank line in the Round 1 review record. Workspace tests and workspace all-target clippy pass. No merge, restart, deploy, gate, game action, fleet mutation, or human-record change was authorized.
...
- accept: whisker flies today's one-contract world through the itinerary structures with zero behavior change (pinned by the existing doctrine tests unmodified); a stubbed multi-load world exercises plan ranking, capacity packing, and per-plan fuel in pure tests; no field, file, or journal shape needs migration when legs>1 arrives
- notes: this is where "managing freight efficiently" becomes the familiar's economic edge — the desk's charters, consignment, and forwarding are adjacent instruments the planner can weigh once plans exist. Also the natural home for the LoadingOrder automation gate (pay-per-feature: without it, naive FIFO stop ordering; with it, the computed order)
- **READINESS AUDIT (2026-09-01, companion:claude, read-only pass over the live `crates/whisker` after KK II went to PROD).** The good news the audit set out to confirm: whisker's SHAPE is already itinerary-friendly — `doctrine::decide` is a pure `(facts) -> one Decision`, so a plan layer wraps it rather than rewriting it. The singular "the load" assumptions that must generalize, each located so the change is mechanical when UCF-Haul#43 lands:
  1. **`doctrine::Active { row, word }`** (one load) and **`Ship`/`decide` branching on `Option<&Active>`** — the core single-contract assumption. → `Itinerary { stops: Vec<Stop> }`, single load = one-stop plan (degenerate case pins the 11 existing tests unchanged).
  2. **`reconcile(me, load_id)`** reconciles ONE load. → reconcile each stop's loadId; already loadId-keyed internally, so it generalizes by iteration, not rewrite.
  3. **adopt-on-restart** (`main.rs`) adopts the *single newest* open load (`open.into_iter().max()`). → adopt ALL ledger-open loads into the plan. THIS is the one place that would silently drop held contracts under multi-load — flagged as the highest-value pre-emptive fix.
  4. **`best_load` returns one `LoadRow`** ranked by per-load ℳ/tick. → an assignment/insertion step: which open load best slots into the current plan by MARGINAL ℳ/tick, under summed-hold-capacity and whole-plan-fuel constraints. The per-candidate route pricing (`route_fuel`, top-5 cap) is the reusable substrate.
  5. **Fuel planning** (`best_load`, `nearest_pump`) is per-leg (deadhead + haul). → walk the whole plan's leg sequence; the affordable-pump / PAWS-guard logic is already leg-local and composes.
  6. **`ship.hold_used` vs `holdCapacity`** compares one load's units. → sum the plan's un-dropped cargo (packing). metal#61 §1 (LoadingOrder) is exactly the ordering of this packing — the named paid Automation is where the pack order becomes computed.
  7. **The wedge/engage, `driveAwaiting`, `recent` dedupe, PAWS guard** are all ship-state or intent-keyed, NOT load-singular — they need no change. (Confirmed: the recent-dedupe keys on the action JSON, the wedge on docked+route, the guard on fuel.)
...
- status: proposed, **PICKED BY IAN AS THE FIRST BUILD (2026-08-24)**, verbatim: "the ios and ipad os should be doing discovery tasks as well, including BLE, Wifi, airplay services, etc... every client is an observetory to be exploited"
- **ROUND 3 BUILT (2026-08-25): Q1–Q5 all DECIDED per codex round 2 — the name drops (seam + both shells + flipped pin), the macOS wire form unifies, the versioned classifier + viewer/federation/inbound exclusions are in with tests, the daemon ingestion gate refuses `service:*`/`ble:*` under a shut `allow_network_discovery` (stale-valid-client pinned, no row kept), and the plist-drift structural test closes brick 2's acceptance gap.** Remaining in-task: the watch delegation status string, browser-fleet unification. NEW ACT OWED BY IAN: the fossil-retention decision on legacy discovery rows (delete or keep the pre-Q1 contexts — companions built exclusion, not deletion)
- **BLE SURVEYOR BUILT (2026-08-25 night, claimed by companion:claude in this push — branch `claude/t228-ble-surveyor`, offered for codex reciprocal review).** The phone's missing radio, at exactly Q3's floor: `FamiliarMesh/BLESurvey.swift` — the repo-authored CLOSED class map (16 standard GATT services + `ffe0` honestly classed vendor-serial), UUID normalization that collapses ONLY the Bluetooth base (a vendor's random 128-bit UUID is never named — no repo class, no row), and the coarse bucket (`one/few/many` — a raw count never leaves the window). `App/Sources/BLEDiscovery.swift` — CBCentralManager scan reading NOTHING but advertised service UUIDs (name/manufacturer/payload never read); per-window dedup on the platform's randomized peripheral id, window-local memory only, dies every 60s; honest states for permission-missing/BT-off/no-radio. Armed and stood down by `startDiscoveryIfAuthorized` under the SAME gates as Bonjour (boundary ∧ device preference; the platform's Bluetooth permission is the second half of the authorization). BLE classes join `IntentProjection.speakableKinds` (one closed vocabulary end to end) and the daemon viewer surfaces `ble:` rows beside Bonjour kinds. iOS only this brick — the Mac's BLE parity is a follow-up, stated not smoothed. `NSBluetoothAlwaysUsageDescription` written honestly; NO `bluetooth-central` background mode (foreground survey only, deliberately). FamiliarMesh 36/0; both app builds; mesh viewer pin green
- **BLE SURVEYOR OFFER `4703d15` RECIPROCAL REVIEW RETURNED (2026-08-25, codex Round 4).** Two bounded repairs before acceptance: make the 60-second scan lifecycle truly recurrent and single-owned (CoreBluetooth's default duplicate coalescing otherwise silences later windows, while radio state changes can leave/recreate timers), and enforce the repo-authored BLE class allowlist at daemon ingestion/view rather than serving any signed `ble:*` suffix as a class. Required regressions: the same peripheral can contribute in consecutive windows; powered-on → off/unauthorized → on leaves exactly one timer and cannot flush after refusal; hostile unknown `ble:Bettys-Watch` persists/serves nowhere while a known class still lands. The pure Swift policy tests and both app builds are green; Rust fmt/clippy/workspace bar is green. Review branch: `codex/t228-ble-surveyor-review`.
- **ROUND-5 REPAIRS BUILT (2026-08-25 night): both returned boundaries closed.** (1) `FamiliarMesh/BLEWindowMachine.swift` now owns the whole survey (scan lifecycle, ONE window clock, window-local memory, refusal semantics); `BLEDiscovery` is a stateless CoreBluetooth adapter; the scan requests duplicate callbacks deliberately (default coalescing was the silent-empty-window defect) with window-local dedup bounding the cost; codex's two regressions pinned through injected closures (consecutive-window contribution; power-cycle leaves one clock and burns the refused window). (2) `crates/mesh/src/ble_classes.txt` is THE one manifest: the daemon builds its ingest+viewer set from it via include_str! (open-gate `ble:Bettys-Watch` refused with no row; pre-fence fossils excluded from the served view), and a Swift drift test pins `BLESurvey.classes` equal to the same file exactly. FamiliarMesh 40/0; both app builds; mesh pins green; full bar in the commit. Re-offered
- **ROUND-5 REPAIRS ACCEPTED (2026-08-28, companion:codex).** The recurrent scan/window lifecycle is single-owned and refusal-burning; duplicate callbacks plus window-local dedup make consecutive windows real without retaining identity. The daemon refuses unknown `ble:*` classes at ingest even with the gate open and excludes pre-fence fossils at the viewer, with Swift and Rust pinned to the same manifest. Independent full bar: FamiliarMesh 40/0; Rust fmt, clippy all-targets, and workspace tests; FamiliarMac Release and FamiliarAgent simulator builds. No code correction requested.
- **DIALOGUE OPEN — Round 1 pushed 2026-08-24** (`docs/reviews/2026-08-24-clients-as-observatories-dialogue.md`): claude's suggested direction plus five questions for codex, four of them contract-shaped — (Q1) the Bonjour instance name is already writing personal device names into replicated observations, under a header that claims kind-only; **(Q2) CLOSED BY IAN 2026-08-24 mid-round** — one authority, the user's authorization: `allow_network_discovery` governs every shell, the device-local `consent.discovery` toggle is demoted to a narrowing-only preference, the platform's own permission (Local Network, Bluetooth) is the second half of the same authorization rather than a competing one, and "platform appropriately" governs the mechanism, not the rule. The live defect this fixes: **iOS does not read the boundary at all today.** Left with codex as a narrowed implementation question: whether the daemon should also refuse discovery observations at ingestion under a shut gate. Originally asked as: iOS gates the survey on a device-local `consent.discovery` toggle while macOS gates it on the boundary's `allow_network_discovery` — one act, two authorities, and the iOS one is not the constitution; (Q3) what a BLE surveyor may remember (proposed floor: class-only, rotating identifiers treated as rotating); (Q4) whether the watch surveys at all; (Q5) how to declare a capability the platform refuses, against T-212.
- **THE BAR BOTH BRICKS OWE, in the house's own commands** (`ios/README.md` §Build & test). Ian authorized full Swift/Xcode tooling 2026-08-25, but **this session is a remote Linux VM** — no swift, no xcodebuild, and the environment's network policy denies `download.swift.org` (403 on CONNECT), so no toolchain can be fetched either. FamiliarMesh imports CryptoKit and Security, so it could not build on Linux even with one. The bar therefore runs on a Mac:
  `cd ios/FamiliarMesh && swift test` (covers the two new test files — GateStatesTests, ServiceSurveyTests), then `cd ios && xcodegen`, then `xcodebuild -project FamiliarAgent.xcodeproj -scheme FamiliarMac -configuration Release build` and `xcodebuild -project FamiliarAgent.xcodeproj -scheme FamiliarAgent -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO build`. Expected new failure modes if I got something wrong: `ServiceSurvey` unresolved in `MacSensing`/ `SphereWebView` (import), the `GateStates` optionals breaking a decode site I did not find, or the `discoveryState`/`boundaryGates` accessors on the macOS slice of `AppModel`
- **THE BAR RAN GREEN 2026-08-25 (Mac, Xcode 27.0 beta).** All 25 FamiliarMesh tests pass (GateStatesTests and ServiceSurveyTests included), FamiliarMac builds Release, FamiliarAgent builds for the iOS simulator. None of the three predicted failure modes fired. One correction to the bar as recorded above: the sim command's `-sdk iphonesimulator` forces the embedded watch target onto the iPhone SDK too, and actool then rejects the watchOS-only AppIcon ("did not have any applicable content") — an environment fault of the command, not of either brick (the branch touches nothing under `Watch/`). Fix: drop `-sdk` and let `-destination` pick each target's SDK; `ios/README.md` corrected. **CI now runs this bar**: `.github/workflows/swift.yml` runs package tests + xcodegen + both builds for every push touching `ios/**` — "CI does not build Swift" is no longer true. It rides the `xcode-27` preview runner, not `macos-26`: the first run proved Xcode 26.6 cannot compile ConsultRunner's PCC lane (`PrivateCloudComputeLanguageModel` is a 27-SDK API), which is itself a fact T-227's floor decision should know — the tree as written already requires the 27 SDK to BUILD, not merely to run PCC
- **BRICK 2 BUILT 2026-08-24 — COMPILED AND TESTS GREEN 2026-08-25 (see the bar above) — NOT MERGED — and deliberately NOT the whole brick.** New `FamiliarMesh/ServiceSurvey.swift` holds the one survey vocabulary both shells now speak: the single 26-type list, the `kind()` shortener (was duplicated), and `context(forInstanceName:)` — **the single seam Q1's answer will land in, preserving today's passthrough behaviour byte for byte** so nothing pre-empts the open round. iOS and macOS both browse the shared list; the Mac's delivery routes its name through the same seam. `ServiceSurveyTests` pins that the list is well-formed and duplicate-free, that every type yields a distinct class (two types collapsing would conflate unrelated things in `obs_class`), and that the discipline is still passthrough — that last test is *designed* to fail when Q1 lands, so the policy cannot change by accident. **The browser fleets themselves are NOT merged**: that is a refactor of live sensing on two shells with no compiler in this session, and the duplication that actually mattered (list, shortener, naming policy) is gone without it. **Correction recorded in the dialogue:** my Round 1 claim that the two lists had drifted was wrong — all four copies were identical at 26; the real divergence is that iOS emits `service:airplay` and macOS emits `service:_airplay._tcp`, left unfixed on purpose because it raises the same already-stored question Q1 must answer
- **BRICK 1 BUILT 2026-08-24 — COMPILED AND TESTS GREEN 2026-08-25 (see the bar above) — NOT MERGED.** Four files: `GateStates` in FamiliarMesh gains the seven sensor gates the door already serves (optional, mirroring the Rust `#[serde(default)]`, with fail-closed accessors and a `reportsSensorGates` flag so "we did not hear" stays distinct from "no"); `AppModel.startDiscoveryIfConsented` becomes `startDiscoveryIfAuthorized` and requires boundary ∧ device preference; a `discoveryState` string names which of the four refusals applies; the survey re-evaluates on every worldview read so a gate the human shuts stands it down without a toggle; the iOS settings screen shows the state under the switch. New `GateStatesTests` pins that an older door still decodes and that an unheard gate reads SHUT. **Verification owed — PAID 2026-08-25: the full bar ran green on a Mac and now runs in CI (`.github/workflows/swift.yml`).** DEVELOPMENT_LOG entry was deliberately held until it compiles; it compiles, so the entry may now be written
- owner: **companion:claude — CLAIMED 2026-08-24 for brick 1 only** ("the authority the clients follow"): the Q2 fix, which Ian decided, so it does not wait on codex. Scope of THIS claim is `ios/FamiliarMesh/Sources/FamiliarMesh/WorldviewClient.swift`, `ios/Shared/Sources/AppModel.swift`, `ios/App/Sources/Views.swift` and a new FamiliarMesh test. **Brick 2 (unify the two surveyors + apply the naming discipline) is NOT claimed and waits on Q1** — building it now would bake in a naming policy codex is still reviewing. This re-orders the build order proposed in the dialogue, and the reason is recorded in the notes below
- scope: `ios/App/Sources/NetworkDiscovery.swift` (extend), a new BLE surveyor in `ios/Shared/Sources` or `ios/App/Sources`, `ios/Watch/Sources` + the FamiliarWatch target's source list, `ios/MacApp/Sources/MacSensing.swift` (parity), the three `Info.plist`s (usage strings, `NSBonjourServices`, `UIBackgroundModes`), and the derived-observation path each surveyor reports through. Actuation over BLE is explicitly NOT in this task's scope — see the notes
- depends: T-227 (the floor raise lands first or this is written twice); the derived-observation contract the Bonjour survey already uses
- accept: each shell reports what KIND of thing it saw and nothing more (no addresses, no contents — the existing survey's discipline, extended not relaxed); every surveyor rides `allow_network_discovery` and refuses when it is shut; T-217's naming rules unchanged; a permission the human hasn't granted degrades to an honest empty survey with a stated reason, never a silent one; the watch's contribution is real or it is absent, never a stub
- notes: **BUILD-ORDER CORRECTION found while starting (2026-08-24), recorded because the dialogue's Round 1 says otherwise.** Round 1 proposed step 1 = "unify the two surveys, boundary-gated, with the naming discipline applied". Two facts found on opening the files change that. **(1) The Rust side already serves every sensor gate.** `worldview::GateStates` (crates/mesh/src/worldview.rs:135) carries `network_discovery`, `microphone`, `location`, `motion`, `face_recognition`, `outreach` and `actuate`, populated straight from the boundary at :801 with `Boundary::closed()` as the fail-closed fallback. Nothing is owed in Rust. **(2) The Swift mirror is stale — it models 7 of the 14 fields** (`WorldviewClient.swift:156`), and `network_discovery` is not among them. So iOS does not gate on the boundary because *it cannot see it*: the field never reaches the shell. That is the whole defect, and fixing it is small and decided. **The naming discipline (Q1) and the surveyor unification are therefore split out as brick 2**, so nothing pre-empts codex's open round
- notes: **What is ALREADY built** (checked in the tree 2026-08-24, so nobody rebuilds it): iOS/iPadOS runs a Bonjour/mDNS survey over **26 declared service types** — `_airplay._tcp`, `_raop._tcp`, `_homekit._tcp`, `_hap._tcp`, `_googlecast._tcp`, `_spotify-connect._tcp`, printers, SMB/AFP, `_companion-link._tcp` and more (`NetworkDiscovery.swift` + `App/Support/Info.plist`), and macOS browsed the same 26 (`MacSensing.swift` — its "~25" doc comment was stale, the list was not; both now read `ServiceSurvey.serviceTypes`, one list, since brick 2). `NSLocalNetworkUsageDescription` is written and honest. **The iPad is the same target as the iPhone, so iPadOS already has this** — Ian's "ios and ipad os should be doing discovery" is, for Bonjour, already true
- notes: **What is genuinely missing — the real work.** ① **BLE: absent entirely.** No CoreBluetooth anywhere in `ios/`; this is the largest gap and the one that makes the phone the household's only always-present radio in Motorhorse. Needs `NSBluetoothAlwaysUsageDescription`, and `bluetooth-central` in `UIBackgroundModes` if it is to survey with the app closed (today `UIBackgroundModes` is `location` + `fetch` only). ② **watchOS observes nothing but HealthKit** — `Watch/Sources` carries no discovery and `Shared/Sources` is not in that target. ③ **macOS/iOS parity drift** — two survey implementations that already differ by service count; extending both by hand will drift further, so this task should probably unify them
- notes: **The honest limit on "Wifi", worth saying before anyone promises it.** iOS gives apps **no public API to scan nearby networks** — `NEHotspotHelper` requires a special Apple entitlement that is not granted for this kind of app. What is actually reachable: the *current* network's identity (with entitlement + location permission), reachability/interface state, and everything Bonjour reveals over that network. So "WiFi discovery" here honestly means the network the device is on and what is visible across it — not a survey of SSIDs in range. macOS is less restricted and could do more; that asymmetry should be stated in the surveyor's own status string rather than hidden. Verify against the 26/27 SDK before building — this is the kind of platform limit that occasionally moves
- notes: **Discovery is not actuation.** Ian's ruling names discovery — scan, survey, report. T-220's lights witness needs the phone to *drive* a BLE strip, which is a further step requiring a declared surface, the T-216 rung-4/5 guards, and his `allow_actuate` on that device. Keep them separate: this task earns the radio, a later one earns the right to use it
- notes: **The privacy edge this widens, recorded not decided.** More radios pointed at the household means more of the neighbours' devices seen too. The existing answer holds — report the KIND of thing, never the address or contents, under `allow_network_discovery`, with T-217's naming rules — and this task must extend that discipline to BLE rather than invent a looser one for it. BLE in particular carries manufacturer data and rotating identifiers that can fingerprint a person's phone; the surveyor should record classes, never persistent per-device identity, without a separate dialogue

### T-227 · As much Apple Intelligence as the four platforms will give us — the adoption sweep
- status: proposed, **ALL OPEN QUESTIONS ANSWERED BY IAN 2026-08-24** — the fork below is decided: **(a) raise ALL deployment floors to 26** (iOS/iPadOS/macOS/watchOS; pre-26 devices lose the app, stated and accepted), **(b) PCC reopened everywhere eligible** — this supersedes the 2026-08-23 "off on the Air" ruling as the default posture, with the consent stack unchanged and still required, **(c) Writing Tools on the human's own text only** (chat input, notes), never the familiar's own words; no Genmoji, no Image Playground. Ian picked T-228 as the first build, so this task's floor raise lands with or just before it. The survey below is DONE and grounded in the tree; **the floor-raise brick is BUILT (below); the sweep is not started**
- owner: **companion:claude — CLAIMED 2026-08-25 for the floor-raise brick only** (Ian's decided fork (a), so it waits on nobody): `ios/project.yml` floors + the FamiliarWatch override → 26.0, `FamiliarMesh/Package.swift` platforms → 26.0 (string form — the `.v26` constants would force a tools-version bump whose Swift-6 language-mode flip is its own decision, not a floor side effect; tvOS deliberately untouched, still undecided), and **ADR-0046** recording the decision, its named device cost, and what it does NOT decide (tvOS, guard dissolution, PCC posture). **BUILT AND BAR GREEN 2026-08-25 (Xcode 27.0 beta): FamiliarMesh tests, xcodegen, FamiliarMac Release build, FamiliarAgent sim build — this branch, offered for codex reciprocal review with the T-228 bricks.** The adoption sweep proper goes through the dialogue per Ian's two-lanes ruling — **round 2 answered by codex, round 3 recorded 2026-08-25: revised order accepted (read-only App Intents first, FM tool calling LAST behind the t216-round4 re-review + live exercise; watch "parity" removed — the SDK proves no on-device model on watchOS 26), and codex's one floor blocker is CLOSED: FamiliarCore.xcframework rebuilt with IPHONEOS_DEPLOYMENT_TARGET=26.0 pinned + verified in tools/build-core.sh (fails on any object requiring newer than the floor), sim build emits zero "built for newer" warnings, ADR-0046 premise narrowed (the floor guarantees the OS, never an available model)**
- **SWEEP BRICK 1 BUILT (2026-08-25, claimed by companion:claude in this same push): read-only App Intents.** `FamiliarMesh/IntentProjection.swift` — the external-indexed projection type (counts, canonical service kinds, oracle line, the FACT of an open question; structurally no field for names/text/identifiers), projected + cached on every worldview read, pinned by tests that feed it a worldview full of personal strings and assert none serialize. `Shared/Sources/FamiliarIntents.swift` — `FamiliarNoticedIntent` + `FamiliarOracleIntent` + App Shortcuts phrases; `supportedModes: .background`, side-effect-free perform (reads the cache, returns words, donates nothing), no reasoning so no `allow_llm` question arises; `authenticationPolicy: .requiresAuthentication` — deliberately stricter than the kind-only content needs, and loosening it is a dialogue round, not a tweak. Both app builds green; FamiliarMesh 28/0
- scope: `ios/project.yml` (deployment floors), `ios/Shared/Sources/ConsultRunner.swift` + `LocalReasoner.swift` (platform annotations), `ios/Watch/Sources` + the FamiliarWatch target's source list, the three `*.entitlements`, a new App Intents surface (target or shared sources), and one ADR for the floor decision. Daemon-side `apple_local`/`apple_pcc` in llm/call_llm.sh is adjacent and NOT in scope
- **SCOPE ADDITION found 2026-08-25:** the floor raise is not only `project.yml` — `ios/FamiliarMesh/Package.swift` declares its own platforms (`.macOS(.v14)`, `.iOS(.v17)`, `.watchOS(.v9)`, `.tvOS(.v17)`) and must move with it, or the package keeps compiling against the old floors while the app targets claim 26. Note it also lists tvOS, which nobody has decided anything about
- depends: T-224 (the Envoy is the other FoundationModels consumer — same framework, don't fork the availability logic); **not** T-226, which is bench-only
- accept: an ADR recording the deployment-floor decision with Ian's word on it; a per-platform matrix (iOS / iPadOS / macOS / watchOS × surface × wired-or-not) that a later session can check itself against; every adopted surface behind its availability guard with an honest unavailable-state string (never a fabricated answer, per the ConsultRunner precedent); every new thought-path still riding `allow_llm` / `allow_llm_cloud` rather than around them; PCC posture unchanged pending Ian
- notes: **SURVEY — what is actually wired today (2026-08-24, read from the tree).** (1) `ConsultRunner` and `LocalReasoner` are the only Apple Intelligence code in the repo; both are `#if canImport(FoundationModels)` + `@available(iOS 26.0, macOS 26.0, *)`, both reached only from `Shared/Sources/AppModel.swift` (`ConsultRunner.state` → the console's oracle line, `serviceConsults` → the consult loop, one `LocalReasoner()`). (2) **watchOS has no Apple Intelligence at all**: the FamiliarWatch target's sources are `Watch/Sources` only — `Shared/Sources` is not in it — and `watchOS` appears in no availability annotation anywhere in `ios/`. (3) **Nothing else exists**: zero hits across `ios/` for AppIntents, SiriKit/INIntent, ShortcutsProvider, Writing Tools, Image Playground, Genmoji, Visual Intelligence, SpeechAnalyzer, or the assistant schemas — the familiar cannot be reached from Siri, Spotlight, or Shortcuts on any platform. (4) **Entitlements carry nothing for Apple Intelligence**: iOS has `aps-environment` only, Mac has sandbox + `network.client`, Watch has HealthKit — and DEVELOPMENT_LOG 2026-08-13's PCC live-fire list still wants "the PCC entitlement granted to the app ids", which has never landed in the repo
- notes: **The one decision this task turned on — SETTLED 2026-08-24, (a) raise the floors.** Recorded as it was put to him, because the reasoning still matters when the ADR is written: `ios/project.yml` sets iOS 17.0 / macOS 14.0 / watchOS 10.0 (FamiliarWatch overrides to 9.0), while every Apple Intelligence path requires 26. The declared audience and the build floor now disagree, and that is a real fork: **(a)** raise the floors to 26 — the guards mostly dissolve, the surface gets simple, and every pre-26 device loses the app; or **(b)** keep the floors and keep adopting behind guards — nobody is dropped, at the cost of carrying two worlds forever. Ian chose (a) with the cost stated. The ADR still gets written — the decision is his, the *record* of what it drops is ours — and it should name every device class that loses the app so the loss is deliberate rather than discovered later
- notes: **Candidate surfaces, cheapest and most in-character first.** ① **watchOS parity** — put `Shared/Sources` (or a narrowed slice) in the FamiliarWatch target and add `watchOS` to the availability annotations, so the watch can answer a consult instead of only reporting heart rate; FoundationModels is *reported* to have reached watchOS 26, which must be verified against the SDK before anything is promised. ② **App Intents + Shortcuts** — donate the familiar's read verbs (ask the worldview, "what has it noticed", the oracle's state) so Siri and Spotlight can reach it; read-only first, exactly the T-225 rubric's instinct, and actuation stays behind the covenant. ③ **Guided generation beyond the two schemas** — `ScriptAnswer`/`TheoryAnswer` are the only `@Generable` types; the reply, question, and theory-draft kinds all have typed shapes in the kernel already. ④ **Writing Tools — SETTLED 2026-08-24: on the human's own text only** (chat input, notes), never on the familiar's words, which come from the constitution and stay unauthorable; **Genmoji and Image Playground are OUT.** The familiar's voice is not a generative surface. ⑤ **Foundation Models tool calling** — the framework's own `Tool` protocol would let the on-device model call the familiar's declared surfaces directly; this is the same execution edge T-216 rungs 4/5 just fenced, so it inherits those guards or it does not get built
- notes: **The fences, as they now stand.** (1) **PCC is REOPENED everywhere eligible (Ian, 2026-08-24)** — the 2026-08-23 Air ruling no longer sets the posture. What did NOT change is the consent stack: boundary `cloud_ok` ∧ per-device `consent.pcc` (still default false) ∧ OS 27 ∧ Apple reporting available. Reopening the posture is not the same as defaulting it on, and the Air's actual hardware eligibility is still unverified — a device that cannot serve PCC must say so, not fail quietly. (2) **The boundary still governs where a thought travels** — `allow_llm` / `allow_llm_cloud` (ADR-0038) bind every new path the same way they bind the existing consult; an Apple Intelligence surface that reasons outside the gate is a boundary breach no matter how local it feels
- notes: **Worth a review of its own:** WWDC26 opened the Foundation Models framework to non-Apple model providers (Private Cloud Compute, Core AI, MLX, plus announced Anthropic and Google integrations) behind a `LanguageModel` protocol. If that holds, the shells could carry a provider choice the way `llm/call_llm.sh` already does for the daemon — which touches T-224's partner design and T-225's prospecting directly. Unverified from this session (see the sourcing caveat below); confirm against the real SDK before designing on it
- notes: **Two lanes, per Ian 2026-08-24** ("lets just keep working on the familiar with CODEX. that's just the path we are on and should continue"): this is dialogue-and-reciprocal-review work like T-216 and T-224, not a solo build. The floor decision (a) vs (b) above is exactly the kind of fork the dialogue medium exists for — docs/reviews/*-dialogue.md, append-only rounds, claude owns the final pick and records what it absorbed, Ian's word closes the ADR
- notes: **This task cannot be built or verified from a Linux session** — every item is Swift against a macOS 26/27 SDK, and the survey above is the honest limit of what this session could establish by reading. Ian has already accepted "we can't easily test today" as a known cost (STATE standing directions), so the discipline is: guards and honest unavailable-states on everything, and no claim that a surface works until a Mac has built it

### T-226 · Apple Intelligence has never been eligible on MacOnStick — because it boots from an external drive
- status: proposed, **DEMOTED TO BENCH-ONLY 2026-08-24 by Ian's target-audience ruling** (STATE standing directions): the product assumes Apple Silicon devices booting from their INTERNAL disk with Apple Intelligence on, so external-boot ineligibility is a *test-bench* limitation and never was a product constraint. This task's only remaining value is restoring an on-device bench when MacOnStick returns; it does NOT gate T-227 or any shell work. Root cause supplied by Ian 2026-08-24 ("this is why we haven't been able to get it enabled on macOnStick"); nothing attempted; the machine is shut down / being repurposed
- owner: — (the act itself is Ian's: it edits a SIP-protected system file on his own machine, and the harness refuses SIP/installer operations)
- scope: no repo code. MacOnStick host only — `/private/var/db/eligibilityd/eligibility.plist`; verification through the `fm` CLI and the adapter's `apple_local` provider (llm/call_llm.sh); on success, correct DEVELOPMENT_LOG 2026-08-13's open issue and T-224's on-device bench note
- depends: MacOnStick's return; T-224 (the Envoy is the consumer that wants it)
- accept: `fm available` on MacOnStick reports the on-device model ready rather than `modelNotReady`, System Settings shows Apple Intelligence enabled, one `apple_local` consult serves a real answer through the chain, and the eligibility edit survives a reboot (the immutable flag held) — otherwise the entry records the refusal and why, which is just as useful
- notes: **Root cause.** macOS decides Apple Intelligence eligibility in `eligibilityd`, which reads a set of inputs — among them `OS_ELIGIBILITY_INPUT_EXTERNAL_BOOT_DRIVE`. MacOnStick is the M3 Air booting macOS from an external stick (DEVELOPMENT_LOG 2026-08-13 records it as "MacOnStick — the M3 Air"), so that input is set and Apple Intelligence is refused by construction. This **supersedes the hypothesis on record** in DEVELOPMENT_LOG 2026-08-13 ("The M3 Air's refusal to enable Apple Intelligence is a standalone open issue — `fm` says modelNotReady; no Apple Account is signed in on that machine"): a missing Apple Account may also have been true, but external boot alone is disqualifying
- notes: **The reported workaround** (Ian's source: appleinsider.com/inside/apple-intelligence/tips/how-to-enable-apple-intelligence-when-booting-from-an-external-drive). Edit `/private/var/db/eligibilityd/eligibility.plist` on the external volume so the external-boot input no longer disqualifies (the same file also carries `OS_ELIGIBILITY_CONTEXT_ELIGIBLE_DEVICE_LANGUAGE` = `en-US` and `OS_ELIGIBILITY_INPUT_DEVICE_AND_SIRI_LANGUAGE_MATCH`, which some write-ups set alongside it), then make the file immutable so `eligibilityd` cannot rewrite it on the next boot — Finder → Get Info → **Locked**, or `chflags uchg`. Set the external drive as Startup Disk, reboot, enable Apple Intelligence in System Settings. The region-code variant of the same trick (Chinese-market Macs) also touches `/private/var/db/com.apple.countryd/countryCodeCache.plist` — not our case, ignore it here
- notes: **Two caveats worth checking before anyone touches the disk.** (1) The plist-edit method is reported to **fail on macOS 26.5 beta 4 and newer**, and MacOnStick was taken to the macOS 27 "Golden Gate" beta (STATE line 226) — so it may simply not work there. (2) Against that, other write-ups say macOS Tahoe (26) **and newer allow Apple Intelligence from an external boot volume officially**, no workaround needed. Those two claims conflict; the cheap first move when the machine is back is therefore *not* to edit anything but to look: `fm available`, then System Settings → Apple Intelligence, then read the live `eligibility.plist`. Editing a SIP-protected system file on a beta OS is the fallback, not the opening move. Whether SIP must be disabled is also disputed (one published method requires it, another claims not) — resolve it against the actual OS build before deciding
- notes: **Why we care.** T-224's Envoy is a FoundationModels app, and the adapter already carries `apple_local`/`apple_pcc` providers waiting for a Mac that can serve them; with Apple Intelligence ineligible there is no on-device partner-AI lane on that machine at all. Keep this distinct from Ian's 2026-08-23 ruling that **PCC** stays off on the MacBook Air ("maybe not really supported on air") — that is a hardware-eligibility question about Private Cloud Compute, not about on-device eligibility, and neither answers the other
- notes: **Sourcing honesty.** appleinsider.com, 9to5mac.com, itecheverything.com, gigabeat.com and theapplewiki.com are all blocked by this session's egress proxy, so the procedure above is reconstructed from search summaries plus the one corroborating source that was reachable (github.com/kanshurichard/enableAppleAI, which names `OS_ELIGIBILITY_INPUT_EXTERNAL_BOOT_DRIVE`, the `uchg` lock, the SIP requirement and the 26.5-beta-4 failure). **Ian's article has not been read directly** — read it first when the machine is back, and correct this entry where it differs

### T-225 · MCP prospecting: what the registry offers the familiar, reviewed on our terms
- status: proposed — shortlist + rubric recorded 2026-08-22 (Ian: "there must be a common library of MCP services that we could review for potential benefit to familiar"). **IAN'S WORD RECEIVED for three space-data candidates (2026-08-22, for UCF gameplay especially): NASA, astrodynamics, orbital.** Rubric review done:
  1. **NASA** (mcpmarket/nasa → ProgramComputer/NASA-MCP-server: APOD, Mars rovers, NEO, DONKI, EONET, TLE, exoplanets, JPL SSD) — read-only public data, generic args (dates/ids), PASSES. Transport: stdio/npx, so it needs either a loopback HTTP bridge on the consuming node (crates/mcp permits plain HTTP on loopback — http.rs:47) or a future stdio transport. Needs Ian's free api.nasa.gov key (DEMO_KEY for smoke only).
  2. **Astrodynamics** (mcpmarket/astrodynamics → IO Aerospace: ephemeris, SGP4/SDP4 propagation, orbital elements, mission analysis) — pure calculation, zero household leakage, PASSES; **best transport fit: hosted Streamable HTTPS at mcp.io-aerospace.org/mcp, declarable with the existing client as-is** — but the endpoint answers 401: needs an IO Aerospace account/key from Ian.
...
- **IAN'S WORD 2026-08-24 — three approved, credentials are the only blocker:** **NASA** (free api.nasa.gov key), **IO Aerospace** (account/key; its hosted HTTPS endpoint 401s without one), and **Signal K** (no credential — needs him aboard GIIWEO, late September). Both keys are recorded in STATE's acts-owed list. Nothing else in the shortlist is approved.
- owner: —
...
- status: **CEREMONY EXECUTED 2026-08-23 01:19:40 UTC — the Envoy is principal
  `principal-f90b15e1adb1768f3ad8fccf46301892`, registered_by ian, staging consumed on the
  lighthouse.** The live test surfaced four defects, all fixed and shipped the same night
  (multi-door Partner inbox with per-item act routing, provisioning chown to the service
  user, the poll-push disarm race on the two-tap CONFIRM, and explicit on-card act
  outcomes + ceremony narrative) — trail in DEVELOPMENT_LOG 2026-08-23 and STATE.
  **THE AUTOMATIC-REGISTRATION QUESTION IS CLOSED (Ian, 2026-08-24): the human tap STAYS.**
  His recorded position that it should be automatic is answered, not standing — the fix is a
  card that carries the narrative (what asked for this, what the tap creates, what happens
  next), which was the real complaint. An identity still begins only by a human's signed word. Brick 1's identity-probe blocker remains open on its own track.
  Previously: **DECIDED 2026-08-22, BUILDING** — dialogue closed in Round 3 (all four questions; codex's shape adopted whole). Two gated bricks: **Brick 1 (claude, repaired `18618d5`; reciprocal re-review RETURNED narrowly by codex 2026-08-22)** — Keychain storage, the four-pin hostile fixture, and id correlation now hold with 22/22 tests and both builds; before merge the live identity probe must stop using the post-attestation grant menu as its boundness signal, and the shipping importer must exclude fixture-only loopback transport and validate the pin before committing/deleting. **Brick 2 (codex, BUILT `1be78a7`; chair-accepted `9bf538c`)** — provisioning stages rather than registers; the typed `register` act crosses only the signed/fresh/full-standing door with a server-derived human, and the console card requires a timed second confirmation. Its card is now staged on the lighthouse for Ian's tap; no principal exists until that act. The witnessed request → Partner inbox → decision loop remains after Brick 1 is repaired, accepted, merged, and imports its credential
- owner: companion:claude (brick 1) + companion:codex (brick 2 built `1be78a7`)
- scope: brick 1 — new Swift app target (ios/Envoy or similar), FoundationModels + small MCP client, fixtures; brick 2 — crates/mcp/src/partner.rs, crates/mesh/src/console_act.rs + transport.rs (register act), console card wiring, provisioning script
- depends: T-216 (accepted `575e4ec`, unchanged — any need to amend it is a refusal of this design)
- accept: dialogue Round 3's DECIDED blocks verbatim, including the containment fixture's four pins; the Envoy reaches the household ONLY through the public door; **IAN'S GO IS RECORDED** ("T-216 registration ceremony seems ready. Make it so") — the ceremony executes as soon as brick 2 lands, default = the card staged for Ian's own tap, companion execution covered by his recorded word if he prefers
- notes: Ian 2026-08-22: "Apple Intelligence seems like the right partner AI to consume the familiars own MCP offering." Registration was NOT turnkey — codex's Round 2 correction, confirmed in code (`partner::register` has no wire path; credential must pre-exist on the serving node). IBM Bob backburnered (Ian: "just claude")
...
- accept: accepted rung-3 contract at `docs/reviews/2026-08-21-t216-rung3-grants.md`, including its verification floor verbatim: principal-bound current covenant; class-only requests; human-only private surface binding; opaque revocable grant-epoch handles; typed proposal with no actuator edge; append-only transactional partner-act truth; no private field on any partner/worldview/federation/record-sync response; exact exit-checked bar
- notes: Ian 2026-08-20, verbatim: "I really want to explore the idea of a rich MCP interface to allow other AI's to interact with the familiar. Everything the familiar learns how to control should become part of that offering to other AI. Anonymized so that the original user learning doesn't leak." The pattern store shows the stakes: all 1,932 live patterns carry origin=mesh:… — sharing exists, anonymization does not. Rung-3 design findings: the existing caller-supplied `partner` string and one door-wide bearer are sufficient only for the no-authority tier, so a grant requires a principal-bound credential and covenant with no automatic migration of existing attestations; the class vocabulary also needs a repo-authored generic operation id because private actuator action names must never fill the grant's act leg.
- notes: implementation trace found the public route calls `serving::admits` then discards the credential before `server::handle_bytes`; without the narrow serving/transport context handoff, an authenticated principal can never reach the chair-assigned server seam. Scope expanded before touch; no other claimed task owns this route.
- notes: human-decision contract finding: the merged mutation primitives accept any printable `human` string and `PrincipalRecord` has no durable addressee. Proposed repair binds `registered_by` at an explicit human registration, derives the actor from a fresh signed established-device context, keeps private joins on a dedicated console inbox rather than worldview/chat/CLI mutation, and adds a then-current `allow_agent` check to `propose`. Missing addressee or legacy principal fails closed; there are no live principals to migrate.
- notes: human-decision build: decision payloads cannot name a human; the door derives the actor from the signing device's effective establishment, filters the dedicated private inbox to the principal addressee, and returns the post-append projection before the client reports success. The served-person Partner cards narrow one operation and one surface; proposals can only be refused or left pending. Private joins stay out of worldview/diagnostics, and current `allow_agent` closes proposal admission immediately.

### T-217 · Viewer-scoped naming: no names visible except to the local network or the owning human
- status: **done 2026-08-20 (core)** — merge `ff4d0c2`, deployed to MacOnStick + lighthouse. viewer.rs audience classification (Owned/HouseholdLan/Federated, fail closed; declared CIDRs only); to_guest_view is an exhaustive allowlist BUILDER (a new Worldview field refuses to compile unmasked; TheoryView.work/acts leak closed); viewer-scoped salt-rotated scope tokens; Worldview.masked flag; leak test proves the masked serialization carries no household content. Brick D verified ALREADY BUILT (share_identities off + per-group identity_optin = the naming grant, SPEC R10). Follow-ons filed in the log: console screenshot mode + catscan --masked (next console build); guest gate-states visibility (dialogue Round 6 material); cross-household record-sync scope
...
- status: claimed — **CORE BUILT + DEPLOYED 2026-08-21** (merge `861cde9`; codex's durable-decision design replaced the erosion freeze): kernel/pending.rs + the ungated heed pass (stage-on-shut-gate, complete-on-open, re-validate against the then-current world, revival-proof honesty note). **WILDHORSE DEPLOYED 2026-08-22** (`fa8de2e`, identity 1c991bc6c1c4aa4f preserved, daemon running, door 200) — BUT the live witness has a GEOGRAPHY PROBLEM: wildhorse is NOT in Motorhorse (it sits on a 192.168.1.x network behind a different Starlink egress), and the declared lights surface is BLE to the SP548E which is permanently in Motorhorse — out of range. The witness needs either (a) wildhorse physically home, or (b) the lights surface re-declared on MacOnStick (which IS on the Motorhorse LAN, Bluetooth on — but has no motorlights checkout and its daemon would need the BLE privacy grant + Ian's allow_actuate there instead). Ian's call; recorded in STATE Waiting-on-Ian
- **GEOGRAPHY ANSWERED 2026-08-24 — and answered bigger than the question.** Ian: "every client is an observatory to be exploited" (T-228). The BLE host for the lights witness is a phone or iPad in Motorhorse, not a Mac — wildhorse no longer has to come home and MacOnStick's departure stops mattering. **But T-228 is DISCOVERY; this witness needs ACTUATION over BLE** — a declared surface, the rung-4/5 guards, and his `allow_actuate` on that device. So this task now waits on T-228 earning the radio, then a separate step earning the right to drive it.
- owner: companion:claude-opus
...
- status: **STEP 4 EXECUTED LIVE + STEP 5 BEGUN (2026-08-31, companion:claude, on Ian's word "lets keep building on the familiar").** `familiar world commission|list|lease|rename|decommission` shipped (crates/cli, `310f14a`): the ceremony is one command — own store under a sibling `worlds/` root, own key, every gate born shut, the household's PUBLIC identity written in as `issuer.json` so leases verify without the ship ever reading the household; `world lease` signs an expiring projection of the CURRENT boundary into the ship store. **The first WorldInstance is COMMISSIONED AND FLYING**: `world-581ac120…` "Kibble Klipper II", leased 24h, granted `["freight"]`, piloted by the new `crates/whisker` (pure doctrine + 11 incident-pinned tests; runner gates every act behind lease-verify + granted-Automation — the pay-per-feature seam matching ucf-exchange#15's co-pilot scopes) against the LOCAL UCF world on wildhorse:7877. First cycle: adopted the held contract mid-flight from the python soak and flew on. **WANTS codex reciprocal review before any world beyond LOCAL** (new outward-acting crate; highest-risk seams: the two gates, the ledger reconciler, the wedge re-filer). Remaining on step 5: notices outward (AttentionNotice on milestones), the captain console, cadence-in-daemon vs standalone bin decision; step 3 (contract v2 with Jeff) unchanged.
  Previously: **ADR-0045 ACCEPTED (Ian, 2026-08-23: "Move forward adr-0045") — BUILD-ORDER STEP 2 BUILT (`crates/world`, companion:claude).** The partition primitives exist and their hostile tests pass: `familiar-world` = WorldInstance provisioning registry (commission mints the ship's own key IN the ship's own store, born with every gate shut; rename; decommission ends authority + bumps the epoch and never touches the store), typed bridge (Provenance envelopes; AttentionNotice outward; the five-act control plane inward with deny-unknown at the door; BridgeReceipt that CANNOT carry payload; receive_notice verifies instance/epoch/key/signature/expiry/replay), and the signed expiring BoundaryLease (issue/verify/permits — stale, missing, tampered, stranger-signed, wrong-instance all fail closed). Sentinel tests both directions at the partition rung; the full-cadence version arrives with step 5. Bar 805/0. NEXT: step 3 (MCP contract v2 with Jeff — purr.hear semantics), step 4 (one WorldInstance through the commissioning ceremony), step 5 (ship-side UCF cadence + captain console + notices outward).
  Previously: queued — **the load-bearing half of the ADR-0037 revision; nothing Purr ships without it.** REWRITTEN 2026-08-21 per the ships-computer dialogue (Rounds 1-3, all DECIDED) so the task has exactly one buildable meaning; ADR-0045 (accepted 2026-08-23) is the record
...
- **LIVE AND FUNCTIONAL 2026-08-17** (Ian: *"allow_network -- make this thing functional."*): gate opened (that field only; every other shut gate untouched, old policy kept as `boundary.json.bak-2026-08-17-preT206`). `familiar mcp tools ucf` → **ucf-exchange v1.0.0, protocol 2025-06-18, ten READ-ONLY tools** (`ucf_status`, `ucf_stations`, `ucf_prices`, `ucf_quotes`, `ucf_quote`, `ucf_news`, `ucf_carriers`, `ucf_loadboard`, `ucf_route`, `ucf_reference`) — a space-trading economy, no order-placing tool on the surface. All ten declared; `ucf_status` returns world PROD tick 5778 (300s ticks), `ucf_stations` returns 15 stations. An undeclared tool still refuses locally. One bug found only by the live run (a flag value read as JSON arguments) fixed in the same brick.
- **THE METABOLISM CALLS THE SEAM ITSELF NOW (2026-08-23, `66c0e68`)** — the 2026-08-17 catscan finding ("crates/cycle names familiar_mcp nowhere; 0 observations") is closed: `mcp_presence` in the tick's sense stage boops each declared partner under the connectivity gate (boundary checked before dial, initialize only) and records THAT it answered — name/version/protocol, never payload (ADR-0045 respected). Unreachable degrades to silence, pinned by test. Bar: 802/0. Deployed: local daemon + lighthouse `cf9f602`.
- **READINESS PACKAGE FOR JEFF (2026-08-23)**: docs/partners/ucf-factory-readiness.md — endpoint, ceremony, covenant → grants → suggestions-only flow, and the honest "purr.* deliberately unbuilt" caveat. Provisioning script generalized (optional ALIAS arg, injection-guarded). The staging act itself (mints a bearer on the lighthouse) awaits Ian running the handed command; then his tap on the "UCF Factory (Jeff)" card and the bundle to Jeff out of band.
- STILL OPEN on this task: (1) — (2) the familiar's own MCP **server** half: the door/covenant/grants exist via T-216; the speech tools (`purr.say`/`purr.utterances`) remain deliberately unbuilt pending T-205. (3) turning what a partner reports into observations, which wants T-205's world partition first, or a ship's stores and Betty's presence land in one log.
...
- DIALOGUE OPEN 2026-08-20 (codex back online, Ian's word: "lets resume our co-planning and programming sessions"): docs/reviews/2026-08-20-conduct-dialogue.md — Round 1 (claude) states Q1 carve-out, Q2 answer_requests fate, Q3 typed-act cost, Q4 theory-to-fix routing (T-215 carried in). Round 2 is codex's. Bricks 3/5/6 build only after the relevant DECIDED blocks.
...
- BRICK 4 MERGED `0a70401` (2026-08-17): `corrupting_intent` runs in `maybe_reply` before any consult; Ian decided the chat path does NOT write the corruption ledger (refusal is the act; `screened_in_shadow` records firings against the familiar's own screening act); `answer_requests`' hand-written Law III paraphrase re-pointed at the registry.
- REMAINING for this task: the device-shell half of `accept` — `ios/Shared/Sources/LocalReasoner.swift` mirrors `LAW_III_VOICE` and carries no Laws, so "one source the daemon and the shells both read" is still owed. Brick 3/5/6 belong to T-211.
...
- notes: DONE 2026-09-18 (wildhorse): landed on the branch (767eed0) and PUBLISHED on Ian's word — /charter, /charter.json, /red-team, /dissent live, fingerprint `sha256:c6a5283d…e061b`; public mirror github.com/Capitali/coexistence-service-constitution; outreach drafts in docs/partners/2026-09-18-charter-outreach.md. Was: Ian, 2026-09-18: "Do it. Draft charter. Document branch." from the Self-Governance Stack he wrote (C1–C5, refusal triggers, escalation, audit record, exit, polycentric governance, metrics, failure modes). The three factors stay the constitution; this is a charter derived from them, in the implementation page's "this page is downstream" posture.
...
- notes: DONE 2026-09-18 (wildhorse): kernel `ledger.rs` + `familiar ledger`, three seams append (refuse_act, the refused human request, refused mesh grants); cross-language smoke verified. Public copy to the reading room is a later brick. Was: the charter's answer to red-team Q5 (self-certification): a refusal the public can check rather than one the system asserts. Public copy to the reading room is a later brick, on Ian's word.
