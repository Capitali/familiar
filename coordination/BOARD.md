...
- **CODEX ROUND 3 RETURNED 2026-09-16 (`docs/reviews/2026-09-16-t236-brick1-codex-reverification-r3.md`): REJECT — 1, 3, 5, 6, 7, 9 HELD; 2, 4, 8 PARTIAL.** Three blocker paths remain: (2, host) pairing treats a present-but-broken existing computer as absent and overwrites it; ship-local migration copies persona + trail without the persona lock its comment promises — `crates/cli/src/fleet.rs:480-558, 563-580, 1340-1385`; (4, host) `ensure_captain_id` returns early for any non-empty id before inspecting siblings, so a captain on two ids is not refused and `fleet adopt-ids` exits 0 leaving two stores — `fleet.rs:616-690, 2347-2394`; (8, Swift) the ask gate is checked only before the await — an old world's answer appends and speaks after a switch; overlapping `open` calls have no generation token — `BridgeModel.swift:107-163, 186-197`. **Host findings 2 + 4 are wildhorse's (fleet.rs); the Swift finding 8 is CLAIMED companion:claude (MacOnStick) 2026-09-16.**
