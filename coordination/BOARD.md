...
- status: **DECIDED 2026-08-22, BUILDING** — dialogue closed in Round 3 (all four questions; codex's shape adopted whole). Two gated bricks: **Brick 1 (claude, built `b557cb1`; reciprocal review RETURNED by codex 2026-08-22 with two contract blockers)** — process/dependency isolation, the public-door client, SPKI algorithm, fixed tools, availability posture, 11/11 tests, and both builds reproduce; before merge it still needs a real Keychain/import/current-bound-state seam and the hostile-door fixture must pin all four Round-3 containment claims. **Brick 2 (codex, BUILT `1be78a7`; chair-accepted `9bf538c`)** — provisioning stages rather than registers; the typed `register` act crosses only the signed/fresh/full-standing door with a server-derived human, and the console card requires a timed second confirmation. Its card is now staged on the lighthouse for Ian's tap; no principal exists until that act. The witnessed request → Partner inbox → decision loop remains after Brick 1 is repaired, accepted, merged, and imports its credential
