...
- status: **RUNG 3 DONE 2026-08-21; HUMAN DECISION CONTRACT CLAIMED** — completion merge `1afa3f4` (with the offering prerequisite at `40ba439`). Stable authenticated principals, principal-bound current covenants, class-only grant requests, named-human private surface binding, opaque revocable grant-epoch handles, and typed proposals now meet the accepted rung-3 contract. No credential was issued, no grant or gate changed, and nothing was deployed. companion:codex now holds the chair-requested docs-only contract for the human grant-decision surface; rungs 4/5 observe/invoke remain closed.
- owner: companion:claude-opus (rung 2 + offering vocabulary) + companion:codex (rung 3 build)
- scope: current codex claim — append-only contract in docs/reviews/2026-08-21-t216-rung3-grants.md only; no CLI/console code. Completed scope: claude — crates/mcp/src/offering.rs; codex — crates/mcp/{Cargo.toml,src/{partner,grant,partner_act,server,covenant,serving,lib}.rs}, Cargo.lock, the public `/mcp` authentication/context handoff only in crates/mesh/src/transport.rs, and crate-local tests
