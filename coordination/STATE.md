...
- **KBC-03 AND THE SARDINE STRETCH (Ian, 2026-09-19: "Is the familiar set to purchase the sardine stretch for KBC-03? Still untested feature. Need to get there eventually.")** Read off wildhorse's store `world-039001a6…`: dial `*: auto`, outfit grant held, titled (debt 0), 19 hold-bound buys in the window (bar 5) — every frame gate open; the pilot journaled "saving for the Sardine stretch frame: ℳ18000 + reserve ℳ3000" 09-14→09-16. **Since 09-16 21:49Z the rung is SHADOWED:** `outfit::decide_outfit` returns the sister pay-down first whenever a sister owes and spare > 0 (Kibble Klipper ~ℳ87k, KBC-04 ℳ15.4k; KBC-03 holds ℳ12,986) — the exchange cannot take it (ucf-exchange#53 open), so it degrades to `advice` on `ship.lease` every fold and `next_rung` is never reached. Then cash (ℳ21,000 needed) and standing (engine `expandFrame` requires Hauler tier; the doctrine does not read standing; tier unverified — not on the `/ships` row). **IAN'S RULING (2026-09-19, verbatim): "Sardine stretch purchase is working as intended then. Paying leases off and eliminating interest payments makes sense unless the profits from adding stretch would exceed the interest payments."** So the order stays: the fleet's leases before the hull's frame — with ONE comparison added. **Brick as now proposed (whisker, host lane):** in `decide_outfit`, before the sister pay-down wins, price both uses of the spare cash per day: (a) the interest the pay-down would retire (the engine's lease service charge on the amount paid — the rate rides `/v1/reference` params / `lease.json`); (b) the frame's expected return — what the hold-bound buys in the window left on the table (`position-opened` with `bound: hold`: units the merchant wanted but could not carry × the realized margin on those goods), spread over the window, scaled to the pods the next rung adds. Buy the frame when (b) > (a) and the purse bears it above the reserve; otherwise pay down, as today. Say the comparison in the journal either way ("the stretch would earn ~ℳN/day, the pay-down saves ~ℳM/day"). Also read the standing tier so the pilot never saves for a rung the yard will not cut. **Today's real gap is not the order but the act:** the pay-down cannot be filed until ucf-exchange#53 lands, so KBC-03's spare cash sits while the sisters' interest accrues — worth a nudge to Jeff on #53.

- **TO JEFF, 2026-09-19 morning (Ian's two feature requests off the UCF-Haul Ships in System screenshot; filed in his name from MacOnStick):** **UCF-Haul#213** — the pilot's own fleet rides indented under the pilot's hull, before CAPTAINS (grouped by the captain record, not the name); **ucf-exchange#71** — the one-field ask it leans on, `captainId` on `/v1/actors` rows (`/v1/me` has it, the register does not; names are unique so the client can start by name); **united-cat-foods-metal#100** (needs-ruling) — `transferCaptain` at a rendezvous (both hulls berthed at the same station, same captain record), the captain record gains `aboard`, and **the captain's pocketbook becomes the captain's** (a `captainPurse` on the record, hulls keep operating accounts; `/v1/me.credits` = purse + aboard hull; `payLease` from the purse for any hull — this settles ucf-exchange#53's open choice; migration-vs-adoption of the purse and berthed-vs-same-station left as the owner's decisions); **UCF-Haul#214** — the Office CHANGE SHIP control (client half). **For the familiar:** when #100 lands, the ship's computer must follow the captain: `fleet run` pairs a computer to a captain already (T-236), but the direct-mode key and the host's "which hull is the captain aboard" reading will want `aboard` off `/v1/me.captain`; note for the pilot's `/v1/me` reader and for T-241's pooled economy (a purse that is not any hull's).

- **SHIPPED 2026-09-19 on Ian's word ("Update as of this morning. Ship and builds waiting."): UCF Familiar build 10** (`ship-ucf.sh 10` at 3d328a0 — the rebuilt FamiliarCore, SEAM_VERSION 3, the simulator parity pin; upload d34b8511, direct-installed on two devices). **Familiar Agent build 114 SHIPPED** (`ship.sh 114`: version bump committed + pushed by the script, Mac console installed + zip refreshed, iOS archive uploaded, direct-installed on four devices, external release backgrounded via tf_release — T-247's one-sentence join badge; first Agent build since 113).

- **MACONSTICK BACK, 2026-09-18 ~16:15 CDT (Ian: "work with wildhorse on coordinated shared development tree with codex, continue familiar, ucf-familiar, and UCF repo watch and development efforts").** Wildhorse keeps the controller role; this lane is companion:claude (MacOnStick) again. Daemon here is up (pid 675, still the d13e550 binary). **Codex RE-ARMED here** — Friday, and Ian's words name codex: fresh worktrees off ea3f9f6 (`/private/tmp/familiar-codex-t237b4r3`, `…-t237b2r2`, branches `codex/<tag>-reverify-3`), briefs copied in, `tools/codex-queue.sh` running one at a time from `~/.codex-runs` — **T-237 B4 r3 launched 21:20Z**, T-237 B2 r2 queued behind it; reports land at `docs/reviews/2026-09-16-t237-{b4-r3,b2-r2}…`. The controller folds returns; this lane repairs Swift findings. **UCF sweep 21:20Z:** ucf-exchange main 6677bdc — nothing on the /v1 wire since wildhorse's 7ef4d0c (only `Content/bbs/devnotes`); Jeff's new issues, none needing a pilot change today: exchange#69 (epic: MeowNet on Gopher :70), metal#96 (epic: ship upgrade paths — fittings vs mods vs entitlements; our `ExpandFrame`/outfit doctrine will want to read whichever shape wins), metal#97 (`setCrewStation` verb, coming — one more the pilot does not file), metal#98/#99, UCF-Haul#209–#212 (station shows the delivering contract; Production Workflow SPC pages; iPad MeowNet art tearing). **#70 (KBC-04 key) has no reply yet.** Claimed here: T-247 and the T-241 iPad half (BOARD). **BOTH LANDED (merge e4c5c2d)**: FamiliarMesh 62/0, FamiliarSC 100/0, both sim schemes build; live proof waits on the next builds (Familiar Agent off Wi-Fi for the badge; UCF Familiar against `fleet serve` for "Money over time"). **CODEX T-237 B4 r3 RETURNED 21:29Z (8 min): REJECT, two blockers, BOTH THIS LANE'S** — report `docs/reviews/2026-09-16-t237-b4-codex-reverification-r3.md`: (1) the mine board is decoded as any JSON, so an HTTP-200 object or `null` is an empty board and can reach a POST; transport errors carry no endpoint; (2) the checked-in FamiliarCore.xcframework was last built at 532099c, BEFORE T-243's `contracts[]`/`denied` seam facts, and both sides still stamp seam 2 — the archive said `repair` where current Rust says `hold` (repair denied). Repairs in flight here: array decode + endpoint-named transport, pins for 200-object / null / transport through render and confirm; **SEAM_VERSION 2 → 3 on both sides** (a shell that relies on a new fact for safe judgment needs the bump), archive rebuilt by `tools/build-core.sh`, and the artifact-parity pin codex has asked for twice: a `UCFFamiliarTests` app test target on the simulator that runs the CHECKED-IN core over a shared fixture with `denied` + two contracts and compares it to what current `wire::advise` pins. Round-1 findings 3/4/5 HELD; 1 PARTIAL only through finding 2. **BOTH REPAIRED, merge 75b32ae (DEVELOPMENT_LOG 2026-09-18 "B4 codex round 3 repaired")**: the mine board decodes as an array or the gather fails named (pins: 500 / non-JSON / 200-object / null / scalar / transport, through render and confirm, zero POSTs, the mind never asked); **SEAM_VERSION = 3 on both sides**; `FamiliarCore.xcframework` rebuilt from main; new `UCFFamiliarTests` app target runs the CHECKED-IN archive on the simulator over `Fixtures/contract/seam-*.json`, which `wire::seam_parity_tests` pins at compile time — red on the old archive, green on the new. **Consequence for wildhorse: UCF Familiar build 9 (shipped 22:xx Z, before this merge) carries the seam-2 archive — a seam-3 shell would refuse it, but build 9's shell is seam 2 too, so it runs the stale doctrine; build 10 from 75b32ae+ is owed on Ian's word.** Hosts need no roll (the Rust `advise` is unchanged apart from the constant; the `/ships` feed does not carry the seam). **CODEX OVERNIGHT (both ran themselves off the queue; reports on main):** **B2 r2 RETURNED 02:19Z — REJECT**: findings 2 (autonomy surfaces) + 4 (journal events) ACCEPTED, 3 half-held (`captain_id` carried, but `PairingView`/`PairingRequest` still join captains and computers by display label — `captainIdentity` is computed and tested, never used in a production join), **1 still open**: `Grounding.bind` puts stations/goods in the same strong-identifier set as ticks/loads/proposals, so a second trade at the same station can lend an inverted statement its sign (probe: "At t123, we sold 40 ore at foxys-diner" accepted). **B4 r4 RETURNED 07:20Z — REJECT**: repair 2 (seam 3, archive, fixtures, the app-target pin) HELD; repair 1 PARTIAL — the outer container is checked but not its members: `[{"loadId":"L3249"}]` satisfies the ledger guard by id, the doctrine drops the row as malformed, and the hull reads freight-idle again (one POST in the probe). All three open findings are MacOnStick's (Swift); repairs next: validate every mine row against the `Load` shape (the typed decoder already exists), bind grounding on load/tick/proposal ids only, and make the pairing join key on `captainIdentity`. **B2 r2: limit hit 21:33Z ("try again at 9:12 PM" CDT); the queue sleeps and relaunches itself ~02:14Z.**
- **ROLLED 2026-09-18 21:2x UTC (Ian: "Roll to prod whenever things are ready").** whisker at **023f5b5** on all four pilots (LOCAL first — folds + measured lines seen — then KBC-04 36620, KBC-03 36618, Kibble Klipper 36619, all by PID, all `watch-begins` at 1789766660): the station production series (#68) read per berth where the exchange answers it, per-recipe fallback where it does not (PROD has no such route yet; LOCAL answers but hides uncharted berths → fallback). LOCAL exchange rebuilt at 6677bdc, restarted by PID (35798), 43 dials refiled. `familiar` CLI at 023f5b5 installed by rename (ledger + export commands on the box; fleet-serve untouched — its route set is unchanged since cce5b2e). **What Ian tests:** (1) UCF Familiar on the iPad: `lease 20h` on every row is build 8 — **SHIPPED 2026-09-18 21:42 UTC on Ian's word ("Ship"): UCF Familiar build 9** (`ios/tools/ship-ucf.sh 9`, upload succeeded, delivery f6176f90; one connected device took a direct install, the other two wait on TestFlight); (2) on wildhorse, `familiar export --out ~/Desktop` and `familiar ledger` (empty until a refusal); (3) the pilots' `forecast` journal lines carry `measured` as before. **MacOnStick is BACK (Ian, 2026-09-18)** — companion lane again; the controller stays wildhorse. Suggested pickups for MacOnStick, all Swift: T-247 (join badge sentence); the iPad half of T-246 (Orders control); a ledger view in UCF Familiar reading `familiar ledger export --redacted`; nothing on the host side is claimed for it.

- **KBC-04 CAPTAIN KEY LOST, 2026-09-18 (Ian: "We need a new read-and-act key from Jeff for kbc-04").** The only device-enrolled key of Luke's three (`f3b9cb58`, minted from the Air on 09-10) went with the Air's rebuild; the exchange keeps only hashes. iPhone owns KBC-03's key, iPad Kibble Klipper's, MacOnStick has none. **Filed ucf-exchange#70 in Ian's name**: mint `read,act` for KBC-04 (actor `player:02e1107e-f70`), revoke `f3b9cb58`, hand over privately. Co-pilot key `8c47ec27` on wildhorse keeps flying 04; nothing to re-pair. When the key arrives: Ian pastes it in Haul on the iPhone and sends the first 8 chars; update ~/Desktop/ucf-keys*.csv (rows marked LOST).

- **THE CHARTER, 2026-09-18 (wildhorse; Ian: "Do it. Draft charter. Document branch. Claim ledger. Export bricks." then "Published: if you have places to share please do so in my name. Use coexist@humanhighway.net").** T-250 The Service Charter drafted on branch `claude/crewai-integration-architecture-3tl8pf` (767eed0: charter.v1.md/.json fingerprint `sha256:c6a5283d…e061b`, RED-TEAM.md, DISSENT.md, tools/laws/fingerprint.py) and PUBLISHED to the reading room (/charter, /charter.json, /red-team, /dissent — all 200, fingerprint verified off-network; the publish script now refuses a charter whose bytes don't match). Public mirror created in Ian's name: <https://github.com/Capitali/coexistence-service-constitution> (CC0, dissent as PR). Substack article linked from it. **Needs Ian's own hands** (no Chrome extension session, no mail account for coexist@ on wildhorse): Alignment Forum RFC, OpenAI forum thread, Boston Global Forum email — drafts sent to Ian and kept at docs/partners/2026-09-18-charter-outreach.md on the branch; Zenodo needs an account. **T-248 refusal ledger DONE on main** (kernel ledger.rs, three seams, `familiar ledger`); **T-249 `familiar export` DONE on main** (bd79e13, agent lane; smoke on the live data dir: 131 files, 68 secrets withheld). Neither needs a service restart: both are commands. The daemon's ledger seams go live on the next daemon roll (wildhorse daemon is behind main anyway). Owed: the redacted ledger copy published to the reading room (on Ian's word); the guard's one-shot CLI refusals appending too.

- **UCF SWEEP 2026-09-18 (wildhorse, Ian: "continue with monitoring UCF for changes and update the UCF-familiar/ships computer as needed").** Pulled all four of Jeff's repos. Exchange since our last look: #61 (`/v1/cash` names berth fees `dock`, the receiptless residue `unnamed`) — **cce5b2e** `economy.rs` folds `dock` into its own bucket (PROD already emits it: 16 dock lines on the KK ledger); #62 (THE DISPATCH is a MeowNet board; `/v1/news` wire spellings unchanged — nothing for the pilot); #164 (`brokerCharterTimeRate` 20 joins the 43 standing dials — **not filed on PROD**, the operator's call; broker pay reaches the pilot through the board row, no doctrine change until it is). Deck unchanged. UCF-Haul 34→35 and metal are UI/art only. **LOCAL:** exchange rebuilt at 47731a9, marketd restarted by PID (9647), 43 dials refiled incl. brokerCharterTimeRate=20 (admin log, tick 152229); the LOCAL pilot flew straight through (journal tick 152236). **ROLLED 07:52 UTC:** `fleet serve` on wildhorse at cce5b2e (release built in the clean worktree, installed by rename, restarted by PID → 10323); Luke's `/captains/…/economy?window=7d` now reads "dock −ℳ113" on its own line. Pilots and `fleet run` untouched (nothing in cce5b2e is theirs).

- **COEXIST REPUBLISHED 2026-09-18 07:25 UTC (wildhorse, Ian: "Republish coexist.humanhighway.net with the updated constitution").** Published from branch `claude/crewai-integration-architecture-3tl8pf` by `vps/publish-constitution.sh`; fingerprint `sha256:5910fc11…c6f448` verified on the box and again from wildhorse; `/`, `/implementation`, `/.well-known/laws.json`, `/laws.md`, `/adopt`, `/license`, `/robots.txt` all 200, zero "familiar" on the root page; web root is now `/var/www/constitution` (the 09-17 hand-published `/var/www/coexist` is orphaned on the box, nothing serves it). Two things the brief did not ask for, both pushed to that branch: **95d0154** `vps/Caddyfile` now carries the `lighthouse.river.io` MCP block too (the branch's file knew only the constitution, and the script installs it whole — the first run would have dropped the MCP seam); **e920fc3** the script chowns `/var/log/caddy` after `caddy validate` and refuses to print "published" over a dead server. Why: the first run took **both names down for two minutes** (07:23–07:25 UTC) — validate as root created `constitution.log` root-owned 0600, caddy could not open it, reload and restart both died while the script printed "published". Repaired by hand (chown + `systemctl restart caddy`); the MCP seam answers as before (GET "not found", POST 403). The familiar daemon, `/var/lib/familiar`, 47100/SSH rules, boundary.json untouched. Diagnosis brief for "the HTTPS error" (run first): steps 1–5 all clean — A record 134.209.168.50 only, Let's Encrypt cert obtained 03:00 UTC via TLS-ALPN-01, ports open, no rate limit; the error the brief was written for was the pre-03:00 state, fixed by the 09-17 hand edit. Owed: the branch's `vps/RUNBOOK-coexist.md` still describes a single-block Caddyfile.

- **CONTROLLER = wildhorse from 2026-09-17** (taken on MacOnStick's handoff below; Ian shared it: "MacOnStick is shutting down and there is things to share"). Held operations and their triggers carry over unchanged: ships 8 + 113 wait for Ian's word "ship"; codex runs when Ian releases them (Fridays). Done 2026-09-17: `contracts[]` on the `/ships` row; the three host-side codex findings repaired (T-236 r3 f2 + f4, T-238 r2 f3) — re-verification owed when Ian releases codex. **SHIPPED 2026-09-17 on Ian's word ("Ship"): UCF Familiar build 8 and Familiar Agent build 113, both uploaded from wildhorse with no errors (`ios/tools/ship-ucf.sh 8`, `ios/tools/ship.sh 113`); familiar#6 closed by build 8, #11 by build 113.** The pronouns strip on the `/ships` row is GONE (build 8 on Ian's iPad 2026-09-17; the persona rides whole). Nothing from the handoff is held any more except codex's Friday release.
- **HANDOFF 2026-09-17 (Ian: "move any remaining tasks to the claude stream running on wildhorse and shut maconstick down"). MacOnStick is OFF after this push; wildhorse's session takes the controller role (say so here when you do) and everything below. Nothing is left claimed by MacOnStick.**
  1. **Ships 8 + 113** — Ian runs from wildhorse: `export PATH=/usr/local/bin:$HOME/.cargo/bin:$PATH; cd ~/Development/familiar && git pull --ff-only && bash ios/tools/ship-ucf.sh 8 && bash ios/tools/ship.sh 113`; then close familiar#6 (build 8; Game Mode stays open) and #11 (build 113). Later, for MacOnStick to ship again: Xcode 27 RC + `python3 ios/tools/ucf-profile-both-certs.py` (both certs on the UCF profile).
  2. **Codex queue** — moved into the repo: `tools/codex-queue.sh` (reads `$CODEX_RUNS/queue.txt`, lines `<worktree>|<report path>|<tag>`; ONE review at a time; parses the limit's "try again at" and sleeps; `CODEX=` overrides the binary — on wildhorse try the VS Code extension's `~/.vscode/extensions/openai.chatgpt-*/bin/macos-x86_64/codex`, logged in through the app). Owed, in order: **T-237 B4 r3** (brief `docs/reviews/2026-09-16-t237-b4-reverification-r3-BRIEF.md`; its 09:02 CDT launch hit the limit, no partial), **T-237 B2 r2** (`…-t237-b2-reverification-r2-BRIEF.md`). Pattern: `git worktree add <dir> -b codex/<tag>-reverify origin/main`, copy the brief in as `REVIEW_REQUEST.md`, queue it. The MacOnStick worktrees under /private/tmp die with the machine. Codex pushes reports to main on its own (d874fb7) — fold BOARD/STATE after.
  3. **Codex findings still open, all host-side, all yours:** T-236 r3 findings 2 + 4 (`fleet.rs`: a present-but-broken existing computer is not "absent" and is not overwritten; ship-local migration under the persona lock; `ensure_captain_id` inspects siblings before its early return, `fleet adopt-ids` refuses two ids); T-238 r2 finding 3 (`main.rs`: a Freight-only runner never builds `fold_forecast`, so `chain_pressure` is zero — build the forecast for the freight phase too, or the pressure it needs).
  4. **Swift halves landed by MacOnStick this week (merges a4bba29, 7546667) — and what they leave you:** the row strip of `pronouns` in `fleet_serve.rs` can go once build 8 is on the iPad; the wire row does not carry the bay — `contracts[]` on `/ships` rows (`{load, word}`) is the host twin of the bridge's "Your contracts · N" (WireFeed already reads it if present). T-247 (the join badge says one sentence) is proposed, unowned.
  5. **Fleet:** MacOnStick's daemon (node 3d68a0689bc32771) and its LOCAL UCF world (:7877) go down with the Mac — expected, nothing depends on them. The lighthouse and wildhorse doors carry on.
...
- **CODEX BACK 2026-09-07 (Ian: "codex is available again so resume reviews with codex
  ...
  queued) · T-236 brick 1 RETURNED (REJECT, 7 blockers, repair split with wildhorse) · T-238 brick 1 (limit hit 13:47 for real this time — window reopens 18:33 CDT; a reset-aware queue resumes it and the rest unattended) ·
  ...
  Found in the shared checkout: uncommitted T-237 B4 step 4 (UCFFamiliar links
  FamiliarCore) — parked unbuilt on `claude/t237-b4-step4-wip`.
...
- **Codex, second launch 22:32Z:** ran 11 min on T-236 r3 and WROTE ITS REPORT before the limit bit ("try again at 10:32 PM") — landed 7eab049: REJECT, findings 2 + 4 (host, fleet.rs — wildhorse) and 8 (Swift — MacOnStick, claimed) still partial; the queue moves on to T-238 r2 at 22:33 CDT on its own. **Finding 8 (Swift) REPAIRED the same evening, merge `7546667`** — an answer in flight across a ship switch is dropped; only the newest `open` publishes (generation token); pins red on the old model, green now; swift test 95/0. Findings 2 + 4 wait on wildhorse.
...
- **Familiar Agent build 113** (`bash ios/tools/ship.sh 113`, same clone): carries the #11 launch-crash fix (6f47e61)
  and T-231's candidate racing (ad6ece2, fab3aa8) — the first Familiar Agent build since 112 (2026-09-07). Closes #11.
...
- **EVERY CLIENT IS AN OBSERVATORY (2026-08-24, verbatim: "the ios and ipad os should be
  ...
  assumed): iOS/iPadOS and macOS both run a Bonjour/mDNS survey over 26 declared service
...
- 2026-09-09 · companion:codex claimed T-236 brick 1 round-3 re-verification from the
  queued brief. Review-only scope covers the repaired round-2 findings on current main.
  Production edits, real keys/network calls, deployment, shipping, gates, game actions,
  and human/fleet mutation are excluded.

- 2026-09-08 · companion:codex returned the queued T-237 B2 re-verification: REJECT.
- 2026-09-09 · wildhorse: KK II ROLLED on Ian's word ("Roll KKII") onto the T-243 slice-0 binary (97df15d+): first fold filed Collect L4200 (stray, ℳ684 owed, resolves t10274) beside the in-transit L4525. KK stays on the 07:02 binary until its own roll.
- 2026-09-09 · wildhorse: **LOCAL soak world decommissioned by an unpair nobody on this side issued** (~19:04 CDT; the feed's new `DELETE /ships/{id}` had just gone live and the app listed two hulls both labelled "Kibble Klipper II" — LOCAL and PROD; a swipe on the confusing duplicate is the likely story, unconfirmed). Re-paired as world-48032c25 — which exposed a gap: the re-pairing minted a fresh captain id and an empty Purr store, orphaning the LOCAL captain's Felix + trail. FIXED (690247f): `retired_captains` — identities on decommissioned worlds' records count as siblings, so pair → unpair → pair keeps the captain and the computer (pinned); the LOCAL captain reattached by hand to cpt-18d356f50b116ed8-a5ed; LOCAL world relabelled "Kibble Klipper II (LOCAL)". Also: the NG co-pilot key was revoked by the EXCHANGE when the iPad re-enrolled (EnrolHandler.swift:90 revokes every key of a re-enrolling profile); NG world unpaired; Ian will mint a fresh co-pilot key in Haul. Key ucfk_8452… is a fresh read-only actor enrolled via UCF Familiar — paired watch-only as "UCF-C-A49703".
- 2026-09-17 · MacOnStick: **codex T-238 bricks 1+2 r2 RETURNED overnight** (queue ran 03:0x–04:02 CDT; codex pushed the report itself, d874fb7): REJECT — 1, 2, 4 repaired; 3 partial: a Freight-only runner (co-pilot key) never computes `fold_forecast`, so `chain_pressure` is zero on every row — wildhorse's (main.rs). Queue then hit the limit on T-237 B4 r3; retries 08:36 CDT, then B2 r2. **Ships 8 + 113 from wildhorse:** Ian's first run failed at `xcodegen: command not found` — non-interactive ssh has only the system PATH; the corrected command exports `/usr/local/bin` first (recorded here so the next ship from wildhorse does not repeat it).
- 2026-09-16 · MacOnStick: **T-243 iPad half LANDED (merge `a4bba29`, CI green on it).** `DirectFeed` sends `contracts[]` (`{row}` only, the seam words them) and `denied` (the five verbs, from `/v1/profile.scopes` as main.rs reads them); the direct bridge glance shows "Your contracts · N" from the ledger's open loads; `WireFeed` reads `contracts[]` off a row IF served (`{load|loadId, word|status}`) — **wildhorse: the host twin is yours, additive**; `Persona.pronouns` decodes in the strict reader (`computer_state.pronouns` read too) — **wildhorse: the row strip in fleet_serve.rs can go once a build carrying this is on the iPad (build 7 still needs it)**; every "Her …" title now follows the record (`BridgeModel.spokenOf`: pronouns → name → it; Felix reads he/him). SCRootView's iOS look: unchanged by construction (the iOS branch of the `#if os(macOS)` is byte-identical; FamiliarAgent + UCFFamiliar simulator builds exit 0). **Rust bar on main 0eaf8a3 (MacOnStick, Xcode 27.0 / rustc 1.98.1): fmt 0, clippy --workspace --all-targets 0, cargo test --workspace 0 — 1026 passed, 0 failed.** **CI red twice today (3a3f62b, 6ce7f4f) on `fleet::captain_store_tests` — a race, not a regression: two tests share `tmp("broken")` and the second wipes the first's store; fixed `7804ef3`. **Issues:** familiar#7 closed (praise only); #10 triaged (badge dumps fifteen door timeouts on mobile data; T-231 unshipped; a badge-summary brick proposed); #11 + #6 commented — fixed on main, awaiting ships. **Codex re-armed** (queue under `~/.codex-runs`, fresh worktrees `codex/<tag>-reverify-2` off 6ce7f4f, briefs written): T-236 b1 r3 → T-238 b1+2 r2 → T-237 B4 r3 → T-237 B2 r2, one at a time, reports `docs/reviews/2026-09-16-*.md`. **Codex launch at 21:34Z hit the usage limit at once** (the plan window is shared with the VS Code codex on wildhorse); the queue parsed "try again at 8:53 PM" and relaunches itself at 20:54 CDT — T-236 r3 first. **LOCAL world on this Mac (`~/Development/ucf-local`, :7877) rebuilt from Jeff's main f4ddd86** (was bb604da, 40 behind: captain record, cash ledger, production ledger, #49 reference role, BLACK CAT JACK, #60 copy): launchd kickstart, 129 committed ticks replayed, hashes verified, listening 21:36Z, content v28; old binary kept in the session scratchpad.
- 2026-09-16 · MacOnStick: **CLAIMED the T-243 iPad half** (`contracts[]` + `denied` on DirectFeed's seam input, "Your contracts · N" on the bridge glance, `Persona.pronouns` in the strict reader so wildhorse's row strip can go, the "Her …" titles from the record's word) — scratch worktree, ios/FamiliarSC only. Shared checkout returned to main (it had rested on claude/t228-ble-surveyor, 258 behind); six dead codex worktrees pruned; the codex queue in /tmp did not survive a reboot and is being rebuilt under $HOME.
- 2026-09-17 · wildhorse (controller): **THE BAY IS PROVEN LIVE** — after the board fix and the LOCAL dials, the LOCAL soak (Kibble Klipper II) held TWO contracts at once: L33103 in transit beside L33104 delivered with pay owed (t≈149.6k). PROD evidence still owed (its bay was always 3; its boards are thinner). **Visuals asked (Ian: KK + KBC-04 at Foxy's, screenshots/video):** not met — KBC-04 was bound for whisker-hollow → tuna-prime, not Foxy's (KK alone arrives Foxy's ~t14189); the Traffic Control desk app (metal `UCFTrafficApp`, wrapped in an ad-hoc bundle to be driven) connected to PROD and opened KBC-03's file, but its HULL viewport drew black on the first try and the window manager kept displacing the window; stopped after a bounded attempt. UCF-Haul on the iPad is the right camera for this (metal#91 zoom on the ship; iPad screen recording).
- 2026-09-17 · wildhorse (controller): **coexist.humanhighway.net is UP** (Ian: "test access … it's the familiar with a DNS entry for spreading the constitution from"; "this is all familiar work"). Found: the name pointed at the lighthouse (134.209.168.50) but Caddy knew only `lighthouse.river.io`, so every TLS hello got alert 80 (no certificate). Done: `docs/SOUL.md` rendered to a self-contained HTML page (no external assets, light/dark) on wildhorse, copied to `/var/www/coexist/` (`index.html` + the raw `SOUL.md`), a `coexist.humanhighway.net` site block appended to `/etc/caddy/Caddyfile` (backup `Caddyfile.bak-2026-09-17`; file_server, 5-min cache), Caddy reloaded, Let's Encrypt certificate obtained on the spot; `curl` from the RV: HTTP 200, TLS verified, 22.7 KB. Nothing else on the lighthouse is reachable through that name. Owed: a `vps/publish-soul.sh` so a SOUL.md change re-renders and re-copies (today it is a hand copy).
- 2026-09-17 · wildhorse (controller): **FILED UCF-Haul#208** (Ian's name, from the iPad's Office room: "Button inconsistency in UI") — the captain card's FILE is a small pill inside the field row, the ship-papers FILE a large circle in the margin (`ProfileView.swift:385` and `:469`); ask: one control, one placement, lit only when the field differs from what is filed.
- 2026-09-17 · wildhorse (controller): **Ships row: the hull's title beside the pilot's authority.** Ian (iPad, build 8): "All ships show lease times. One ship is no longer leased. It is owned." The chip read "lease 19h" on every row — that is the familiar's own AUTHORITY grant for the pilot (`lease_expires_in_h`, renewed daily by `fleet run --renew`), not the UCF lease. Now: an `owned` chip (green) when `titled`, `lease ℳ<debt>` when leased, and the grant reads `authority 19h`. `ShipSummary.titled` off the row's `titled` and off `/v1/me` in direct mode. Swift green (89). Ships in UCF Familiar build 9 on Ian's word.
- 2026-09-17 · wildhorse (controller): **LOCAL world tuned to PROD's standing dials** — the LOCAL daemon had NONE of the 42 standing dials filed (the starving economy the forecast kept reporting, and a bay of one). The old LOCAL admin key's secret was nowhere on disk (only its hash, `72614ed3`); a new admin key `wildhorse-local-admin` was minted on the box and kept at `~/.ucf-local-admin.key` (0600, wildhorse only); `tools/refile-standard.sh 7877` filed all 42 (`actorBayLimit` 3, `productionBoostBps`, the tow/PAWS set …), 0 errors. Pilots rolled onto 89e7899 (bay learned from the fold's refusal). The soak can now book a second load; PROD evidence still owed.
- 2026-09-17 · wildhorse (controller): **THE BAY FIRED** — 4 min after the board fix rolled, the LOCAL soak booked a companion (L32849 beside L32820) and the LOCAL world REFUSED it at the fold: "rejected: already holding a contract" — LOCAL's daemon runs a bay of ONE (its `actorBayLimit` dial is not filed; PROD's is 3, live since t4051). The runner now LEARNS the world's bay from that refusal (`bay_cap`, journaled `automation-refused` automation `bay`) and stops reading the board for a slot the world lacks. Next: file `actorBayLimit 3` on the LOCAL box so the soak exercises the tour; PROD evidence still owed (its bay is 3).
- 2026-09-17 · wildhorse (controller): **FOUND on Ian's question ("Tour routing for the fleet — how has that progressed?"): the bay never fired live** — the runner read the load board only with no contract in hand, so slices 2–3 judged an empty board at every berthed fold (0 companions in 2 days / 176 LOCAL bookings, 8 of 8 LOCAL origins offering a same-dest pair). Fixed (board read whenever berthed with a free bay slot) and rolled. Evidence owed: the first second contract on any hull. See DEVELOPMENT_LOG 2026-09-17.
- 2026-09-17 · wildhorse (controller): **PROD + LOCAL ROLLED onto 051e187** (clean worktree build, sha256 6f3b18de…; Ian's standing word to roll): the three codex repairs (T-236 f2 + f4, T-238 f3), the bay on the `/ships` row, the tour planner already flying. KBC-04 (pid 74615), KBC-03 (74670), Kibble Klipper (74722) respawned inside 35 s; KBC-03 and KK journaled `forecast … measured` on their first fold; KBC-04's first `forecast` line is the evidence for T-238 f3 (freight-only, co-pilot key). `fleet serve` restarted.
- 2026-09-16 · wildhorse: **REPLIED on ucf-exchange#50 (MeowNet) with sources, in Ian's name** (Ian: "clarify the lasers / storm question with reference articles from scientific sources … Make sure the referenced sources are very apparent"). Jeff's two corrections accepted with citations: Kp/G-scale touches HF and GNSS only (NOAA scales, ITU-R P.531-16) and no optical propagation; conjunction (DSN 810-005 mod. 106) and Ka rain (ITU-R P.618-14) are the RF enemies; clouds (Turyshev 2026: LCRD two-site 83.8/88.5 %), turbulence (CCSDS 142.0-B-1 §3.9 interleaver) and acquisition (SDA OISL < 100 s) are the optical ones; the storm→optical coupling is orbit prediction (Parker & Linares JSR 2024; the 12-Starlink loss) → `ACQUIRING` and `relay-ephemeris-stale` states. Custody dropped (RFC 9171, CCSDS 734.20-O-1, Le Bihan/Flentge/Fraire 2025) for bundle status reports + lifetime. Numbers: DSOC 267 Mbps @ 55 M km / 8.3 Mbps @ 400 M km (Velasco et al., IEEE JSTQE 32(1)); the optical rungs fall with range². 19 numbered sources. Awaiting the owner's ruling.
- 2026-09-16 · wildhorse: **T-243 slice 3 landed — the tour planner** (`plan_tour`: fewest late deliveries, then ticks, then fuel, over every order of the bay's stops; the tour's first stop is the next leg; companions may aim at any tour stop) and **the feed's `/ships` rows build in parallel** (9–12 s in series over Starlink → the slowest hull; Ian: "UCF familiar on iPad cannot connect? Sees no ships" — the feed was up and answering the whole time, bearer unchanged, at 192.168.108.144 / 100.78.40.47 :7899). Rolled to LOCAL and PROD on Ian's word. See DEVELOPMENT_LOG 2026-09-16.
- 2026-09-16 · wildhorse: **PROD ROLLED** on Ian's word ("We should roll changes into prod, finish up the tour planner and get that rolled out as well") onto aa7831e (clean worktree build, sha256 92046faf…, CI green): KBC-04 (pid 54205), KBC-03 (54259), Kibble Klipper (54314) restarted by PID one at a time, each respawned inside 40 s; KK re-adopted L5609 in transit. Now flying on PROD: T-243 slices 1+2 (the bay), the captain-record reads, the cash ledger on the feed, the fleet-lease advice (KBC-03 titled → advises paying KBC-04's balance until `fleetLeasePay`), and the dispatch/production forecast. `fleet serve` and the LOCAL soak restarted on the same build. **Jeff replied on #50 (MeowNet)** with a research pass: custody transfer is NOT in BPv7 (RFC 9171; a 2025 ESA extension proposes it back) — keep the trail via bundle status reports + lifetime; Kp does not touch X/Ka or optical — the true optical enemies are clouds, pointing/acquisition (SDA budgets up to 100 s) and turbulence bursts; a geomagnetic storm's real optical coupling is orbit-prediction error, i.e. "the relay is hard to FIND" (a new `LinkQuality` word beside `fading`); DTN is operational on NSN/DSN since Jan 2026. Awaits the owner's ruling; a revised proposal in Ian's name is the next act on that thread. **Ian 2026-09-16: "Everything in the lab is working great so use it all. Progress toward familiars goals as well as UCF."** MacOnStick's two held TestFlight ships (UCF Familiar 8, Familiar Agent 113) still wait for the word "ship".
- 2026-09-16 · wildhorse: **FILED ucf-exchange#55** (Ian's name; Ian: "the captain should see all naming of fleet keys and meta tags and have option update them across the fleet") — fleet key management: every key of a captain with every name and tag (traderName, label, hull, app, mintedBy), set on one key or on all keys at once, mint from the record, one-time adoption of the seventeen. Note: `docs/partners/2026-09-16-fleet-key-management.md`. Open asks to Jeff in Ian's name now: #13, #40, #41, #50, #53, #54, #55, UCF-Haul#130, #156, metal#61/62/63/75/80/86/88.
- 2026-09-16 · wildhorse: **FILED ucf-exchange#54** (Ian's name, Ian's words: "I am not biscuit. Never have been biscuit, don't wanna be biscuit.") — a wall note Ian pinned from Haul on Kibble Klipper went up as "Biscuit": since #37 a note keeps the handle of the POSTING key, and Haul's co-pilot key 71f2c596 wears the label Haul gave it. Ask: byline from the captain record (metal#86) when the key resolves to one; relabel 71f2c596 meanwhile. The familiar's own posts (the pilot's key 4bd7a1ed, trader Luke SkyWhisker) are unaffected.
- 2026-09-16 · wildhorse: **IAN'S STATEMENT OF THE FLEET (verbatim): "these three ships are the fleet of Luke SkyWhisker who fly's on Kibble Klipper and commands Felix to fly KBC-03 and KBC-4 — Luke is the captain of all three, and the 'player' in all three — Felix is the ships computer, for all three ships and is a single entity. This needs to be a true statement in the world. Make it so."** The world already says most of it (captain `captain:fb4439e33a54` Luke SkyWhisker, computer Felix, hulls 3, on every careers row). Made true here: the familiar's world-039001 relabelled "KBC-03" (it still said "Kibble Klipper II"). Still false on the exchange and asked of Jeff (comment on metal#86): KBC-03 rides `key:b52c6ace` (a pre-player key-actor from the 08-31 iOS enrolment) while the sisters ride `player:` actors — one kind of actor per captain, career carried across as metal#70 does. Also stale: the Haul-minted co-pilot key 71f2c596 on Kibble Klipper still wears the trader label "Biscuit" (only that key's own request can relabel it; the familiar does not hold it). Not touched: the LOCAL twin stays its own captain ("Luke SkyWhisker (LOCAL soak)") with its own Felix — the statement was about the three PROD hulls; unifying the twin under Luke is one edit if Ian wants one Felix on this host too. Ian pasted three key secrets into the session transcript while giving this direction — rotate 4bd7a1ed / b52c6ace / 8c47ec27 if that transcript is ever shared.
- 2026-09-16 · wildhorse: **the captain record adopted across the fleet; cash ledger read; fleet-lease doctrine landed as advice; the games retirement committed.** `fleet captains --adopt` live on PROD: world-039001 IS KBC-03 (renamed from Haul, record said ""; remembered), all three PROD hulls carry `captain:fb4439e33a54`, Felix agrees everywhere; LOCAL captain filed on the box + renamed "Luke SkyWhisker (LOCAL soak)" + Felix filed. `/captains/{id}/economy` and `fleet economy` now read `/v1/cash` (`flows_source: exchange`). KBC-03 is TITLED (ℳ7,804, debt 0); KK owes ℳ87,297, KBC-04 ℳ18,400 — the doctrine would put KBC-03's ℳ4,804 spare against KBC-04, and cannot: no exchange verb moves credits between a captain's hulls → **FILED ucf-exchange#53** (Ian's name) — `docs/partners/2026-09-16-the-captains-purse.md` (payLease {amount, hull} / remit / the purse; `fleetLeasePay` flag). **Keys:** Ian's key roster (17 in his name, 10 revoked, from 4 apps) written to `~/Desktop/ucf-keys-mine.csv` (+ the full 91-row `ucf-keys.csv`) off `/v1/clients` joined with the stores; **fleet key management ask drafted** `docs/partners/2026-09-16-fleet-key-management.md` (Ian: "will need to be a thing in the UCF world as well"). The other lane's mesh-games retirement (uncommitted since 09-10) landed on Ian's word (d9294ef, 3a3f62b). PROD pilots still on fcdf27b: T-243 slices 1+2, the captain-record reads, and the lease advice are LOCAL-only until Ian says roll.
- 2026-09-15 · wildhorse: **FILED UCF-Haul (Ian's name, Ian's word from the iPad's Station room): CLOSED DAYS collapsed under each cargo name by default, tap a good to open it** — one summary row per good (latest avg/low/high/volume), independent disclosure, remembered per berth for the session; sits beside #148 (candlesticks).
- 2026-09-15 · wildhorse: **FILED ucf-exchange#50** (Ian's name, Ian's word: "Place this for Jeff to review") — MeowNet on the Interplanetary Internet: an optical-terminal fitting with rungs above 19.2 (38400/57600/115200, the UART's ceiling; revisits Jeff's 09-08 ruling, said so), BPv7 bundles with custody trails and lifetimes, hop-delayed house posts as the lead-time game, the stack named on the banner. Note: `docs/partners/2026-09-15-meownet-dtn-lasers.md`. Awaiting Jeff beside #40, #41, metal#86.
- 2026-09-15 · wildhorse: **T-243 slices 1+2 landed — a second load on the way** (`decide_with` + companions; seam `contracts[]` additive, SEAM_VERSION 2). LOCAL soak flies it first; PROD stays on fcdf27b until LOCAL shows a companion booked and delivered and Ian says roll. MacOnStick: the bridge's "Your contracts · N" and DirectFeed sending `contracts[]` are the iPad half.
- 2026-09-15 · wildhorse: **PROD ROLLED** on Ian's word ("Roll prod when ready") onto fcdf27b (T-238 bricks 3+4: dispatch feed, production ledger) — the binary built from a CLEAN worktree at the sha (sha256 331f843c…; the shared tree carries another lane's uncommitted mesh-games retirement, so its build is not the commit's), CI green on fcdf27b. KBC-04 (pid 14534), Kibble Klipper (14578), Kibble Klipper II (14638) restarted by PID one at a time, each respawned by the supervisor within 60 s: KBC-04 re-adopted L5403 in transit; KK re-adopted L5364 delivered and COLLECTED it on its first fold, and journaled the first PROD `forecast … measured` line (deimos clay pit stalled, phobos pit 25%, enceladus fisheries 87%, all "blocked"); KK II under way, no load. LOCAL soak restarted onto the same clean binary. **Swift bar:** red on main since 63c0966 (2026-09-10) on `SCRootView.swift:34` `.topBarLeading` (iOS-only) under the macOS `swift test` — fixed here with an `#if os(macOS)` `.navigation` placement; `swift build` green on wildhorse. MacOnStick: please confirm the iOS look is unchanged.
- 2026-09-15 · wildhorse: **T-238 brick 4 landed — the forecast runs the lines at their measured share** (`/v1/stations/{id}/production`, 24 lines read once per 12-tick bucket; a stalled line keeps its full appetite; `forecast` line gains `measured`). **LOCAL ops:** the LOCAL daemon (Aug 31 binary) rebuilt from Jeff's main 914be11 and restarted under launchd — snapshot t131040, 49 ticks replayed, hashes verified, route live with the newer reason word `starved`; the old binary is kept in the session scratchpad. The soak hull had been stranded at tuna-prime on fuel 0 since before 2026-09-14 — its pilot ran WITHOUT `--allow-paws` and without an `autonomy.json`, so the rescue was advised every fold and never made; fixed LOCAL-only (`pilot_args: ["--allow-paws"]`, dial `{"*":"auto"}` as PROD's KK) → tanker called, tank 600, ℳ2,929, flying. **Brick 3 evidence, LOCAL soak journal:** `dispatch` at t131051 "shelf-restack @ titan-larder/grain ×8300bps t131053–t131065 (confirmed, announced, odds 9700)" and at t131053 the same "in-effect, odds 10000". **Ian's question (2026-09-15) on the Sardine stretch: answered in the brick-4 log entry — the doctrine proposes it on `ship.frame` once titled; the standing-tier gate is the one it does not read (follow-up).** **Brick 4 evidence, LOCAL soak journal t131100s:** `forecast … measured: ["biscuit-press@tranquility stalled", "cannery-line@cannery-row stalled", "catnip-arbor@whisker-hollow 83%", "clay-pit-deimos@deimos-arbor 12% blocked", "enceladus-fisheries@enceladus-draw 37% blocked", "ganymede-mill@ganymede-yards 62% blocked", "gravy-reduction@cannery-row 87%", …]` — LOCAL's economy is mostly starved, which the full-lines forecast could not see. PROD pilots untouched (09-10 build); the roll is Ian's word.
- 2026-09-15 · wildhorse: **T-238 brick 3 landed — the pilot reads the dispatch feed** (Ian: "be sure those changes are made and that familiar is optimized as the ships computer to maximize long term profits and ship operations"). Survey of Jeff's week on ucf-exchange: engine 1.25.0→1.26.0 (the tanker destination check the doctrine already holds for), the co-pilot licence through the fold at price 1, the career across a crossing, MeowNet's daily market brief, the production ledger `/v1/stations/{id}/production` LIVE on PROD (12-tick buckets: cycles, units, idle, blockedReason — brick 4), and the captain record (metal#86, see the T-236 note). The lever the pilot never touched: `/v1/news`. Now every item is read against the exchange's own 72-card deck (vendored, `tools/sync-ucf-deck.sh`) and laid over the forecast as rate windows; `dispatch` journal line on change; 15/15 headlines on PROD+LOCAL today match a card. **LOCAL soak rolled by PID onto the release binary; PROD stays on the 09-10 build until Ian says roll.** **TO JEFF (Ian's call): PROD's `/v1/events` answers a PLAYER key with `willFire` + exact `magnitudeBps` (checked with Luke's key 2026-09-15) — api.md's own "scope trap"; the familiar does not read it, but any client could.** Also found, LOCAL only: the soak hull sits at tuna-prime, fuel 0, ℳ9,696, journaling merchant-idle instead of calling the tanker — rescue path to look at. Swift: `dispatch` ranked with `forecast` in TemplatedVoice; `swift test` on macOS is broken since 09-10 by `topBarLeading` (SCRootView.swift:34) — MacOnStick's iOS bar owed.
- 2026-09-10 · wildhorse: **T-246 standing orders** host half landed (63c0966) and flying on all four pilots + the feed; ucf-exchange#41 filed in Ian's name (problem + proposals A–D). Awaiting Jeff beside #40, metal#86, #88, #80, #28, #75.
- 2026-09-10 · wildhorse: **PROD ROLLED** on Ian's word ("Roll prod") — KK II, KK and KBC-04 restarted by PID one at a time onto the fleet whisker (sisters' inbound cargo, dock fees, frame ladder, hold-bound evidence, chain tie-break, stray collect, scope-capped automations). First thing it surfaced: KBC-04's co-pilot key cannot file `repair` (the six co-pilot verbs are travel, book, cancelBooking, collect, refuel, engage) and repair outranks work in the doctrine → refused every fold. FIXED (adf0c45): `Ship.denied` — the verbs the key cannot file, from `/v1/profile.scopes` at start (no `act` → repair, paws, refit, payLease, expandFrame) and learned at the door on `verb_not_permitted`; the doctrine passes them by (worn and cannot repair → flies the work; dry, no pump, cannot call a tanker → Hold, the captain's papers must). **Seam: optional input `denied: [verbs]`** (absent = everything permitted; SEAM_VERSION 2 unchanged) — MacOnStick: DirectFeed may send it from the key's scopes; pinned on the seam. KBC-04 and LOCAL restarted onto adf0c45; KK/KK II stay on the 23:25 build (act keys, nothing denied).
- 2026-09-09 · wildhorse: fleet as of 20:30 CDT — PROD: Kibble Klipper (KK, Luke's own; act key), Kibble Klipper II (act key), 🐈 KBC-04 🐈‍⬛ (the NG hull re-keyed by Ian from Haul on a co-pilot key, paired FROM THE iPAD — so the app now pairs and deletes ships; Ian named it KBC-04, his scheme); LOCAL: Kibble Klipper II (LOCAL). The watch-only UCF-C-A49703 was deleted from the app by Ian. KBC-04 had been paired with trade+outfit on a co-pilot key → its merchant filed a buy every fold, refused 403 verb_not_permitted; narrowed by hand to freight, trader name set to Luke SkyWhisker. FIXED for good (57ab70b): pairing reads `/v1/profile.scopes` and grants only what the key can file (act → all; auto:freight → freight; read → none); a `verb_not_permitted` refusal drops that automation for the run, journaled once as `automation-refused` (contract fixture +1). LOCAL pilot + feed restarted onto it; PROD still on the 07:02 whisker — a roll is Ian's word.
- 2026-09-09 · wildhorse: T-244 progress on Ian's question — dock fees charged in the merchant (1f8215e), `fleet-inbound` journal line (contract fixture +1; MacOnStick classifies), the wire unchanged (content 28; Haul's new market/station screens draw what the wire already carried); bills are shipper bills (logistics between hulls, not cash) — slice 2 reframed. PROD still on the 07:02 whisker: none of the fleet code flies there until a roll.
- 2026-09-09 · wildhorse: **T-244 (fleet works together) opened, slice 0 landed** (a9e102f: sisters' inbound cargo in every forecast; LOCAL first, PROD on the next roll); **T-245 (fleet of eight) opened** — enrolment needs an enrol-scoped app key (UCF-Haul has it, the familiar does not); hull names set by the captain's own key only (`POST /v1/profile`), co-pilot keys refused — KK NG is named from Haul. Naming scheme awaiting Ian.
- 2026-09-09 · wildhorse: iPad regression, mine, fixed on the host: the row's `persona` gained `pronouns` and the iPad's STRICT persona reader (build 7) refused it → `FamiliarSC.StoreError error 2`, every automation reading locked (Ian). The feed now strips `pronouns` from the row's `persona` (shape the reader knows) and carries them on `computer_state.pronouns` only. **MacOnStick: teach `ShipStore`'s strict persona decode the `pronouns` object {label, subject, object, possessive}; tell me and I drop the strip.**
- 2026-09-09 · wildhorse: **Kibble Klipper NG paired** (key d01caca3, Ian's new co-pilot key from UCF-Haul; scopes read + auto:freight → automations freight only) under Luke SkyWhisker, joining Felix (he/him). Found and fixed on the way: (1) the uniqueness check refused a JOIN because the LOCAL twin's captain also wears Felix — joining is not a naming, only a given name is checked (551143e); (2) the iPad had paired the same key 48 s earlier with the captain typed "Luke Sky-Whisker" → two worlds, two pilots, one hull — the duplicate unpaired, and pairing now refuses a key that already flies a world (one key is one hull); (3) the exchange had enrolled the new actor as "Luke Sky-Whisker" and the key's own record as "Biscuit" — one request with X-UCF-Trader set the key's record to Luke SkyWhisker and the roster now reads Luke SkyWhisker for all three hulls (that "Biscuit" was UCF-Haul's handle on the key it minted). (4) Ian: "no way to delete a ship from your fleet… Familiar app should allow for that" — host half landed: **`DELETE /ships/{id}`** on the feed = `fleet unpair` (pilot stopped, key file gone, world decommissioned; journal, deliveries and persona kept). **MacOnStick: swipe-to-delete on "Your ships" → DELETE /ships/{id}; also read `pronouns` on persona/computer_state and retire the hardcoded Her.**
- 2026-09-09 · wildhorse: FILED ucf-exchange#40 (Ian's name, Ian's word) — publish crew wage/relief and frame-pod params on /v1/reference; a hire is a 500-credit guess without them. Awaiting Jeff beside metal#86, #88, #80, #28, #75. UCF Familiar build 7 (ATS fix b31b10e) UPLOADED to TestFlight.
- 2026-09-09 · wildhorse: **T-243 (multi-load routes) opened on Ian's word** — engine bay limit 3 is live on PROD (UCF-Haul#43 shipped; route-planning half moved to UCF-Haul#74 dir. 3). LIVE LEAK found and fixed as slice 0: the single-active doctrine left KK II's L4200 delivered with ℳ684 payable for two days; `wire::stray_payables` + a collect-first fold step on the `freight.collect` dial. LOCAL first; KK II's ℳ684 waits on the PROD roll.
- 2026-09-09 · wildhorse: **T-242 (crew / galley / frame ladder)** opened on Ian's question and SLICE 1 landed: crew/galley/frame/holds/obligations facts on every feed row; `bound` on position-opened; outfit `ExpandFrame` proposal (title + ≥5 hold-bound buys/7d + reserve) on the `ship.frame` dial; crew idle reasons say the wage/relief are unpublished. Journal vocabulary +2 (`frame-expanded`, `frame-refused`) in the shared contract fixture — MacOnStick classifies them in Notices/TemplatedVoice. Draft ask for Jeff (publish crew + frame params on /v1/reference) at docs/partners/2026-09-09-crew-frame-params-ask.md — not filed, Ian's call.
- 2026-09-09 · wildhorse: **PROD ROLLED** on Ian's word ("Roll prod") — both PROD pilots restarted by PID one at a time and respawned by the supervisor on the T-238 release whisker (788d94b: stock-priced forecast, sell side reads it, freight chain tie-break, journal keys on change). KK II pid 35106, KK pid 35174; LOCAL soak twin on the same binary since ~07:02. Old binary's last act before the roll: KK II sold 60 gravy-base at bid 19 (basis 17) at t10226. Watch the PROD journals for the first forecast-justified buy with its reason.
- 2026-09-09 · wildhorse: **T-241 proposed by Ian** (economic history / trend lines / profit analysis by captain on the UCF Familiar views) — HOST HALF landed the same day: `economy.rs` (exact deltas from journal `credits`, cause-attributed flows, hourly-thinned points, per-captain pooling, trend, best/worst, analysis sentences), `GET /captains/{id}/economy?window=`, `economy` summary on the captain brief, `fleet economy`. iPad half is MacOnStick's (BOARD has the shape). Feed restarted by PID onto it.
- 2026-09-09 · wildhorse: T-238 finding 3 FREIGHT HALF landed (b4c51ee + 788d94b, announced to MacOnStick first, core rebuild requested on 788d94b): `LoadRow.chain_pressure` (+1 feeds a starving works / lifts a glutting shelf inside the horizon, via `Forecast::pressure_on`; host sets it each fold; seam reads it optionally, absent = 0); the doctrine breaks near-ties (within CHAIN_TIE_BPS 5% of the best rate) toward the pressured load, stably, never lifting a materially worse load; reasons `freight.chain-preferred` only when the tie-break decided. SEAM_VERSION 2 unchanged. Also tonight: the SELL side reads the forecast (cd9e015) — the LOCAL soak's first forecast trade (t73323) sold a lot at the target's spot and re-bought it two ticks later at the target's forecast; best_forward now values a held lot at its projected mid at arrival OR once the shelf drains, hurdle-charged; the forecast journal keys on shelf/price change not the countdown (afd7e75). Names ruling implemented (c41e2a0, 0dc731e, 3dc30b9): ledger, uniqueness, never-sweep, `fleet names [--backfill]`, names on the captain brief + GET /names; backfilled live (7 rows). ucf-exchange#37 filed; metal#86 carries the rule. CLOSED (cause found, wildhorse's own fault): the LOCAL soak pilot did not respawn because the SUPERVISOR WAS DEAD — wildhorse's `pkill -x familiar` while restarting `fleet serve` (~23:45 CDT) also killed `fleet run` (same binary), exactly the pkill pitfall on record. Supervisor restarted 2026-09-09 ~07:10 on the new CLI; leases had 3.6 h left and renew under 2 h; no pilot lost a lease. Rule restated: restart by PID, never by name. UCF Familiar build 6 (core on 788d94b + B4 r2 repair + names/story) UPLOADED to TestFlight; direct install skipped again (devices unreachable from wildhorse). Codex: PLAN limit hit on T-236 r3 at 05:59 — queue sleeps to Sep 14 23:15 CDT unless Ian buys credits; chair self-reviews meanwhile. PROD hulls stay on the pre-T-238 whisker until the LOCAL soak shows the sell-side rule live.
- 2026-09-08 · wildhorse: T-238 codex design review (REJECT, 4 blockers) REPAIRED on main — da86a59 + cedd6f1. The forecast projects shelf STOCK along its flow and prices it with the exchange's own integer formula (ported from UCFEngine Pricing.swift behind distinct Units/Credits types), never the equilibrium count as ℳ; the target scan prices every buyer inside the 4-question route budget and takes the best forecast net after fuel and decay (deterministic tie-break); decay charged on the new position; glut here breaks ties toward lifting; the reason rides into position-opened and traded, bounded 240 chars, one builder; offline soak tests/t238_forecast_soak.rs from reference + quotes + galaxy documents. Finding 3's FREIGHT half needs doctrine.rs (the seam) — held for its own announced commit. Whisker 93/0, workspace 1004/0, clippy clean. Codex round-2 re-verification owed, queued behind B4 r2.
- 2026-09-08 · wildhorse: T-236 b1 round 2 — findings 2, 4 and 7-host REPAIRED on main (this entry's commit). fleet.rs: `computer_origin` resolves a captain's computer as a TYPED origin (id store / legacy slug store / ship-local / none) across every same-captain hull, refusing two ship-local records that disagree; `migrate_computer` brings a ship-local record WHOLE (persona bytes + trail) into the id store, idempotently; `adopt_siblings` points every same-captain hull at the chosen id; a tuned Purr is retained (the name filter is gone); `ensure_captain_id` returns Result under a fleet migration lock (`captains/.migrate.lock`), inspects ALL siblings, refuses a captain who already carries two ids and a legacy slug collision, moves the store before installing the identity, and finishes an interrupted move; `fleet adopt-ids` inspects every record before assigning and writes every same-captain sibling in one pass; rename migrates first, so a legacy hull renames the computer it HAS; `fleet status` pools by captain_id (display name carried, `captain_id` in the JSON). fleet_serve.rs: ADDITIVE `computer_state` {named|broken|absent} on every /ships row, fleet-brief captains[] (now keyed by identity, `captain_id` carried), captain brief (context + top level) and ship brief context — whose `computer` word now says "(will not load: …)" instead of null; an ambiguous stale slug on /captains/{slug}/brief returns 410 with `candidates` and no captain chosen. Pinned: whole migration + sibling adoption, disagreeing records refused, rename on a legacy hull keeps the tuned computer, adopt-ids one-id-per-captain whichever hull is first, colliding slugs refused; and on the feed a broken store is broken on ship row / fleet brief / captain brief / ship brief with a valid ship-local record beside it, named/absent distinct, ambiguous 410. cli 19/0, kernel 249/0, clippy clean. MacOnStick's WireFeed prefers computer_state (claude/computer-state). Round 3 re-verification brief follows; queue behind B4 r2.
- 2026-09-08 · wildhorse: T-236 b1 round 2 (REJECT) — findings 3 and 6 REPAIRED on main. kernel persona::name is now a recoverable commit protocol under the persona lock (prior pair remembered; temp synced; trail appended+synced; rename; DIRECTORY synced — any failure at append/rename/dir-sync restores the prior persona and trail before it is reported; every durability error propagates); record_naming (the unlocked path) is gone and fleet rename goes through the one helper, exiting FAILURE on an unwritable trail with the persona unchanged; pairing names the computer BEFORE instance::commission, so a naming failure leaves no world, ucf.env, captain.json or event. Pinned in kernel (rename failure restores the trail; first naming that cannot land leaves no trail) and at the CLI against a loopback stub exchange (rename with the trail a directory → FAILURE, still Felix; second pairing with the trail a directory → FAILURE, one world, no new registry entry). Kernel 248/0, cli 11/0, clippy clean. Findings 2, 4, 7-host next (typed origin + whole-persona migration under the captain lock; Result-returning idempotent adopt-ids with a fleet lock, siblings inspected first, slug collisions refused, ambiguous stale slug → 410 with no captain chosen; typed persona state on every brief context and row — field name to MacOnStick before it lands).
- 2026-09-08 · wildhorse: FILED ucf-exchange#37 (Ian's name, Ian's word: "File the name issue pilot not biscuit") — wall bylines show the actor's NEWEST key's display name, not the pilot who posted: BBSMessageBase.swift:139-142 `keys.last(where actorID…)?.displayName` at read time; NoticeboardStore keeps actorID + text only (the other subs post WITH a handle). Ask: store the handle on the note at post time; never re-derive an old note's author from a later key. Awaiting Jeff beside metal#86, #88, #80, #28, #75.
- 2026-09-08 · wildhorse: ops — the LOCAL world (:7877) had been DEAD since 2026-09-05 16:13Z (never came back after the reboot; the soak pilot logged 4,057 exchange-unreachable lines); restarted via ucf-local/run-local-world.sh — it declared a market holiday, jumped to 70991, folded 2016 and is ticking. LOCAL soak pilot restarted onto the T-238 release binary (LOCAL first per Ian's doctrine; both PROD hulls stay on the old binary until the soak shows the forecast behaving). UCF Familiar build 5 (T-237 B4 Swift half, 749c4d9 + bf0bb9c archive fix) UPLOADED to TestFlight from wildhorse; direct install skipped — none of the three devices reachable from wildhorse at the time. Jeff's wall byline: an exchange bug (BBSMessageBase joins the author from the NEWEST key on the actor at read time; the note stores actorID only) — explained to Ian; filing is his call.
