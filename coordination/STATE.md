...
- **KBC-03 AND THE SARDINE STRETCH (Ian, 2026-09-19: "Is the familiar set to purchase the sardine stretch for KBC-03? Still untested feature. Need to get there eventually.")** Read off wildhorse's store `world-039001a6…`: dial `*: auto`, outfit grant held, titled (debt 0), 19 hold-bound buys in the window (bar 5) — every frame gate open; the pilot journaled "saving for the Sardine stretch frame: ℳ18000 + reserve ℳ3000" 09-14→09-16. **Since 09-16 21:49Z the rung is SHADOWED:** `outfit::decide_outfit` returns the sister pay-down first whenever a sister owes and spare > 0 (Kibble Klipper ~ℳ87k, KBC-04 ℳ15.4k; KBC-03 holds ℳ12,986) — the exchange cannot take it (ucf-exchange#53 open), so it degrades to `advice` on `ship.lease` every fold and `next_rung` is never reached. Then cash (ℳ21,000 needed) and standing (engine `expandFrame` requires Hauler tier; the doctrine does not read standing; tier unverified — not on the `/ships` row). **Proposed brick (whisker, host lane — wildhorse unless Ian hands it here):** (1) a pay-down the exchange cannot file does not shadow the hull's own next rung — advise the lease once and fall through to `next_rung`; (2) read `standingBps`/tier into the frame decision so the pilot saves only for a rung the yard would cut, and says the tier gap plainly. Untested feature, by Ian's word: the first live `expandFrame` should be watched.
