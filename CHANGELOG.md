...
[Semantic Versioning](https://semver.org/) once it reaches 1.0. This file is the
human-readable summary; the per-change engineering detail lives in the commits.
...
### Removed
- **The mesh party games are retired** (the owner, 2026-09-10: "I think we can remove the game
...
### Added
- **The tour planner.** With up to three contracts in the bay, the pilot
  ...
  Status: **shipped**.
- **The fleet's records follow the world's captain record.** `fleet captains
...
- **The economy view reads the exchange's cash ledger** (`/v1/cash`): every credit that moved,
  ...
  Status: **shipped**.
- **An owned hull's surplus pays the fleet's leases**: the smallest sister
...
- **A second load on the way.** Berthed with a contract booked or laden,
...
- **The forecast runs the lines at their measured share.** The exchange's production
...
- **The pilot reads the dispatch feed and moves before the effect** (the owner,
...
- **The familiar has its first partner AI.** Other AIs meet the familiar at its door over
  ...
  household's own Mac, minted as a principal on 2026-08-23 by the owner's own two-tap ceremony at
...
- **A confirmed intent becomes a standing rule.** "When I'm away, dim the lights; when
  ...
  `familiar rules add <human> away lights dim`, fired on real presence transitions through
...
- **Every device has its own record now.** Machine facts (kind, OS, architecture,
  ...
  deliberate human-given fact (`familiar mesh device name <id> <name>`) that outranks
  hostnames and generic labels everywhere the roster speaks, and replicates door to
  door. The roster's sentence — *name : kind : human* — is now data all the way down.
...
- **Devices lead with their names, everywhere.** The roster, the welcome screen, the globe
  ...
  name, with the hardware's own label as quiet small print beside it and the numeric node
  id demoted to small print and the node detail screen. A device the mesh hasn't named
  yet appears as what it honestly is (*unnamed Mac*, *unnamed iPhone*) instead of a bare
  hex id: a node id is an address, not a name.
...
- **Answers steer + LLM authors solutions (with question-fade):** replying in
  ...
  *Validated by unit tests* (`cycle::pursues_open_threads_into_candidates`) + real-world
  operation.
- **The familiar acts on its theories:** a theory carries a *direction*;
  ...
  "answered" when the human replies. TickReport.pursued; CLI shows it.
- **The familiar theorizes:** the Interpret step — `kernel/thread.rs` (a
...
- **Daemon control + launchd:** `familiar daemon status|start|stop|reload`
...
- **GUI control bar + interaction channel:** the Glass gains
  ...
  `question.txt`) with a text box; the human's reply is recorded as an observation
...
- **The cycle closed** — the metabolism is now a full loop:
  - **Execution** (`crates/exec`): a sandboxed runner (resource limits,
  ...
  - **The LLM in the loop** (`crates/llm`): boundary-gated `consult`; the
  ...
  - **The unbounded daemon**: `run --daemon` / `--ticks 0`, every
    `--interval` (default 60s), Ctrl-C to stop.
  - **The capacities signal** (`capacities.rs`): Law II deepened toward
    HUMANITY.md — flags the *comfortable replacement* (present but hollowed out), not
    just absence. CLI `capacities`.
- **The evolutionary kernel** — ported from v1 to Rust, subordinate to the
...
- **Sense** — `crates/sense`: the familiar perceives its host (OS/CPU/memory,
...
- **The metabolism** — `crates/cycle`: one tick = sense → detect loops →
...
- **Presence signal (Law II)**: `presence_signal()` measures served
...
- **Obedience guard (Law III)**: `guard::evaluate()` returns allow /
...
- **A typo can no longer mint a member.** Membership acts (grant, name, corrections)
  ...
  record-layer regressions; the live ghost this fixes was found wearing a real
  machine's name.*
...
- **A console no longer files under a neighbour's machine.** From the lighthouse's point
  ...
  that as proof of residence — one Mac's console nested under another Mac's card. A
...
- **The workspace** — Cargo (`crates/kernel` + `crates/cli`), Rust, with
  `#![forbid(unsafe_code)]`; `store.rs` JSONL persistence over `serde`.
- **The observation spine** — the `Observation` record (the only truth)
  and `substrate observe` / `observations`.
- **The service signal (Law I)** — `service_signal()` measures
