...
#      147cfa12 turns out to be a guest's first install rather than a reinstall of the owner's
#      phone, their next launch restores them and nothing was lost.
#
...
#
# Env (both required — the lighthouse's address is deployment-specific and is never baked in):
#   LIGHTHOUSE     ssh target for the lighthouse, e.g. root@198.51.100.10
#   PUBLIC_ADDR    the lighthouse's own public address, the one it should advertise
set -euo pipefail

: "${LIGHTHOUSE:?set LIGHTHOUSE (ssh target for the lighthouse, e.g. root@198.51.100.10)}"
: "${PUBLIC_ADDR:?set PUBLIC_ADDR (the public address the lighthouse should advertise)}"
