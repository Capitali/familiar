...
#
# Env:
#   LIGHTHOUSE   the ssh target, e.g. root@198.51.100.10 (required unless given as $1)

set -euo pipefail

TARGET="${1:-${LIGHTHOUSE:?set LIGHTHOUSE (ssh target for the lighthouse) or pass user@host}}"
...
# The file is untracked (it names the deployment's own nodes); the deploy carries it so the roll
