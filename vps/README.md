...
peers (FamTalker01, behind CGNAT) run the same posture and simply wait for the
network to be kinder.
...
```sh
# On any minting member (a host already in the mesh):
...
From there convergence is automatic: the fleet nodes gossip with the lighthouse
