...
#   - the VPN/overlay-network login, and the decision: fresh mesh node vs migrating the
#     old data dir.
...
if [ "$MODE" != "daemon" ]; then
  ...
  step "git identity"
  # Set once per machine, from the environment — this script never bakes in a person.
  git config --global user.name  >/dev/null 2>&1 \
    || { [ -n "${GIT_AUTHOR_NAME:-}" ] && git config --global user.name  "$GIT_AUTHOR_NAME"; } \
    || echo "  (set GIT_AUTHOR_NAME, or run: git config --global user.name  \"Your Name\")"
  git config --global user.email >/dev/null 2>&1 \
    || { [ -n "${GIT_AUTHOR_EMAIL:-}" ] && git config --global user.email "$GIT_AUTHOR_EMAIL"; } \
    || echo "  (set GIT_AUTHOR_EMAIL, or run: git config --global user.email \"you@example.com\")"
  ...
  if [ ! -f "$HOME/.ssh/id_ed25519_github" ] && ! ssh -o BatchMode=yes -T git@github.com 2>&1 | grep -q "successfully authenticated"; then
    if [ ! -f "$HOME/.ssh/id_ed25519" ]; then
      ssh-keygen -t ed25519 -C "$(id -un)@$(hostname -s)" -f "$HOME/.ssh/id_ed25519" -N ""
    fi
  ...
  echo "  5. Install the VPN/overlay client and log in, if this Mac joins an overlay network."
fi
...
if [ "$MODE" = "daemon" ]; then
  ...
  # help (observed live, 2026-08-13). So: bootout while the OLD binary is
