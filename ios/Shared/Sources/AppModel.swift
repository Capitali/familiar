...
final class AppModel: ObservableObject {
    ...
    /// A join is in flight — the console shows progress instead of the static path card.
    ...
    /// Stacks with the hub's cloud_ok — both must hold; default off. (Toggle UI: sphere, a later pass.)
    ...
    init() {
        ...
        // standing is UNKNOWN — it is not a visitor. Assuming guest here meant every
    ...
    func deviceStateJSON() -> String {
        ...
        // The app has never read this, so `Mine — the owner's` in the console and
        // `established='some-mac'` in the record could not disagree in any way the system was
        ...
        // This is deliberately phone-local: the shared Device screen may render what the
        // iPhone already knows about its paired watch, but no observation/worldview field is
        // created here. Any later, retention-governed mesh report is owned elsewhere.
        ...
        let d: [String: Any] = [
            ...
            "attempts": attemptLog,
            // The join story as machine state: the console shows what is being TRIED
            ...
            // The mesh's own answer, and whether it names a machine instead of a person.
            "establishedHandle": establishedHandle,
            "establishedIsMachine": looksLikeMachine,
            // Join in flight: the console shows a progress indicator instead of snapping
            ...
            // the toggle no longer decides it on its own.
    ...
    /// A lowercase, dash-separated handle from a display name — "New Member" -> "new-member". Mirrors
    ...
    /// What the join machinery is DOING right now — a state the screens can branch on,
        ...
        for host in candidates {
            ...
            do {
                ...
                if let tok = pendingInvite {
                    ...
                    // The human named themselves before the handshake finished. Their act
    ...
    private func introduce(claim: IdentityClaim?, evidence: Evidence) async -> Bool {
        ...
        // anybody says to it, and it must never be dropped on the floor.
        //
        ...
        // had already been given. The owner hit exactly this adding an iPad, 2026-08-15.
        //
        ...
        introduceHeldReason = ""
        // Show the join is IN FLIGHT: a name entered kicks off a round trip (evidence,
    ...
    func unenroll() {
        ...
        // Intents projection dies with the enrollment.
    ...
    func maybeProbeTailnet() async {
        ...
        // theories screen flickered between a Mac's and the lighthouse's). A path upgrade
    ...
    func heartbeatStatus(readHost: String) async {
        ...
        // we saw — "X is here" and "X was here an hour ago" must not arrive as the same fact.
        let claim = presence
        ...
        // Active use of the console IS presence: if the ladder holds no live claim but the
        ...
        // presence/session are computed from local evidence, so a device whose status
    ...
    /// a game move, a consent toggle. Feeds the interaction-presence signal.
    ...
    /// so a waiting acceptance is visible even on the first read.
    ...
    /// disestablish ("that's not who that is"), hold, restore — signed and sent to the node this
    ...
    /// these concurrently, so none of them may read the mutable `host`.
    ...
    /// checkpoints, never on every healthy answer (an external re-verification).
    ...
    func refreshWorldview() async {
        ...
        // The SECOND journey narrates too. An enrolled console at launch is not
        // joining — it is walking its doors for a first read, and that walk can take a
        // minute across timeouts. The join was taught to speak; silence here resolved
        ...
        // The race: the doctrine's order becomes a HEAD START, not a serial
        ...
        if let win = race.winner {
            ...
            // the second person is us. A waiting acceptance must announce itself: the
            ...
            worldviewError = nil
            // Refresh the external-indexed projection App Intents serve:
            ...
            await refreshPartnerInbox()
            // The first successful read closes the join story — but only on a
        ...
        // also land in attemptLog, which the Device screen already renders (the field
        // existed with a render path but was never written).
        attemptLog = attempts
        // The badge says ONE sentence: how many doors, what most of them
        ...
        // A link that had been joined and dropped narrates the same way.
    ...
    /// authority every client follows (the owner, 2026-08-24: "The clients are authorized by the user, so
    ...
    /// permission — there is no API to ask, so this string must never imply it (that honesty
    /// belongs to a separate, still-open question).
    ...
    ///
    /// **Authorization, not consent** (the owner, 2026-08-24). Two things must hold and they are
    ...
    /// narrow it — it can decline, never permit. Before this change, iOS armed on the local toggle
    ...
    func startDiscoveryIfAuthorized() {
        ...
        discovery = d
        d.start()
        // The BLE surveyor rides the SAME authorization — one gate, every radio.
