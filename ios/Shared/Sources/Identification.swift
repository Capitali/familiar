...
// being recognised ever granted access, then "the camera thinks this is so-and-so" would start
// handing out that person's body data to whoever happened to be standing there. A claim here is a
...
//   2. face      — VERIFY that prior 1:1 ("should be the owner; does the face agree?"), not 1:N.
...
enum DeviceRole: String, Codable, CaseIterable {
    ...
    case personal
    /// Many people touch it — a kitchen iPad, a desk Mac. No prior; identification must be earned.
    case shared
...
/// "X is here" and "X was here an hour ago" must never be the same fact, or routing a
/// question to X is a guess dressed as a decision.
struct PresenceClaim: Equatable {
    ...
    /// the fact it rests on ("this is that person's phone") is durable, where a face is a moment.
