...
- **Ship a release with ONE command:** `bash ios/tools/ship.sh <build-number>` — bumps and
  ...
  direct-installs to the paired devices, uploads to TestFlight. Never hand-roll the chain.
  It reads its account from the environment and refuses to start without it:
  `ASC_KEY_ID`, `ASC_ISSUER_ID`, `ASC_APP_ID`, `ASC_BETA_GROUP` (and optionally
  `ASC_KEY_PATH`, plus `FAMILIAR_DEVICE_UDIDS` for a device this Mac cannot discover).
