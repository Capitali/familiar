...
final class WatchModel: NSObject, ObservableObject {
    ...
    /// worked and could not see, or report, one word about why. Failure is a fact the
    /// familiar owes the person in front of it, in the same way progress is.
    ...
    /// iPhone screen that offers it (`StatusView`) is not reachable in the shipping app,
...
extension WatchModel: WCSessionDelegate {
    ...
    // A live message carries the address too, for a watch that is reachable right now.
