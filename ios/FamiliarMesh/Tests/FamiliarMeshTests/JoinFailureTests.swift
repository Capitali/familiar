...
/// T-247: every door failed → the badge carries one sentence; the per-door lines stay
/// diagnostics. The shapes here are the live ones from familiar#10 (build 100, 5G+).
final class JoinFailureTests: XCTestCase {
    ...
    func testFifteenTimeoutsAndALostLighthouseBecomeOneSentence() {
        // Ian's iPhone off Wi-Fi: every LAN and tailnet door timed out, the lighthouse dropped.
        var lines = (10...23).map { "192.168.108.\($0)→t:The request timed out." }
