...
/// The launch read races its doors — the doctrine's preference is a head
/// start, dead doors are demoted or expired, and the lighthouse is always worth a
/// knock. Every rule here exists because a stale remembered LAN door cost an
...
final class CandidateRaceTests: XCTestCase {
    ...
    // MARK: - external re-verification 2026-09-07 (rejected → repairs)

    private let poll: TimeInterval = 5
    private let week = CandidateRace.expireAfterSeconds

    /// A door that has NEVER answered, failing on every 5 s poll,
    ...
    /// The live shape: the dead LAN door's request outlives the
    ...
    /// that clock (the review's ask: the two silences are different facts).
    ...
    /// Expiry is one transition over BOTH stores — the expired door
    ...
    /// The remembered set is finite. Far more changing valid addresses
    ...
    /// An expired address learned again later returns with fresh health.
    ...
    /// Many healthy polls against a live lighthouse must not write the
    ...
    /// A dead door failing on every poll writes on the way to demotion
    ...
    /// The state that matters survives a relaunch through the store —
