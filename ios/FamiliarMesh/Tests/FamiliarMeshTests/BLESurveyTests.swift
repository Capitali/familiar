...
/// The BLE survey's floor (closed): service-UUID class + coarse per-window
...
final class BLESurveyTests: XCTestCase {
    ...
    func testTheWindowReportIsClassAndBucketOnlyAndDeterministic() {
        let report = BLESurvey.windowReport(classCounts: [
            ...
            "battery": 3,
            "vendor-serial": 9,
            "Someones-Watch": 2, // a class the repo never authored cannot enter a report
        ...
        for entry in report {
            XCTAssertFalse(entry.object.contains("Someones"))
