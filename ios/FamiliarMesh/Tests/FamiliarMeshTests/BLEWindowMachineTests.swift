...
/// The scan-window state machine (from an external BLE review): the two regressions the
/// return named, plus the cross-language vocabulary drift pin it also asked for.
final class BLEWindowMachineTests: XCTestCase {
    ...
    /// First case: the same stationary peripheral contributes once in EACH of two
    ...
    /// Second case: poweredOn → refused/off → poweredOn leaves exactly ONE armed clock, and
    ...
    /// The drift pin, Swift half: the Swift survey classes and the daemon's
