...
/// The survey vocabulary both shells now speak. What is pinned here is not the
...
final class ServiceSurveyTests: XCTestCase {
    ...
    /// **Q1 CLOSED: no advertised name reaches an observation. Ever.**
    ///
    ...
    func testNoAdvertisedNameSurvivesIntoContext() {
        XCTAssertEqual(ServiceSurvey.context(forInstanceName: "A Person's MacBook Pro"), "")
        XCTAssertEqual(ServiceSurvey.context(forInstanceName: "Someone's AirPods"), "")
        XCTAssertEqual(ServiceSurvey.context(forInstanceName: "lighthouse._familiar-mesh._tcp"), "")
        XCTAssertEqual(ServiceSurvey.context(forInstanceName: ""), "")
    }

    /// The external review's acceptance gap, closed: the shared list is only the whole truth if BOTH
