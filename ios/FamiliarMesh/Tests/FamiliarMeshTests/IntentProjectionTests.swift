...
/// The external-indexed projection: what Siri/Spotlight/lock screen may hear is
...
final class IntentProjectionTests: XCTestCase {
    ...
    private func worldview() throws -> Worldview {
        let json = """
        {
          "group_label": "river",
          "node_id": "node-abc",
          "question": "Has the other iPad been seen since Tuesday?",
          "question_owner": "ada",
          "presence": 0.8, "withdrawn": false, "service": 0.5, "capacity": 0.9,
          "observation_count": 1234,
          "peers": [
            {"node_id": "peer-1", "label": "Lighthouse Node", "last_seen": 100,
             "tools_offered": 0, "patterns_offered": 0},
            {"node_id": "peer-2", "label": "Older Mac", "last_seen": 90,
             "tools_offered": 0, "patterns_offered": 0}
          ],
          "recent": [
            {"actor": "phone:ian", "action": "reports", "object": "motion:walking",
             "context": "near the back door", "source": "mesh:node", "ts": 100, "confidence": 0.9}
          ],
          "services": [
    ...
    func testProjectionCarriesKindsCountsAndFactsOnly() throws {
        ...
        let leaks = ["ada", "Lighthouse Node", "Older Mac", "phone:ian",
                     "walking", "Tuesday", "back door"]
        for leak in leaks {
    ...
    /// `ServiceView.kind` upstream is NOT an allowlist — a
    ...
    func testAHostileKindNeverBecomesSpeakable() throws {
        let json = """
        {
          ...
          "services": [
            {"kind": "Someones-iPhone", "name": "", "seen_by": "x", "last_seen": 1},
        ...
        let raw = String(data: try JSONEncoder().encode(p), encoding: .utf8)!
        XCTAssertFalse(raw.contains("Someones"), "a hostile kind leaked into the projection")
    }

    /// A timestamp without enforcement is not a freshness fence.
    ...
    /// The re-review edge: the freshness interval is closed on BOTH sides. A
