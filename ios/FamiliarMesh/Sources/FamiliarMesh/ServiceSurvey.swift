...
/// The one vocabulary every shell's Bonjour survey speaks.
///
...
public enum ServiceSurvey {
    ...
    /// Service types worth surveying — peers first, then the everyday advertisements a household
    /// network exposes: remote access, media endpoints, printers, smart-home, file shares,
    /// brokers.
    ///
    ...
    /// **Closed (2026-08-25): the name drops. Every name.**
    ///
    /// A survey reports the service kind; the advertised instance name — a person's laptop,
    /// someone's earbuds — is overwhelmingly personal and never becomes household memory. No
    ...
    ///
    /// Do not add a second path to `context`; one seam, one discipline, every radio.
