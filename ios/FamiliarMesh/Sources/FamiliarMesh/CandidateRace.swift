// CandidateRace — the launch read stops paying a timeout per dead door.
//
// The complaint, live on an iPad (2026-08-31): the hub's LAN lease moved on a
...
public struct DoorHealth: Codable, Equatable, Sendable {
    ...
    /// every miss refreshes it, so it must never drive expiry (an external
    /// review: "recently touched" is not "recently alive").
...
public enum CandidateRace {
    ...
    /// Expiry as ONE transition over both stores (an external re-verification):
