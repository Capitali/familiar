...
/// The kind-only projection served to EXTERNAL-INDEXED audiences.
///
...
public struct IntentProjection: Codable, Equatable {
    ...
    /// Canonical service kinds around the network (the survey classes) — kinds only;
    ...
    /// The closed kind vocabulary this projection may speak.
    ...
    /// `service:Someones-iPhone` and a trusting projection would say it as a "kind". Only
    ...
    /// vocabulary and the BLE survey's classes (both repo-authored, both reviewed
    /// when edited).
    ...
    /// for a fresh read instead. One hour: worldview reads
    ...
    ///
    /// The accepted interval is explicit and closed on BOTH sides (an external re-review): a
