...
/// Bonjour/mDNS service survey. It no longer *mirrors* the iOS shell's list — both shells now
/// browse `ServiceSurvey.serviceTypes`, one list, so they cannot drift apart
...
final class MacNetworkDiscovery: NSObject, ObservableObject {
    ...
    /// The shared survey vocabulary — the same list the iOS shell browses, so the
