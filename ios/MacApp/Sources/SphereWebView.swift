    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            if bridge.mode == .street {
                ...
                // The sat/street toggle lives in the LOWER-RIGHT corner: a deliberate
            ...
            // strip swallowed the top-centre Dialogue glyph's clicks on macOS, because
...
final class SphereBridge: NSObject, ObservableObject, WKScriptMessageHandler, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func start(web: WKWebView) {
        ...
        discovery.onDiscovery = { [weak self] type, name in
            // The divergence closed WITH Q1 (2026-08-25), as it was staged to:
    ...
    func poll() async {
        // Push what the app knows BEFORE the read: the first worldview walk can
        ...
        if let json = model.worldviewJSON {
            ...
            // Say WHY the link is down — this call used to pass no argument, so the
        ...
        Task { @MainActor in
            ...
            case "partnerAct":
                ...
                Task {
                    ...
                    // refusal reason — so a tap never dissolves into silence.
    ...
    nonisolated func locationManager(_ m: CLLocationManager, didUpdateLocations locs: [CLLocation]) {
        ...
        Task { @MainActor in
            // Relay this Mac's own fix in the status heartbeat: without it, a Mac read
