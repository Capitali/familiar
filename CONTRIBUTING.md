...
(2026-08-15); the same trap was later removed from `ship.sh`.
...
The owner, 2026-08-16, after this bit us six times in one session: *"no more assuming
that no data equals a negative state."* Every one of these shipped, and every one was
invisible because a confident wrong answer looks exactly like a right one:
...
4. records what changed, why, which checks were run, and what is next.
