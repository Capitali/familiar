...
mod tests {
    ...
    fn tablet_name(ip: &str, _timeout: Duration) -> Option<String> {
        (ip == "198.51.100.42").then(|| "tablet".to_string())
    }

    #[test]
    fn a_gated_reverse_lookup_names_a_numeric_neighbor_before_it_reaches_the_frontier() {
        let numeric = device("198.51.100.42", "198.51.100.42");
        ...
        assert_eq!(reaches[0].label, "tablet");
        assert_eq!(obs[0].object, "device:tablet");
        assert!(obs[0].context.contains("ip=198.51.100.42"));

        let (closed_reaches, closed_obs) =
            assess_with(deaf, tablet_name, &[numeric], 42, 100, false);
        assert_eq!(closed_reaches[0].label, "198.51.100.42");
        assert_eq!(closed_obs[0].object, "device:198.51.100.42");

        let already_named = device("dhcp-name", "198.51.100.42");
        let (named_reaches, _) = assess_with(deaf, tablet_name, &[already_named], 42, 100, true);
        assert_eq!(
    ...
    fn ptr_output_becomes_a_host_label_and_numeric_or_local_answers_name_nothing() {
        assert_eq!(
            clean_resolved_name("Spare\\032iPad.local.\n", "198.51.100.42"),
            Some("Spare iPad".to_string())
        );
        assert_eq!(
            clean_resolved_name("mac.example.net.\n", "198.51.100.3"),
            Some("mac".to_string())
        );
        assert_eq!(
            clean_resolved_name("198.51.100.42.\n", "198.51.100.42"),
            None
        );
