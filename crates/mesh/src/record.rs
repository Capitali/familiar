...
/// before being fully purged (ADR-0026 — the presence lease). Two hours — long
...
///
/// Decided: the clock runs from the last sighting, not the
...
/// Claim and establishment, side by side. "Someone says they're so-and-so" is worth displaying;
...
pub struct Correction {
    ...
    /// "that's not who they said" — the reason travels with the act.
    pub reason: String,
    pub ts: i64,
    pub nonce: String,
...
/// establishment carried a released handle onto a visitor card. The door's own mint paths keep
...
/// a named member a guest wearing its own name on every welcome screen.
...
///   naming it. Typing "I am so-and-so" cannot become them.
...
/// How long until this guest is purged, or `None` for a member/established record or one
...
pub fn guest_purge_in(r: &MembershipRecord, now: i64) -> Option<i64> {
    ...
    // The lease clock: two hours from the LAST evidence of presence. A visitor
...
/// A live sighting renews an existing GUEST record's lease. Deliberately narrow:
...
/// Fully purge visitors who never established an identity and have sat past
/// [`GUEST_PURGE_SECS`]. Clears the record and its admission files and peer row, so nothing
/// about a place the household never chose to know lingers. Members and any established
...
pub fn resolve_node_id(dir: &Path, given: &str) -> Result<String> {
    ...
    match hits.len() {
        ...
        // forgotten by design). The exact full id of a held grant resolves, and the
...
mod tests {
    ...
    /// A mesh with one established device: a household member's iPad.
    fn ctx_with<'a>(
    ...
    fn other_ipad(tag: &str) -> (NodeKey, Vec<EstablishedDeviceRef>) {
        let k = key(tag);
        let id = k.identity();
    ...
    fn e2_a_voucher_from_the_humans_own_device_establishes_the_new_one() {
        let (ipad, est) = other_ipad("e2_ipad");
        let phone = key("e2_phone");
        let v = DeviceVoucher::mint(&ipad, "ada", &phone.identity().pubkey, NOW, "n1").unwrap();
        let ctx = ctx_with(&est, "g", "unused");
        let e = evaluate_admission(
    ...
    fn e2_rejects_a_voucher_for_someone_elses_handle_and_a_forged_one() {
        let (ipad, est) = other_ipad("e2_neg_ipad");
        let phone = key("e2_neg_phone");
        let ctx = ctx_with(&est, "g", "unused");

        // That iPad cannot vouch anyone in as "ian".
        ...
        let mut forged =
            DeviceVoucher::mint(&ipad, "ada", &phone.identity().pubkey, NOW, "n2").unwrap();
    ...
    fn e3_a_named_invite_admits_and_is_single_use() {
        ...
        let t = mint_invite_token(&ian, &m, "dana", NOW).unwrap();
    ...
    fn e3_expires_after_ten_minutes_and_refuses_a_non_member_mint() {
        ...
        let forged = mint_invite_token(&stranger, &m, "dana", NOW).unwrap();
        let ctx = ctx_with(&[], &gid, &gpk);
        assert!(evaluate_admission(
            &subject(&newcomer.node_id()),
            None,
    ...
    fn e3_an_unnamed_invite_establishes_a_new_human_but_never_an_existing_one() {
        ...
        let (_ipad, est) = other_ipad("e3_unnamed_ipad");
        ...
        // Claiming an existing handle through an unnamed invite: refused — attaching to an
        // existing human takes the inviter naming it, or that person's own hardware.
        let t2 = mint_invite_token(&ian, &m, "", NOW).unwrap();
        let other_claim = IdentityClaim {
            handle: "Ada".into(),
            ts: NOW,
        };
        assert!(evaluate_admission(
    ...
    fn e4_an_introduction_on_the_mesh_network_establishes_a_new_human() {
        ...
        let e = evaluate_admission(
            ...
            &Evidence::Introduction {
                intro: Introduction {
                    handle: "dana".into(),
                    statement: "hi, I'm a friend of the owner's".into(),
                    ts: NOW,
                },
    ...
    /// does not admit. Typing an existing handle cannot become its owner.
    #[test]
    fn negative_a_typed_claim_to_an_existing_handle_does_not_admit() {
        let (_ipad, est) = other_ipad("neg1_ipad");
        ...
        let err = evaluate_admission(
            ...
            &Evidence::Introduction {
                intro: Introduction {
                    handle: "Ada".into(), // case-insensitive on purpose
                    statement: "I am Ada".into(),
                    ts: NOW,
                },
                provenance: Provenance::MemberColocatedNetwork,
            },
    ...
    fn no_evidence_admits_a_device_that_never_attested_the_covenant() {
        let (ipad, est) = other_ipad("filter1_ipad");
        let phone = key("filter1_phone");
        let v = DeviceVoucher::mint(&ipad, "ada", &phone.identity().pubkey, NOW, "n").unwrap();
        let ctx = ctx_with(&est, "g", "unused");
    ...
    fn a_same_second_rename_dance_stays_member_everywhere() {
        ...
        // handle riding on a spent establishment — a named member a visitor wearing its name.
        let dir = fresh("dance");
        ...
        name_established(&dir, "aaaa1111", "SecondMac", NOW).unwrap();
    ...
    fn a_membership_act_lands_on_a_record_that_exists_or_not_at_all() {
        ...
        name_established(&dir, "3d68a068", "SecondMac", NOW + 1).unwrap();
        assert_eq!(
        ...
        // record purged (the sweep forgets un-established guests): the enroll store is the
    ...
    fn merge_corrections_union_and_the_latest_deliberate_act_wins() {
        ...
        a.corrections.push(Correction {
            ...
            reason: "that's not who they said".into(),
            ts: NOW + 10,
            nonce: "c1".into(),
            sig: String::new(),
        });
        b.corrections.push(Correction {
            ...
            subject_device: "dev2".into(),
            corrected_by: "ian-mac".into(),
            reason: "it was them after all".into(),
            ts: NOW + 20,
            nonce: "c2".into(),
            sig: String::new(),
        });
    ...
    // ---- The purge collects, and nothing refills it ----
    //
    // The live failure (2026-08-17): seven un-vouched visitors announced as purged
    ...
    // ---- The presence lease — the cause behind that symptom ----

    /// **The invariant (decided): continuous anonymous presence
    ...
    #[test]
    fn presence_continuous_presence_cannot_produce_unbounded_mint_purge_history() {
        let dir = fresh("presence_lease");
    ...
    #[test]
    fn presence_a_sighting_renews_but_never_mints_or_touches_the_established() {
        let dir = fresh("presence_narrow");
    ...
    #[test]
    fn visitor_the_sweep_collects_and_announces_once() {
        let dir = fresh("visitor_sweep");
    ...
    #[cfg(unix)]
    #[test]
    fn visitor_a_sweep_that_did_not_collect_says_nothing() {
        use std::os::unix::fs::PermissionsExt;

        let dir = fresh("visitor_evidence");
    ...
    #[test]
    fn visitor_a_sibling_cannot_resurrect_a_visitor_we_have_already_forgotten() {
        let dir = fresh("visitor_resurrect");
    ...
    #[test]
    fn visitor_an_establishment_still_arrives_for_a_guest_we_already_hold() {
        let dir = fresh("visitor_establish");
    ...
    #[test]
    fn visitor_a_visitor_inside_the_window_still_absorbs() {
        let dir = fresh("visitor_fresh");
    ...
    #[test]
    fn visitor_a_door_does_not_offer_a_visitor_it_owes_the_bin() {
        let dir = fresh("visitor_offer");
