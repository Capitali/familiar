...
//! 2. The familiar records it as **pending** and surfaces it to the human ("Kali-Nomad wants to
...
/// guest purge. Best-effort and idempotent; a missing file is success.
...
mod tests {
    ...
    fn setup(tag: &str) -> (PathBuf, NodeKey) {
        ...
        let joiner = NodeKey::load_or_mint(&fresh(&format!("dev_{tag}")), "Kali-Nomad").unwrap();
        (host, joiner)
    }
