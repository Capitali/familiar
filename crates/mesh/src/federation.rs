...
mod tests {
    ...
    fn the_projection_ladder_shows_strictly_more_per_rung() {
        ...
        // content the owner's ruling protects — a named member with an address and a position,
        // a dialogue body, a standing rule — and prove none of it serializes.
        let mut guest = full.clone();
        if let Some(m) = guest.members.first_mut() {
            m.human = "ian".into();
            m.label = "Greymac".into();
            m.addr = "192.168.1.7:47100".into();
            m.lat = 43.1234;
            m.lon = -89.5678;
        }
        guest.recent.push(crate::worldview::ObsView {
            ...
            object: "the stairwell light is too bright after dark".into(),
        ...
        guest.rules.push(crate::worldview::RuleView {
            id: "rule-0001".into(),
            sentence: "when ian leaves wifi, dim the nightlamps".into(),
            enabled: true,
