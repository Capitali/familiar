...
pub struct TheoryView {
    ...
    /// "" for a theory; "inquiry" for the wondering kind — additive, so a
...
pub struct ArrivalView {
    ...
    /// door saw, so a human can VERIFY who they are welcoming (a new phone in the house should
    /// not be knocking from a Beijing datacenter). Zero/empty when unknown. Projected away for
...
pub struct Worldview {
    ...
    /// unaddressed. A console names who is being asked rather than putting everyone's question
    /// in front of whoever happens to be holding the device. Ownership is about who is ASKED —
    /// the answer, once confirmed, is an ordinary public observation.
    #[serde(default)]
    ...
    /// For a GUEST reader who hasn't identified: seconds until this visitor is purged, so
    ...
    /// True when this view was deliberately masked for its audience: labels are
...
pub(crate) fn read_worldview(
    ...
    // (decided: names display only for devices on the household LAN or owned by the
    // human). Owned = the reading device carries an
    ...
    // Tell a still-unidentified visitor how long until they're forgotten, so the console
...
/// a carrier-grade-NAT uplink — those are different things. The public one is a NAT exit with
/// nothing behind it, so gossiping it hands every device in the mesh an address that can only
/// ever time out.
///
/// This was not theoretical. A mobile uplink's public IP changes whenever it moves, each new one
/// entered the gossip as a fresh "host", nothing ever pruned them, and a single iPad accumulated 33
...
pub fn assemble_worldview(
    ...
    // The drill-down's raw material (the owner, 2026-08-14: "a stacked, hierarchical view of
    ...
    Ok(Worldview {
        ...
        question_owner,
        guests_waiting,
        guest_purge_in_secs: 0, // set per-reader in read_worldview
...
fn frontier_devices(
    ...
    // dropped — a discovered name standing beside the device it names was the mesh
    // discarding a name it had already learned (2026-08-14).
...
/// of the compat act, so legacy `service:_airplay._tcp` rows and unified
/// `service:airplay` rows read as one class — newest first, capped.
///
/// **The instance name never leaves this function** (closed 2026-08-25): legacy rows
...
fn discovered_services(obs: &[familiar_kernel::observation::Observation]) -> Vec<ServiceView> {
    ...
    for o in obs {
        ...
        // (`ble:*` — the class IS the kind, the ingest fence's floor). Both are already
        // class-only at ingestion; this viewer never widens what they may say.
...
mod discovered_services_tests {
    ...
    /// row — class as kind, name empty, everything else refused upstream.
    #[test]
    fn ble_class_rows_surface_beside_bonjour_kinds() {
        ...
        assert!(!serde_json::to_string(&v).unwrap().contains("Owners"));
    }

    /// Both compat acts at the viewer: legacy full-type rows and unified short-kind
    /// rows conflate to ONE class, and no stored instance name — a personal device label — ever
    ...
    fn legacy_rows_conflate_to_one_class_and_no_name_is_served() {
        let obs = vec![
            row("service:_airplay._tcp", "A Person's MacBook Pro", 100),
            row("service:airplay", "", 200),
            row("service:mqtt", "house broker", 150),
        ];
        let v = discovered_services(&obs);
        ...
        assert!(!raw.contains("house broker"));
    }
}
...
mod gossip_addr_tests {
    ...
    /// Roughly thirty were public NAT exits handed out by a carrier-grade-NAT uplink as the device
    /// moved; each one was retried at a 10s timeout before anything real was reached, which is what
    /// a closed eye and a spinning wheel actually were.
    #[test]
    ...
    fn boundaries_that_are_easy_to_get_wrong() {
        ...
        assert!(!is_gossipable_addr("gateway.example.net")); // not an address at all
    }
}
...
mod tests {
    ...
    fn a_discovery_at_a_members_face_names_the_member_instead_of_ghosting_beside_it() {
        // A discovered name once stood on the map beside the very device it names: the reach
        ...
        crate::device::note_network(&dir, "d5c314724525d19c", "lan", "192.168.1.42", NOW - 50)
            .unwrap();
        ...
        let obs = vec![Observation::new(
            ...
            "can-reach",
            "device:tablet",
            "class=observable-only open=- ip=192.168.1.42",
            "reach",
            NOW - 10,
            0.9,
        )];
    ...
    fn an_arrival_appears_in_the_welcome_window_and_is_projected_for_guests() {
        ...
        let stranger = NodeKey::load_or_mint(&fresh("arrivals_stranger"), "Kali-Nomad").unwrap();
    ...
    fn a_member_not_on_the_standing_roll_reads_a_guest_view() {
        ...
        assert_eq!(
            ...
            "timestamps must survive — the owner asked for this"
        );
    ...
    fn asserted_advertise_hosts_lead_the_hosts_list() {
        ...
        std::fs::write(
            ...
            r#"{"advertise_hosts":["lighthouse.example.net","203.0.113.7"]}"#,
        )
        .unwrap();
        let (raw, sig) = signed_request(&cred, &device, NOW, "v1");
        let view = read_worldview(&host, &raw, &sig, NOW, &ring(), "192.168.1.9").unwrap();
        ...
        assert_eq!(&view.hosts[..2], ["lighthouse.example.net", "203.0.113.7"]);
    }
