...
impl NodeKey {
    ...
    pub fn load_or_mint(dir: &Path, label: &str) -> Result<NodeKey> {
        ...
        // (FamTalker01, greymac, …), so a fleet's roster never reads "familiar" seven times.
...
mod tests {
    ...
    fn tmp(tag: &str) -> std::path::PathBuf {
        let p =
            std::env::temp_dir().join(format!("familiar_mesh_node_{}_{tag}", std::process::id()));
