//! **Audience classification for a worldview read** (decided).
//!
//! The owner's ruling (2026-08-20, verbatim): *"no more names visible. They still must be present
...
//! structurally, whatever network they connect from. Source network is context:
...
mod tests {
    ...
    /// The owner's ruling, both halves: an OWNED device keeps the names from any network, and a
    ...
    fn a_declared_lan_matches_and_a_private_range_alone_never_widens() {
        assert!(cidr_match("192.168.1.0/24", "192.168.1.44"));
        assert!(!cidr_match("192.168.1.0/24", "192.168.2.44"));
        ...
        assert!(!cidr_match("192.168.1.0/oops", "192.168.1.44"));
        assert!(!cidr_match("192.168.1.0/40", "192.168.1.44"));
        assert!(!cidr_match("not-a-net/24", "192.168.1.44"));
    }
}
