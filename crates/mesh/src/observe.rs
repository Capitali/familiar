...
/// Swift survey policy by a structural drift test on each side (an external review of the BLE
/// path: the producer being closed is not enough; the authority-bearing ingest seam must
/// enforce the same closure, or a stale/defective signed client can mint `ble:Owners-Watch`
/// into a durable row and a served kind).
...
/// One canonical survey class for both wire forms (the compat act): legacy macOS rows
...
pub(crate) fn ingest_observations(
    ...
    // The discovery gate, enforced at the LAST authority-bearing point (an external review's
    // finding): a client can be stale, offline during revocation, old, or defective without
...
mod tests {
    ...
    // ---- The discovery gate at ingestion ----
    ...
    /// still lands. Emit-side gating is necessary; this is the sufficient half.
    #[test]
    ...
    fn an_open_discovery_gate_lets_survey_rows_land() {
        ...
        let (raw, sig) = signed(
            ...
            vec![
                ...
                // or defective signed client is refused even here (an external review's finding).
                obs("ble:Owners-Watch"),
            ],
        );
        let n = ingest_observations(&host, &raw, &sig, NOW, &ring()).unwrap();
        ...
        assert!(!serde_json::to_string(&stored).unwrap().contains("Owners"));
    }
}
