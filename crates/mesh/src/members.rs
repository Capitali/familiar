...
pub struct Member {
    ...
    /// "MacIntel" and "Mac" apart (the owner's roster sentence). Empty when the node never
...
fn presence_evidence(
    ...
    //    wrong, contaminating the very room observations the shared-lighting consensus
...
fn derive_presence(
    ...
    // Same human is matched case-insensitively (a recognized `face:Sam` and a `phone:sam`
...
pub fn classify(dir: &Path, now: i64) -> Vec<Member> {
    ...
    for p in &peers {
        ...
        // member instead of ghosting beside it (a discovered name standing beside the very
        // device it named, 2026-08-14). A console shares its machine's IP, so the IP-keyed
        // tailnet hostname names the MACHINE — and a console is not its machine: renaming
        // "<machine> console" to "<machine>" erased the very stem attach_consoles pairs on,
        // which is how both Macs came to stand twice on the roster. A console-shaped peer
        ...
        // still fall back to what the device said about itself: a guest iPhone with a
    ...
    for (node, (actor, object, ts)) in &reports {
        ...
        out.push(Member {
            node_id: node.clone(),
            // A friendly label, not the raw actor — "watch:sam" beside the prominent human
            // name read as a stutter ("watch:sam Sam"). The namespace names the hardware;
    ...
    // from the roster on every door their device can't reach. A member is never trapped
    ...
    // |lat| ≥ 85 is the pole convention the console's `anchorable` already honours.
...
fn attach_consoles(out: &mut [Member], self_hosts: &[String]) {
    ...
    for (ci, c) in out.iter().enumerate() {
        ...
        // view the greymac daemon classifies as a DevicePeer (it reports observations there),
        ...
        let host = out
            ...
            .or_else(|| {
                ...
                // it, and matching on it filed SecondMac's console under greymac's daemon
...
/// Display capitalization for a discovered hostname ("tablet" → "Tablet") — display-only,
/// the slug stays the identity, same convention as the console's cap().
/// The SystemName ladder as a pure function: given name, else discovered,
...
/// "<machine> console" to "<machine>" erased the stem attach_consoles pairs on,
...
mod tests {
    ...
    fn a_station_still_learns_who_speaks_to_it() {
        ...
        let (who, _, via) = derive_presence(&obs, NOW, true, |o| o.source == src);
        assert_eq!(who, "ada", "a station must learn the name it is given");
    ...
    fn presence_is_derived_by_strength_freshness_and_run_length() {
        ...
        let obs = vec![
            mk("phone:sam", "reports", "motion:walking", NOW - 20 * 60),
            mk("phone:sam", "reports", "motion:still", NOW - 12 * 60),
            mk("phone:sam", "reports", "presence", NOW - 5 * 60),
            mk("host", "recognized", "face:Sam", NOW - 60),
        ];
    ...
    fn watch_attaches_to_phone_and_summarizes_its_signals() {
        ...
        assert!(d.contains("gps 40.7,-74.0"), "gps in summary: {d}");
    }
    ...
    fn a_mac_console_nests_under_the_machine_it_runs_on() {
        ...
        // Seen from a third node (the lighthouse): greymac's daemon is a gossip peer and its
        // console a device peer, both reporting the same machine address.
        let mut out = vec![
            mk("selflh", "lighthouse", "", "self_node", "localhost"),
            mk("1c99", "greymac", "", "gossip_peer", "192.168.1.10"),
            mk(
                "a24d",
                "Greymac console",
                "mac:ian",
        ...
        attach_consoles(&mut own, &["192.168.1.10".to_string()]);
        assert_eq!(
        ...
        // The live shape on greymac: a console that has only been gossiping (no recent
        ...
        let mut quiet = vec![
            mk("1c99", "TheRiver", "", "self_node", "localhost"),
            mk("a24d", "Greymac console", "", "gossip_peer", "192.168.1.10"),
            mk("aaaa", "Tablet console", "", "gossip_peer", "192.168.1.10"),
        ];
        attach_consoles(&mut quiet, &["192.168.1.10".to_string()]);
        assert_eq!(
    ...
    fn a_console_behind_nat_never_files_under_another_machine() {
        ...
        // SecondMac's console must not nest under greymac's daemon just because the
        ...
        let mut nat = vec![
            mk("selflh", "lighthouse", "", "self_node", "localhost"),
            mk("1c99", "greymac", "", "gossip_peer", "203.0.113.7:47100"),
            mk(
                "77aa",
                "SecondMac console",
                "mac:ian",
        ...
        assert!(
            ...
            "greymac is not badged with a console that isn't its own"
        );

        // And once SecondMac's own daemon is on the roster, the label stem claims the
        // console for the machine it actually runs on — same public address throughout.
        let mut named = vec![
            mk("selflh", "lighthouse", "", "self_node", "localhost"),
            mk("1c99", "greymac", "", "gossip_peer", "203.0.113.7:47100"),
            mk("3d68", "secondmac", "", "gossip_peer", "203.0.113.7:47101"),
            mk(
                "77aa",
                "SecondMac console",
                "mac:ian",
    ...
    fn dedup_collapses_reinstalled_device_identities() {
        ...
        let members = vec![
            mk("self1", "Greymac", "", "self_node", 500, 0), // no device actor — untouched
        ...
        // Greymac + one iPhone + one iPad == 3 rows (the three phone:ian identities collapsed).
...
#[cfg(test)]
mod console_pairing_tests {
    use super::ladder_label;

    /// The root cause, pinned: a console sharing its machine's tailnet IP keeps
    ...
    /// machine's name (the sticky damage self-heals). The owner's explicit given name
    /// still outranks everything.
    #[test]
    fn a_console_is_not_its_machine() {
        // The live failure: tailnet hostname would have renamed the console.
        assert_eq!(
            ladder_label("", "", Some("greymac"), "Greymac console", true),
            "Greymac console"
        );
        // Sticky damage: a record already stamped "greymac" is outranked by self-report.
        assert_eq!(
            ladder_label("", "greymac", Some("greymac"), "Greymac console", true),
            "Greymac console"
        );
        // A daemon (not console-shaped) still takes the discovered/tailnet name.
        assert_eq!(
            ladder_label("", "", Some("greymac"), "Mac.example.net", false),
            "Greymac"
        );
        // The human's explicit word outranks everything, console or not.
        assert_eq!(
            ladder_label("Hall Display", "", Some("greymac"), "Greymac console", true),
            "Hall Display"
        );
    }
}
