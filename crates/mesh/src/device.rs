...
pub struct DeviceRecord {
    ...
    /// The SystemName ("SecondMac", "Tablet") — deliberate, human-given, never invented.
    #[serde(default)]
    pub name: String,
    ...
    /// device-shaped question got a human-shaped answer ("set the name to shared", the owner
    ...
    /// The name DISCOVERY gave this device — its mDNS/tailnet hostname ("tablet"), which
    /// is the name its human gave it on the device itself. Outranked by an explicit
    /// `name`, outranks everything else (the owner, 2026-08-14: autodiscovery should gather
...
/// posture from what it observes but never enters one on its own, because the two
...
/// the member (a discovered name at the LAN ip IS the iPad reading via the lighthouse —
/// the association the map once lost, 2026-08-14). Deduped by address, capped.
...
mod tests {
    ...
    fn a_name_is_given_through_the_membership_space_and_survives_merge() {
        ...
        let rec = set_name(&dir, "d5c31472", "Handset", NOW).unwrap();
        assert_eq!(rec.device_id, "d5c3147245250000");
        assert_eq!(rec.name, "Handset");
    ...
    fn discovered_names_are_cleaned_guarded_and_outranked_by_the_given_name() {
        ...
        set_discovered_name(&dir, "aa11aa11aa11aa11", "Tablet.local.", NOW).unwrap();
        assert_eq!(
        ...
        note_network(&dir, "aa11aa11aa11aa11", "lan", "192.168.1.42", NOW).unwrap();
        note_network(&dir, "aa11aa11aa11aa11", "lan", "192.168.1.42", NOW + 10).unwrap();
    ...
    fn a_device_sync_round_trips_and_refuses_a_stranger() {
        ...
        let cred =
            crate::group::create_group(&dir, &node, "g", NOW, crate::group::DEFAULT_CERT_TTL_SECS)
                .unwrap();
        membership_record(&dir, "aa11aa11aa11aa11");
        set_name(&dir, "aa11aa11", "Tablet", NOW).unwrap();
