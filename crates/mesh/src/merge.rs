...
pub fn build_outbox(
    ...
    let knowledge = if cfg.share_knowledge {
            ...
            obs.iter()
                ...
                .filter_map(|o| {
                    obs_origin(&o.source, self_node).map(|origin| ObsShare {
                        ...
                        // excluded from every federation payload; the stored row stays local,
                        // the class travels.
        ...
        // definitions. Existing goal rows are not merged until signed goal events supply
    ...
    // a need, but a remote answer cannot open it: positive gate reports are refused until a
    // human/device-bound receipt can carry them. Merely asking never writes the boundary.
...
fn merge_one(
    ...
    if cfg.share_observations && !brief.body.knowledge.observations.is_empty() {
        ...
        for s in &brief.body.knowledge.observations {
            ...
            let o = observation::Observation::new(
                ...
                // no NEW derived record inherits it.
    ...
    // progress, human accountability, or terminal state. Until authenticated goal events supply
    // per-field authority, every non-identical report for an existing id is refused
            ...
            Some(local) => {
                ...
                if refused_goal_rewrites.insert(marker.clone()) {
                    record_obs(
                        ...
                        &marker,
                        "a signed member reported different fields for an existing goal; refused until authenticated goal events define per-field authority",
                        now,
                    );
                    report.observations_ingested += 1;
                }
            ...
            .unwrap_or(false)
    {
        ...
        for req in &brief.body.knowledge.theory_requests {
            ...
            // Unkeyed mint: a delegated theory arrives as prose; its id comes
    ...
    // (origin,kind,ref). A boundary is opened only by a human at the node it governs. ---
    ...
    // refusal until a human/device-bound receipt exists. Applied/refused reports are deduped
        ...
        for grant in &brief.body.authority_grants {
            ...
            match apply_authority_grant(dir, node_id, grant, now) {
                ...
                Some(GrantOutcome::Refused {
                    ...
                    // The charter's record: a peer's grant refused at the boundary.
...
/// This function is deliberately not an update path — it is permitted only when the id is absent.
...
mod tests {
    ...
    fn remote_positive_gate_is_refused_stop_narrows_and_answer_is_not_ian() {
        ...
        assert_ne!(
            answer.actor, "ian",
            "a peer cannot assert the owner's identity"
        );
        assert_eq!(
            corruption::score(
    ...
    fn a_headless_peers_authority_needs_surface_at_a_human_facing_peer() {
        ...
        body.authority_requests = vec![crate::brief::AuthorityRequest {
            ...
            summary: "admit node abc123 (kali-dana) to the group?".into(),
        }];
        ...
        assert!(ask.context.contains("kali-dana"));
    ...
    fn a_personal_need_thread_never_federates() {
        ...
        familiar_kernel::thread::append(
            ...
            &familiar_kernel::thread::Thread {
                ...
                theory: "Someone may want softer light after dark.".into(),
    ...
    fn a_face_signature_never_reaches_the_wire_even_when_the_handle_is_opted_in() {
        ...
        identity::remember(&dir, "Ada", NOW).unwrap();
        identity::link_face(&dir, "ada", Some(vec![0.123_456, -0.987_654, 0.5])).unwrap();
        ...
        let identities = brief
            .body
            .identities
            .expect("ada was opted in and should be shared");
        assert_eq!(identities.entries.len(), 1);
        assert_eq!(identities.entries[0].handle, "ada");
    ...
    fn sensitive_personal_signals_never_reach_the_wire() {
        ...
        rec("watch:ada", "heart_rate:elevated", "mesh:watchnode");
        rec("watch:ada", "location:51.501,-0.141", "mesh:watchnode");
        rec("watch:ada", "gyro:turning", "mesh:watchnode");
        // Ordinary, non-sensitive activity from the same person's device — this SHOULD federate.
        rec("phone:ada", "service:mqtt", "mesh:phonenode");
