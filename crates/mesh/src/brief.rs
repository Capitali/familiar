...
pub struct Knowledge {
    ...
    /// existing ids are informational only until authenticated goal events define field
    /// authority. `#[serde(default)]` for back-compat with briefs that predate the field.
    #[serde(default)]
...
/// transitions, and terminals require authenticated goal-event authority. Deduped by `id` (goal
...
pub struct GoalShare {
    ...
    /// definition, but cannot rewrite an existing goal without that authority. Skipped when empty
...
/// widening waits for a human/device-bound receipt.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// grants are refused until a human/device-bound receipt exists.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
mod tests {
    ...
    fn tmp(tag: &str) -> std::path::PathBuf {
        let p =
            std::env::temp_dir().join(format!("familiar_mesh_brief_{}_{tag}", std::process::id()));
    ...
    #[test]
    // The "excessive" precision IS the test: a coordinate pair whose shortest-round-trip
    // form needs all seventeen significant digits, which is exactly what the default float
    // parser re-read 1 ULP off. Truncate them and the regression stops guarding anything.
    #[allow(clippy::excessive_precision)]
    fn a_brief_carrying_hostile_floats_still_verifies_after_the_wire() {
        // The 2026-08-13 outage, pinned: a Mac's IP-geolocated longitude, printed
        ...
        // A pair of the same shape as the live geo.json that broke the mesh: seventeen
        // significant digits, so the shortest printed form is the hostile one.
        body.capability.lat = 51.682844458456294;
        body.capability.lon = -0.2019009805748228;
