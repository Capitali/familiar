...
async fn heartbeat_status(dir: &Path, cfg: &MeshConfig, cred: &crate::group::GroupCredential) {
    ...
    let status = crate::status::MemberStatus {
        ...
        // true place on every other door — this is what puts a daemon where it actually
    ...
    for st in statuses {
        ...
        if peers.iter().all(|p| p.node_id != st.node_id) {
            ...
            peers.push(PeerRecord {
                ...
                // The relayed GPS fix, so a device met only through the lighthouse
        ...
        if let Some(p) = peers.iter_mut().find(|p| p.node_id == st.node_id) {
            ...
            // lighthouse relays its GPS here so it maps at its true place. A locally
        ...
        Err(crate::Error::Untrusted(m)) => {
            ...
            if let Some(claim) = req.claim.as_ref().filter(|c| !c.handle.trim().is_empty()) {
                ...
                if existing {
                    ...
                    // The claim has always been recorded here; until now it waited
...
/// `GET /mesh/threads` → this door's theory offer, record-sync's third twin.
...
/// `POST /mesh/thread-sync` → absorb a sibling door's theories.
///
...
fn status_directory_response(dir: &Path) -> Response<Full<Bytes>> {
    ...
    for p in load_peers(dir) {
        ...
        out.push(crate::status::MemberStatus {
            ...
            tailnet_addr: String::new(),
            tailnet_up: false,
            // Relay the peer's known fix onward, so a third door learns where it is too.
            lat: p.lat,
            lon: p.lon,
...
async fn exchange_with(dir: &Path, addr: &str, our_brief: &[u8]) -> Result<()> {
    ...
    // Theories ride it too — so a retirement reaches the fleet instead of meaning
...
fn upsert_peer(dir: &Path, brief: &MeshBrief, addr: &str) -> Result<()> {
    // A verified brief is a live sighting — it renews a guest record's lease so the
...
mod tests {
    ...
    fn local_answer_speaks_as_the_current_identity_not_a_hardcoded_name() {
        ...
        familiar_kernel::identity::remember(&dir, "Ada", 100).unwrap();
        familiar_kernel::identity::set_current(&dir, "ada").unwrap();
        familiar_kernel::thread::append(
            &dir,
            &familiar_kernel::thread::Thread {
                id: "thread-0001".into(),
                question: "Long evenings — worth watching?".into(),
                theory: "Someone may want softer light.".into(),
        ...
        assert_eq!(
            t.origin, "observer",
            "the speaker's own console answer confirms the need"
        );
        assert_eq!(t.actor, "ada");
        let obs = familiar_kernel::observation::load(&dir).unwrap();
        assert!(
            obs.iter()
                .any(|o| o.actor == "ada" && o.action == "answered"),
    ...
    fn door_and_guest(tag: &str) -> (std::path::PathBuf, NodeKey, group::GroupCredential, NodeKey) {
        ...
        let guest =
            NodeKey::load_or_mint(&fresh_dir(&format!("door_{tag}_guest")), "Kali-Nomad").unwrap();
    ...
    fn a_guests_fix_never_locates_the_mesh_a_members_does() {
        ...
        .unwrap();
        assert_eq!(freshest_device_fix(&dir), Some((40.7, -74.0)));
    }
    ...
    fn an_invite_token_completes_admission_from_anywhere_and_is_single_use() {
        ...
        let token =
            crate::record::mint_invite_token(&host, &cred.membership, "dana", now_secs()).unwrap();
        ...
        assert_eq!(rec.identity.established.as_ref().unwrap().handle, "dana");
        assert_eq!(
    ...
    fn an_introduction_needs_the_meshes_own_ground_and_cannot_take_an_existing_name() {
        ...
        let intro = crate::record::Evidence::Introduction {
            intro: crate::record::Introduction {
                handle: "dana".into(),
                statement: "hi, I'm new here".into(),
                ts: now_secs(),
            },
    ...
    fn local_observe_recognized_face_reaches_the_identity_registry() {
        ...
        let body = br#"{"actor":"host","action":"recognized","object":"face:Ada"}"#;
        assert_eq!(body_status(&local_observe(&dir, body)), StatusCode::OK);
        assert_eq!(
    ...
    fn the_status_directory_teaches_us_members_we_have_never_met() {
        ...
        let statuses = vec![
            ...
            mk(&node.node_id(), &ours, "Greymac", "ian", 1_785_100_000),
        ];
    ...
    fn parses_tailscale_status_fixture() {
        let json = r#"{
          "Self": {"HostName":"greymac","TailscaleIPs":["100.64.0.10"],"Online":true},
          "Peer": {
            "keyA": {"HostName":"beta","TailscaleIPs":["100.64.0.96","fd7a:1::1"],"Online":true},
            "keyB": {"HostName":"alpha","TailscaleIPs":["100.64.0.99"],"Online":false},
        ...
        assert_eq!(peers[0].host, "alpha");
        assert!(!peers[0].online);
        assert_eq!(peers[1].host, "beta");
        assert_eq!(peers[1].ip, "100.64.0.96"); // first IPv4, not the v6
