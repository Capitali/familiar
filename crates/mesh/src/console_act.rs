...
mod tests {
    ...
    fn another_established_human_gets_no_rows_and_cannot_decide() {
        let (dir, _owner_node, cred) = member("partner-other-human");
        let (request_id, _) = partner_request(&dir, "ian");
        let other_dir = fresh("partner-other-key");
        let ada = NodeKey::load_or_mint(&other_dir, "ada phone").unwrap();
        let membership = cred
            .mint_membership(
                &ada.node_id(),
                &ada.identity().pubkey,
            ...
            .unwrap();
        standing::grant(&dir, &ada.node_id(), "full but another human").unwrap();
        establish(&dir, &ada, "ada");
        let guard = Mutex::new(IngestGuard::default());
        let (read_raw, read_sig) = signed_inbox(&ada, membership.clone(), "ada-read-1", NOW);
