...
/// The per-door scope salt: random, minted once, never on the wire. Scope tokens
...
pub fn to_guest_view(dir: &std::path::Path, view: &mut Worldview, reader_node_id: &str) {
    ...
    // struct literal (decided): a field added to `Worldview`
    ...
    *view = Worldview {
        ...
        // Open question carried to the design dialogue: whether the boundary's
...
mod tests {
    ...
    fn actor_keeps_its_kind_and_loses_its_human() {
        ...
        assert_eq!(anon_actor("watch:ada"), "watch:someone");
