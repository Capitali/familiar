...
pub struct MeshConfig {
    ...
    /// The household's declared LAN: explicit IPv4 CIDRs (`"192.168.1.0/24"`) or
...
mod tests {
    ...
    fn tmp(tag: &str) -> std::path::PathBuf {
        let p =
            std::env::temp_dir().join(format!("familiar_mesh_config_{}_{tag}", std::process::id()));
    ...
    fn missing_file_is_safe_default() {
        ...
        assert!(!c.identity_opted_in("ada", "river"));
    ...
    fn partial_file_fills_defaults_and_optin_is_scoped() {
        ...
        fs::write(
            ...
            r#"{"share_identities":true,"identity_optin":[{"handle":"ada","group":"river"}]}"#,
        )
        .unwrap();
        let c = load(&dir).unwrap();
        assert_eq!(c.gossip_interval_secs, 30); // default filled
        assert!(c.identity_opted_in("ada", "river"));
        assert!(!c.identity_opted_in("ada", "other"), "opt-in is per-group");
