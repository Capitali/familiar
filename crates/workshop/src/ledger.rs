...
//! were.
//!
//! Six authority boundaries this module holds closed (named by two external
//! reviews, 2026-08-28):
...
fn apply(state: &mut OrderState, ev: &LedgerEvent) -> Result<(), LedgerError> {
    ...
    match &ev.kind {
        ...
        EventKind::GenerationReturned {
            ...
            // could be commissioned on generation N's declaration (an external
            // review found this). Each candidate must be proposed and
            // declared afresh.
            state.proposed = None;
...
/// which a live holder is unidentifiable and could be stolen from (found by
/// an external review). The lock file is never deleted: unlinking a
...
impl Ledger {
    ...
    fn write_validated(
        ...
        // merely flush into the kernel (an external review found this).
