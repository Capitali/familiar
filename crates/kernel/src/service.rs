...
//! trials port in later), it measures **served-facing attention**: how much
//! of what the factory observes concerns the humans it exists to serve. That is
//! the cold-start *proxy* for service, not service rendered; later work folds in
...
/// Resolving proper names (a person's given name -> person) waits for entity tagging (the
...
mod tests {
    ...
    fn sensitive_personal_matches_health_position_biometric() {
        ...
        assert!(is_sensitive_personal(&obs("watch:ada", "gyro:turning")));
    ...
    fn classifier_matches_markers_not_bare_names() {
        ...
        assert!(!names_served("ada")); // proper names need entity resolution (deferred)
    }
