...
/// A stable, lowercase handle derived from a name (`"Ada Jo"` -> `"ada-jo"`). Falls back
...
mod tests {
    ...
    impl Temp {
        fn new(t: &str) -> Self {
            let p = std::env::temp_dir()
                .join(format!("familiar_identity_test_{}_{t}", std::process::id()));
    ...
    fn a_recognized_face_observation_learns_and_becomes_current() {
        ...
        let learned = maybe_learn_from_observation(&t.0, "recognized", "face:Ada", 100)
            .unwrap()
            .expect("a recognized face: observation should learn someone");
        assert_eq!(learned.handle, "ada");
        assert_eq!(current(&t.0).as_deref(), Some("ada"));

        // Seeing the same person again bumps interactions rather than duplicating.
        maybe_learn_from_observation(&t.0, "recognized", "face:Ada", 200).unwrap();
        assert_eq!(load(&t.0).unwrap().len(), 1);
        assert_eq!(find(&load(&t.0).unwrap(), "ada").unwrap().interactions, 2);
    }
    ...
    fn unrelated_observations_are_a_no_op() {
        ...
        // Wrong action.
        assert!(
            maybe_learn_from_observation(&t.0, "reports", "face:Ada", 100)
                .unwrap()
                .is_none()
        );
        // Wrong object shape.
