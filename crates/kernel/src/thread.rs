...
pub struct Thread {
    ...
    /// pursued (corruption awareness). `"familiar"` for its own theories; a human's
    /// id for observer answers. Empty = unattributed (always
    /// pursued). `#[serde(default)]` so older threads still load.
    #[serde(default)]
    pub actor: String,
    /// Citations this theory stands on: observation ids ("obs-0042") or loop
    ...
    /// 0 = predates the floor. A changed fact supersedes a revision; it
    /// never silently reinterprets old threads.
    #[serde(default)]
    pub facts_rev: u32,
    /// Digest of the DECLARATION the deployment facts were derived from at admission.
    /// Empty on rows predating it. A thread admitted against surfaces the human
    /// has since changed is detectable rather than quietly stale.
    #[serde(default)]
    pub facts_digest: String,
    /// Row schema version. 0 = legacy row written before versioning existed;
    /// rows minted through [`mint`] carry [`THREAD_VERSION`].
    #[serde(default)]
    pub v: u32,
    /// Typed identity, part 1: what part of the world this is
    ...
    /// Set when this thread was folded into a survivor (the conservative
    /// migration): the tombstone stays, append-retained, pointing home.
    #[serde(default)]
    pub superseded_by: String,
    /// "" | "inquiry": an Inquiry is a DIFFERENT KIND, not a
    ...
    #[serde(default)]
    pub expires_at: i64,
    /// The typed both-edges policy this theory proposes: carried until the
...
/// How long an unrenewed Inquiry stands before it expires (seven days).
...
/// What a centralized mint needs to know. The typed identity travels in the
...
pub struct Mint {
    ...
    pub anchors: Vec<String>,
    pub facts_rev: u32,
    /// Declaration digest seen at admission.
    pub facts_digest: String,
    ...
    pub acts: Vec<String>,
    /// "" for a theory; "inquiry" mints the Inquiry kind (with `expires_at` set by the caller).
    pub kind: String,
    /// Unix secs an Inquiry expires (0 = never; theories always 0).
    pub expires_at: i64,
    pub predictions_sig: Vec<String>,
    /// Typed both-edges policy the theory carries toward assent.
    pub rule_proposal: Option<crate::reaction_rule::RuleProposal>,
}

/// What the mint decided: a brand-new thought, a competing alternative
...
/// not hashes — identity must be auditable (an external review).
...
/// THE one way a thread is born: every minter routes here, so identity is
...
pub fn mint(dir: &Path, m: Mint, now: i64) -> io::Result<Disposition> {
    ...
    if !var.is_empty() {
        ...
        // Promotion: a standing INQUIRY holds the same claim minus the
...
/// the count derives from evidence, not an unexplained integer (an external review).
...
/// The conservative fold (the migration path): members become
...
/// Retire the theories minted before the honest engine. A thread with no
...
///
/// This is the whole safety property of federation. Nodes reach terminal verdicts
...
/// The owner saw exactly that when a restarted peer republished a theory from the day before.
...
pub fn is_mature(t: &Thread) -> bool {
    ...
    // Wondering drill-down: the mind stays inspectable without pricing human
    // attention at zero.
    if t.kind == "inquiry" {
        return false;
    }
...
pub fn add_answer(dir: &Path, id: &str, text: &str, now: i64) -> io::Result<bool> {
    ...
    if matches!(
        ...
        // exactly as a human answer outranks the factory's earlier triage.
        t.status = "open".into();
        t.status_at = now;
    }
    let wrote = store::update_by_id(dir, THREADS_FILE, id, &t)?;
    // The answer reaches the QUESTION REGISTRY too, by durable id — the registry
...
mod tests {
    ...
    fn need_thread(id: &str) -> Thread {
        Thread {
            id: id.into(),
            question: "Would warmer evening light help?".into(),
            theory: "Someone here may want softer light after dark.".into(),
    ...
    fn a_confirm_answer_from_the_subject_makes_the_need_stated() {
        ...
        assert_eq!(t.actor, "ada", "the need now belongs to its human");
    ...
    // Federation must never argue with the past. Each test is a way the mesh could
