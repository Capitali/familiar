//! Who bears the consequence of an act.
//!
//! Decided across a whole-system review dialogue, with a shared-lighting case as the worked
//! example: one household's light is shared by two people and two dogs. **Affected is a
//! RELATION, not a standing** (an external review): `person`, mesh
...
//! it may never be narrowed. The dogs are subjects who live with the light's
...
mod tests {
    ...
    /// The shared-lighting household, exactly as the owner described it: one person assents,
    /// another says nothing, and two residents cannot use a console at all. One yes does not
    /// carry the household — the act takes the smaller experiment (invariants 1, 4, 5).
    #[test]
    fn one_yes_does_not_carry_a_household() {
        let set = vec![
            person("ian", Channel::DirectStatement, Response::Favorable),
            person("ada", Channel::Inference, Response::Unknown),
        ...
        assert_eq!(
            disposition(&set, Some(&owner_yes()), 0.5),
    ...
    fn a_being_that_cannot_speak_can_still_refuse() {
        let set = vec![
            person("ian", Channel::DirectStatement, Response::Favorable),
            person("ada", Channel::DirectStatement, Response::Favorable),
    ...
    fn authority_never_erases_exposure() {
        let set = vec![
            person("ian", Channel::DirectStatement, Response::Favorable),
            person("ada", Channel::DirectStatement, Response::Adverse),
        ];
        assert_eq!(
            disposition(&set, Some(&owner_yes()), 0.5),
            Disposition::NarrowOrRevert,
