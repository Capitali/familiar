...
//! only ever *narrow* it), these are **shared**. The owner adjusts them from the Glass; the
...
//! "the owner can't set what the familiar can't put back." `last_set_by` records who set them
...
// as serving. The owner may set anything *inside* it freely; a value *outside* it the familiar
...
// Law II). This is the mechanism of "the owner can't set what the familiar can't put back."
...
pub struct Parameters {
    ...
    /// Settlement grace for predictions whose own grace is 0 (grace rides
...
impl Parameters {
    ...
    /// familiar would make — empty when the owner's choices all serve. The familiar accepts any
...
mod tests {
    ...
    impl Temp {
        fn new(t: &str) -> Self {
            let p = std::env::temp_dir()
                .join(format!("familiar_params_test_{}_{t}", std::process::id()));
    ...
    fn review_accepts_choices_within_the_envelope() {
        ...
        assert!(
            reverts.is_empty(),
            "in-envelope choices are the human's to make"
        );
