//! **The typed answering act** — what the familiar says to a person, as a
//! shape the kernel admits rather than prose the kernel hopes about.
//!
//! An earlier step put the constitution in front of the model, which made the Asimov recital
...
pub struct ReplyDraft {
    ...
    /// One question back, or empty. (Stakes arrive with the question registry.)
    #[serde(default)]
    pub ask: String,
...
/// recital happened; the fix is the one applied everywhere else, so the words come from
...
mod tests {
    ...
    /// (the owner's word, 2026-08-20: regressions, no detector).
