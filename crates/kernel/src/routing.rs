...
//!   question is addressed; an answer is common property. (Decided 2026-07-29.)
...
/// `"familiar"`. Own speech is never evidence: it
...
mod tests {
    ...
    fn present_humans_ranks_by_evidence_then_freshness() {
        let obs = vec![
            ob("watch:ada", "reports", "presence", "device", NOW - 60),
            ob(
                "phone:ian",
                "told the familiar",
                "hello",
    ...
    fn a_recognised_face_names_its_subject_not_the_device() {
        ...
        assert_eq!(
            p[0].handle, "ada",
            "the face names ada, not the iPad's actor"
        );
    ...
    fn an_owner_who_is_here_keeps_the_question() {
        ...
        // The other is the more surely-present, but the question was put to this owner and
        // they are here.
    ...
    fn an_absent_owner_hands_off_to_whoever_the_question_serves() {
        ...
        // The owner is gone; the question exists for another person's need, so it goes to that
        // person — not to a third merely for being the most confidently present.
        assert_eq!(route("ian", "ada", &present), "ada");
        assert!(owner_is_absent("ian", &present));
        assert!(!owner_is_absent("ada", &present));
    }
    ...
    fn is_substrate_knows_the_machine_from_the_person() {
        ...
        assert!(!is_substrate("ian") && !is_substrate("ada"));
    }
