...
mod tests {
    ...
    fn fold_accumulates_by_hour_with_the_evidence_ladders_weights() {
        ...
        let t19 = 19 * 3600;
        observation::record(&p, obs("phone:ada", "told the familiar", "hello", t19)).unwrap();
        observation::record(&p, obs("watch:ada", "reports", "presence", t19 + 60)).unwrap();
        fold(&p, HL).unwrap();
        let c: Contribution = store::load_by_id(&p, DOSSIER_FILE, "ctb|ada|presence|h19")
            .unwrap()
            .unwrap();
        assert_eq!(c.count, 2);
        assert!(
        ...
        let s: Contribution = store::load_by_id(&p, DOSSIER_FILE, "ctb|ada|standing|dialogue")
            .unwrap()
            .unwrap();
    ...
    fn decay_halves_a_slot_after_one_half_life_and_fades_on_read() {
        ...
        let t = 8 * 3600;
        observation::record(&p, obs("phone:ada", "told the familiar", "hi", t)).unwrap();
        fold(&p, HL).unwrap();
        let d = read(&p, "ada", t + HL, HL).unwrap();
        let h8 = d.presence_hours.iter().find(|s| s.slot == "h08").unwrap();
        // Share is normalized (only slot → 1.0), but the raw weight has halved:
        let c: Contribution = store::load_by_id(&p, DOSSIER_FILE, "ctb|ada|presence|h08")
            .unwrap()
            .unwrap();
        assert!(
    ...
    fn the_cursor_resumes_without_double_counting() {
        ...
        observation::record(&p, obs("phone:ada", "told the familiar", "hi", 100)).unwrap();
        fold(&p, HL).unwrap();
        fold(&p, HL).unwrap(); // nothing new — must not re-fold
        let c: Contribution = store::load_by_id(&p, DOSSIER_FILE, "ctb|ada|presence|h00")
            .unwrap()
            .unwrap();
    ...
    fn observer_familiar_and_mesh_actors_feed_no_dossier() {
        ...
        let mut peer = obs("phone:ada", "told the familiar", "hi", 100);
    ...
    fn thin_history_presents_humbly() {
        ...
        for i in 0..3 {
            observation::record(&p, obs("phone:ada", "told the familiar", "hi", 100 + i)).unwrap();
        }
        fold(&p, HL).unwrap();
        let d = read(&p, "ada", 200, HL).unwrap();
    ...
    fn withdrawal_reports_what_went_and_survives_a_refold() {
        ...
        observation::record(&p, obs("phone:ada", "told the familiar", "hi", 100)).unwrap();
        observation::record(&p, obs("watch:ada", "reports", "presence", 200)).unwrap();
        fold(&p, HL).unwrap();
        let receipt = withdraw(&p, "ada", 300).unwrap();
        assert!(
        ...
        fold(&p, HL).unwrap();
        let after: Vec<Contribution> = store::load_prefix(&p, DOSSIER_FILE, "ctb|ada|").unwrap();
        assert!(
            after.is_empty(),
            "the tombstone outranks the log: {after:?}"
        );
        assert!(read(&p, "ada", 400, HL).unwrap().withdrawn);
        let _ = fs::remove_dir_all(&p);
    }
    ...
    fn a_coarse_summary_speaks_in_day_parts_not_distributions() {
        ...
        for i in 0..6 {
            observation::record(
                &p,
                obs("phone:ada", "told the familiar", "hi", evening + i * 86_400),
            )
            .unwrap();
        }
        fold(&p, HL).unwrap();
        let d = read(&p, "ada", evening + 6 * 86_400, HL).unwrap();
