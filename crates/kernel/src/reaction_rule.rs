...
pub struct ReactionRule {
    ...
    #[serde(default)]
    pub last_fired: i64,
    /// Pairs the two edges of one consent: "dim on away" without
...
/// The typed proposal a theory carries toward assent: both edges of a
/// presence-bound policy, actions LITERAL members of the declared surface. Bound to
/// the person's presence judgment, never to today's sensor (an external review).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
pub fn mint(
    ...
    let rule = ReactionRule {
        ...
        // second (a policy pair) must never share an id.
        id: format!(
            "{:08x}",
            (now as u64)
...
pub fn disable_reverted(dir: &Path, id: &str, now: i64) -> io::Result<Option<String>> {
    ...
    // A policy is ONE consent: reverting either edge takes down its pair —
...
mod policy_tests {
    ...
    fn one_standing_policy_per_surface_until_calibration() {
        ...
        let mut ada = proposal();
        ada.subject = "ada".into();
        let e = mint_policy(&p, &ada, "thread:thread-0002", 200).unwrap_err();
