...
/// It is derived, deterministic, and — per an external review —
/// **factual, not editorial**: the familiar's own recent settled
...
/// abstention — the failure where a theory that predicts nothing
...
/// Rows written since polarity grouping landed additionally contribute a bounded,
...
mod tests {
    ...
    fn feedback_digest_never_editorializes() {
        ...
        // (that rewards abstention). Facts only.
        let now = 100_000;
    ...
    fn dir(tag: &str) -> std::path::PathBuf {
        // Unique per process AND tag — learned the hard way: fixed names collide across
    ...
    fn an_anchored_claim_confirms_on_arrival_and_misses_after_grace() {
        ...
        let obs = vec![
            ...
            o("c1", "hub", "reported", "lighting:dim", NOW + 120),
        ];
        ...
        assert_eq!(written[0].predicted_class, "hub|reported");
        ...
        assert_eq!(final_pass[0].predicted_class, "hub|reported");
