...
/// (`obs_class`): one classing contract for every lens, versioned so a
...
mod tests {
    ...
    fn triple_recovers_from_description() {
        let loops = detect(&[
            obs("o1", "ada", "asks_for", "digest", 1),
            obs("o2", "ada", "asks_for", "digest", 2),
        ]);
        assert_eq!(
            loop_triple(&loops[0]),
            Some(("ada".into(), "asks_for".into(), "digest".into()))
        );
    }
