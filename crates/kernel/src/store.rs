...
mod tests {
    ...
    fn load_prefix_is_a_range_not_a_scan() {
        ...
        let f = "acc.jsonl";
        for id in [
            "ctb|ada|presence|0002",
            "ctb|ada|presence|0001",
            "ctb|ian|presence|0001",
        ] {
            append(d.path(), f, &rec(id, "x")).unwrap();
        }
        let ada: Vec<Rec> = load_prefix(d.path(), f, "ctb|ada|").unwrap();
        assert_eq!(ada.len(), 2, "only ada's rows");
        assert_eq!(
            ada[0].id, "ctb|ada|presence|0001",
            "id order, not insertion order"
        );
        assert_eq!(ada[1].id, "ctb|ada|presence|0002");
    ...
    fn delete_by_id_and_prefix_report_what_went() {
        ...
        let f = "acc.jsonl";
        for id in ["ctb|ada|a", "ctb|ada|b", "ctb|ian|a"] {
            append(d.path(), f, &rec(id, "x")).unwrap();
        }
        ...
        assert_eq!(
            delete_prefix(d.path(), f, "ctb|ada|").unwrap(),
            2,
