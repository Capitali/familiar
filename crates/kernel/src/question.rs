...
/// What may turn on an answer (ADR-0040 D2). There is deliberately no
...
/// already appears in the question. The mechanical floor of the anti-vacuity rule: four
/// populated strings can still encode no real dependency, so a field that merely
/// re-treads the question refuses.
...
pub struct Question {
    ...
    /// Why the question arose. Empty only on rows minted before stakes existed.
    #[serde(default)]
    pub because: String,
    /// The decision or belief that awaits the answer.
    #[serde(default)]
    ...
    #[serde(default)]
    pub stake: String,
    /// Retired by explicit policy (its subject stopped existing, or its class was
...
/// Mark answered every registry question bound to `thread_id` — the durable-id
...
/// The conservative backfill: close every unanswered thread-bound question whose
...
/// thread binding are untouched — their retirement is explicit policy, never an
/// invented answer.
...
/// Retire a question by explicit policy, with the reason kept beside its history.
...
mod tests {
    ...
    fn a_thread_answer_closes_its_registry_question_by_durable_id_only() {
        ...
        admit_addressed(&t.0, &d1, "need", "ada", "thread-0001", 10)
            .unwrap()
            .unwrap();
        let d2 = AskDraft {
        ...
        assert!(!by_text("Long evenings — worth watching?").answered);
    ...
    fn rows_minted_before_stakes_existed_still_load() {
        ...
        // A row minted before stakes existed, byte-for-byte without the stake fields.
        std::fs::write(
