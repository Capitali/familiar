//! **MachineryFinding** (ADR-0043 §5) — a theory about the familiar's own machinery,
//! given a typed addressee instead of a dead end.
//!
//! The lesson it encodes: the reasoning engine produced a *correct causal
...
/// finding instead of minting a duplicate per consult (the theory-churn lesson).
    ...
    store::append(
        ...
        &MachineryFinding {
            ...
            uncertainty: "authored by the reasoning engine; subject and causality unverified \
                          (it has misattributed subjects before)"
                .to_string(),
...
mod tests {
    ...
    fn terminal_transitions_are_human_acts_only_and_final() {
        ...
        decide(&t.0, &id, "dismissed", "Ada", 200).unwrap().unwrap();
        let f = &load(&t.0).unwrap()[0];
        assert_eq!(f.disposition, "dismissed");
        assert_eq!(f.decided_by, "ada");
        assert!(
