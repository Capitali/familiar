//! The knowledge floor (decided 2026-08-15): the system's designed facts,
...
//! theorize reply must parse into: the model proposes
...
///
/// rev2 (2026-08-17): the Three Laws joined the registry. Audited before bumping,
...
/// an unknown mechanism is not waved through because no keyword matched.
...
/// The strict admission contract for an LLM theorize reply.
...
pub struct TheoryDraft {
    #[serde(default)]
    pub v: u32,
    /// Citations chosen ONLY from the eligible set the system enumerated —
    ...
    pub question: String,
    /// The question's stakes: why it arose, what awaits the answer, and
    ...
    /// The typed both-edges policy this theory proposes — validated against
...
/// typed enforcement (a named v1 gap): the daemon cannot type what the
...
/// Which KIND of truth a fact is. The categories are
...
/// THE runtime source of system truth. Every consumer — the theorize prompt,
...
pub fn view(dir: &Path) -> io::Result<FactsView> {
    ...
    // rather than of every prompt's string concatenation. Until this line existed,
...
/// The bounded rendering the REQUEST-ANSWERING path receives. Same registry,
...
mod system_facts_tests {
    ...
    /// **The constitution is in the registry, and it comes first.** The module that
    ...
    /// path can no longer hold different pictures of what is true here.
    #[test]
