//! `familiar export` — the household's whole record in open files, with a manifest (T-249).
//!
...
//! familiar — the UCF exchange key, the fleet-serve bearer token, the mesh node key, the
//! group secret, TLS material, push tokens — is withheld and named in the manifest with
//! the reason, so the reader can see what is missing rather than wonder.
...
pub(crate) struct FileEntry {
    /// Path relative to the export directory (`data/…`, `worlds/…`, `captains/…`).
    pub path: String,
    pub bytes: u64,
    pub sha256: String,
}
...
pub(crate) fn cmd_export(args: &[String]) -> ExitCode {
    ...
    let worlds = crate::world_store_root(&dir, f.get("store-root").map(String::as_str));
    let captains = worlds.parent().unwrap_or(&worlds).join("captains");
    let out = f
        .get("out")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
    ...
    let now = unix_now();
    match run_export(&dir, &worlds, &captains, &out, now) {
        Ok(r) => {
            println!(
...
/// Build `<out>/familiar-export-<YYYY-MM-DD>/` from the three roots. `now` is unix seconds
...
pub(crate) fn run_export(
    ...
    // The export must never fold itself in (an `--out` inside worlds/ or the data dir).
    ...
    if worlds_root.is_dir() {
        w.tree(worlds_root, Path::new("worlds"))?;
    }
    if captains_root.is_dir() {
        w.tree(captains_root, Path::new("captains"))?;
    }
...
/// in the export (`data/mesh/node_key`, `worlds/<id>/ucf.env`, …). The rules are by name
...
pub(crate) fn withhold_reason(rel: &Path) -> Option<&'static str> {
    ...
    if name == "ucf.env" {
        return Some("UCF exchange API key — lets the holder trade and fly as this captain");
    }
    if name == "fleet-serve.token" {
        return Some("fleet-serve bearer token — lets the holder drive this familiar's fleet");
    }
    if name == "daemon.log" {
...
fn readme(withheld: &[Withheld]) -> String {
    ...
    s.push_str("- `worlds/` — one directory per ship world (ADR-0045): journals, captain, orders, cargo,\n");
    s.push_str("  the world's own store. Copied whole, less secrets.\n");
    s.push_str(
        "- `captains/` — the captains' stores and the fleet's name ledger (`names.jsonl`).\n",
    );
    s.push_str(
    ...
    s.push_str("(trade on its exchange key, drive its fleet, speak as its mesh node) is left behind and named\n");
    s.push_str("in the manifest. A new home mints its own keys. Withheld in this export:\n\n");
    if reasons.is_empty() {
    ...
    s.push_str("Put `data/`, `worlds/` and `captains/` side by side (they already are, here) and start the\n");
    s.push_str("new familiar with `--data-dir <this dir>/data`; it finds `worlds/` and `captains/` as the\n");
    s.push_str("siblings of the data dir, exactly as the original did. Then: `familiar mesh create-group`\n");
    s.push_str("or `mesh join` for a fresh node key, a new `fleet-serve.token` on first `fleet serve`, and\n");
    s.push_str("your own `ucf.env` in each world that flies. The record — every observation, thread, journal\n");
    s.push_str("and name — carries over as it was.\n");
    s
}
...
mod tests {
    ...
    fn export_copies_the_record_and_withholds_secrets() {
        ...
        let worlds = base.join("worlds");
        let captains = base.join("captains");

        let boundary = r#"{"allow_llm":false}"#;
        let obs =
            "{\"actor\":\"ian\",\"action\":\"said\"}\n{\"actor\":\"ian\",\"action\":\"left\"}\n";
        let node = r#"{"node_id":"node-abc123","label":"wildhorse"}"#;
        let journal = "{\"t\":1,\"line\":\"undocked\"}\n";
        let captain = r#"{"name":"Luke Skywhisker"}"#;
        let names = "{\"name\":\"Luke Skywhisker\",\"kind\":\"captain\"}\n";
        ...
        write(&data.join("fleet-serve.token"), "SECRET-TOKEN");
        write(&data.join("daemon.log"), "noise\n");
        let world = worlds.join("world-abc");
        write(&world.join("ucf.env"), "UCF_API_KEY=SECRET\n");
        write(&world.join("journal.jsonl"), journal);
        write(&world.join("captain.json"), captain);
        write(&captains.join("names.jsonl"), names);

        let out = base.join("out");
        let now = 1_789_000_000; // 2026-09-10 UTC
        let report = run_export(&data, &worlds, &captains, &out, now).expect("export runs");
        ...
        let mut expected: Vec<(String, String)> = vec![
            ("captains/names.jsonl", names),
            ("data/boundary.json", boundary),
            ("data/mesh/node.json", node),
            ("data/observations.jsonl", obs),
            ("worlds/world-abc/captain.json", captain),
            ("worlds/world-abc/journal.jsonl", journal),
        ]
        .into_iter()
        ...
        let withheld: Vec<(String, String)> = manifest["withheld"]
            ...
            .collect();
        for secret in [
            "worlds/world-abc/ucf.env",
            "data/mesh/node_key",
            "data/fleet-serve.token",
            "data/daemon.log",
        ] {
            let entry = withheld
                .iter()
    ...
    fn withholding_rules_are_by_name_and_by_place() {
        ...
        assert!(held("worlds/w1/ucf.env"));
        assert!(held("data/fleet-serve.token"));
        ...
        assert!(held("worlds/w1/mesh/scope_salt"));
        assert!(held("data/daemon.log"));
        assert!(held("captains/x/api.key"));
        assert!(held("captains/x/client_secret.json"));
        ...
        assert!(!held("worlds/w1/journal.jsonl"));
        assert!(!held("captains/names.jsonl"));
    }
