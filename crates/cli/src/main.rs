...
commands:
  ...
  export         the household's whole record as open files: `export [--out DIR]` copies
                 data/ + MANIFEST.json (sha256) + README; secrets withheld
...
fn main() -> ExitCode {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("ledger") => {
            ...
            // the value of a flag is never a subcommand.
...
fn cmd_mesh(args: &[String]) -> ExitCode {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("grant") => {
            ...
            // not a human act. `gate ... deny` remains a fail-safe stop; widening waits for a
        ...
        Some("device") => {
            ...
            // SystemName, e.g. "Kitchen iPad"), distinct from `mesh name` which binds the
...
fn cmd_observe(args: &[String]) -> ExitCode {
    ...
    match observation::record(&dir, obs) {
        Ok(o) => {
            ...
            // A person at this terminal is as present as one at a console. The mesh
...
fn cmd_theories(args: &[String]) -> ExitCode {
    let f = flags(args);
    let dir = store::data_dir(f.get("data-dir").map(String::as_str));
    // `theories retire [--legacy] [--dry-run] --reason "…"`. A thread with no
    ...
    // `theories fold <manifest.json>` — a conservative migration: an explicit,
...
/// The development inbox (ADR-0043 §5): findings the reasoning engine made about
...
fn cmd_run(args: &[String]) -> ExitCode {
    ...
    if unbounded {
        // The cadence floor/ceiling default from the co-owned parameters (the owner can
        // set them from the Glass); `--interval` / `--max-interval` still override. Read once
...
/// is not a budget; it is a surprise waiting. The owner, 2026-08-16: *"I don't have a clear
...
/// `mcp` — the partner seam (ADR-0037 §A). Read what a declared server offers, and
