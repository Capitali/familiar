//! **The MCP seam, client half** (ADR-0037 §A) — the familiar reaching another
...
//! (`purr.say`, `purr.utterances`). That is the other half of the seam and later work; the
...
mod tests {
    ...
    fn urls_parse_or_refuse_and_never_carry_credentials() {
        let u = Url::parse("https://exchange.example.com/mcp").unwrap();
        assert!(u.https && u.port == 443 && u.path == "/mcp");
        assert_eq!(u.origin(), "https://exchange.example.com");
        assert_eq!(u.host_header(), "exchange.example.com");
