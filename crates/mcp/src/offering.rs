...
//! (the grant/ledger design) and are not represented in this module at all.
...
mod tests {
    ...
    fn sentinel_surface(dir: &std::path::Path) {
        let actuators = serde_json::json!({ "actuators": [{
            ...
            "description": "a bedside lamp at 203.0.113.44",
            "state_cmd": "curl -s http://203.0.113.44/state",
            "state": { "fields": { "power": { "kind": "enum",
                "values": ["dimmed", "restored"],
                "source": { "kind": "json", "key": "power" } } } },
            "actions": {
                "dim-ians-lamp": "curl -s http://203.0.113.44/dim",
                "restore-ians-lamp": "curl -s http://203.0.113.44/restore"
            },
            "buckets": [
                { "name": "dim-ians-lamp",
