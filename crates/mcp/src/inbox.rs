...
mod tests {
    ...
    fn setup(tag: &str) -> (std::path::PathBuf, PartnerContext, PartnerContext) {
        ...
        let records = [("p-ian", "Partner A", "ian"), ("p-ada", "Partner B", "ada")]
            .into_iter()
        ...
        (
            ...
            PartnerContext {
                ...
                alias: "Partner A".into(),
            },
            PartnerContext {
                principal: "p-ada".into(),
                credential_fingerprint: "fp-p-ada".into(),
                alias: "Partner B".into(),
            },
        )
    }
    ...
    fn projection_is_private_to_the_registered_human_and_joins_only_valid_surfaces() {
        let (dir, ian, ada) = setup("partner_inbox_private");
        for (context, key) in [(&ian, "ian-request"), (&ada, "ada-request")] {
            grant::request_grant(
                &dir,
                context,
                GrantRequestInput {
        ...
        assert_eq!(view.pending_requests[0].partner_alias, "Partner A");
        assert_eq!(
            view.pending_requests[0].eligible_surfaces[0].surface,
            "private-lamp"
        );
        let raw = serde_json::to_string(&view).unwrap();
        assert!(!raw.contains("Partner B"));
        assert!(!raw.contains("ada-request"));
    }
