...
/// Paths the jailed candidate must not see: the familiar's data directory and
/// this repository. `FAMILIAR_REPO_ROOT` names the checkout when the binary is
/// not run from the tree it was built in.
fn repo_and_data_to_hide(data_dir: &Path) -> Vec<PathBuf> {
    let mut hidden = vec![data_dir.to_path_buf()];
    let repo = std::env::var_os("FAMILIAR_REPO_ROOT")
        .map(PathBuf::from)
        .or_else(|| {
            // <repo>/crates/factory-run → <repo>
    ...
    WorkOrder {
        ...
        research: vec![ResearchEntry {
            title: "SP548E BLE protocol: framing 53|type|key|total_frags|frag_idx|len|payload; \
                ...
                .into(),
            source: "household device notes, verified 2026-07-28; \
...
fn main() {
    ...
    // Resume, never delete (an external review of the whole factory found this): the ledger
    ...
    let env = ConvergeEnv {
        ...
        // mandatory set. The repo root is derived from where this binary was
        // built, and `FAMILIAR_REPO_ROOT` overrides it — a checkout path baked
        // into the source only ever hides one machine's copy.
        extra_hidden: repo_and_data_to_hide(&data_dir),
        bench_wall_secs: 20,
    };
