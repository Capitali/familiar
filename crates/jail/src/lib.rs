//! The containment jail — the floor an external review signed, and the
//! repairs a second review asked for (2026-08-28).
//!
...
//!
//! The review's floor asked for a read allowlist. On macOS 27 that is not
...
//! exactly the property the review named ("the boundary, data dir, repo, and
...
pub fn mandatory_hidden_roots(home: &Path) -> Vec<PathBuf> {
    [
        ...
        "Development/familiar",                 // the repo's other home on a second Mac
        ".ssh",                                 // ssh keys
        ".appstoreconnect",                     // ASC API keys
        ".codex",                               // coding-agent auth + session data
        ".aws",
        ".config",
        ".gnupg",
    ]
