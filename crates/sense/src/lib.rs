...
///   FQDN ("iphone.home.arpa"). Modern phones/watches randomise their MAC, noted as a hint.
...
mod tests {
    ...
    fn parses_arp_and_drops_noise() {
        // Real macOS `arp -a` shape from a home network — names resolved by the router DNS,
        // addresses from the documentation ranges.
        let fixture = "\
router.home.arpa (192.0.2.1) at 00:00:5e:00:53:01 on en0 ifscope [ethernet]
desk-mac (192.0.2.10) at 00:00:5e:00:53:10 on en0 ifscope permanent [ethernet]
? (192.0.2.36) at (incomplete) on en0 ifscope [ethernet]
ipad.home.arpa (192.0.2.145) at 02:00:5e:10:00:45 on en0 ifscope [ethernet]
? (192.0.2.42) at 02:00:5e:10:00:42 on en0 ifscope [ethernet]
...
mdns.mcast.net (224.0.0.251) at 1:0:5e:0:0:fb on en0 ifscope permanent [ethernet]";
        ...
        assert_eq!(n.len(), 4);
        let ipad = n.iter().find(|x| x.ip == "192.0.2.145").unwrap();
        assert_eq!(ipad.name.as_deref(), Some("ipad.home.arpa"));
        let unnamed = n.iter().find(|x| x.ip == "192.0.2.42").unwrap();
        assert_eq!(unnamed.name, None);
        assert!(
            mac_randomized("02:00:5e:10:00:42"),
            "phone MAC is randomised"
        );
        assert!(
            !mac_randomized("00:00:5e:00:53:10"),
    ...
    fn parses_dnsmasq_leases_naming_watch_and_phone() {
        // A real dnsmasq lease table's shape, from a home router: it names the Watch and
        // iPhone the ARP cache never showed, and reports `*` for a client with no hostname.
        let fixture = "\
78178 00:00:5e:00:53:38 192.0.2.38 studio-mac 01:00:00:5e:00:53:38
86400 02:00:5e:10:00:45 192.0.2.145 iPad 01:02:00:5e:10:00:45
74473 02:00:5e:10:00:41 192.0.2.41 Watch 01:02:00:5e:10:00:41
74473 02:00:5e:10:00:42 192.0.2.42 iPhone 01:02:00:5e:10:00:42
50298 02:00:5e:10:00:bb 192.0.2.188 * 00:03:00:01:02:00:5e:10:00:bb
duid 00:03:00:01:00:00:5e:00:53:00";
        let leases = parse_dnsmasq_leases(fixture);
        assert_eq!(leases.len(), 5); // the duid line is skipped
        assert!(leases
            .iter()
            .any(|l| l.name.as_deref() == Some("Watch") && l.ip == "192.0.2.41"));
        assert!(leases.iter().any(|l| l.name.as_deref() == Some("iPhone")));
        let anon = leases.iter().find(|l| l.ip == "192.0.2.188").unwrap();
