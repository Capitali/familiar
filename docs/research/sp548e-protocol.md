...
1. **The owner's own reverse-engineering notes**, kept outside this repo:
   reverse-engineered and verified live on the physical device on firmware
   `V3.0.10`, 2026-07-28. This is the primary source; every framing claim below
   is from it.
2. **Reference implementation (informational only)** — `banlanx_6xx.py` in
   [monty68/uniled](https://github.com/monty68/uniled). Those notes record it
   as byte-accurate for the BanlanX 6xx protocol at `key=0x00`;
...
3. **Live re-verification, 2026-08-28** — a passive BLE scan from a second Mac
...
- **Match rule for the broker:** manufacturer `0x5053` **and** WiFi MAC
  ...
  per-host (a different UUID on every Mac) and not portable.
...
- Bluetooth TCC: a launchd daemon's child is denied CoreBluetooth on first
  headless run (the 2026-08-08 incident on the Intel Mac; the 60s tool budget
  in `cycle` exists because of it). The broker owns the radio and must acquire
