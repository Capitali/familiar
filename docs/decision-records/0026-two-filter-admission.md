...
- **Status:** **accepted** (the owner, 2026-08-01) — written as the Phase 0 gate of the
...
MembershipRecord {
  ...
  identity {
    claim              // "says they are so-and-so" — addresses only, admits nothing
...
- **Claiming an existing handle never establishes via E4.** Only E1/E2/E3 naming that handle
  can attach a new device to an existing human. Typing a name cannot become that person; the
  record shows it as an unestablished claim — *someone says they are so-and-so* — which is a fact
...
the device kind, how the identity was established (*via handoff from an old iPad*,
*invited by an existing member*, *introduced themselves here*), and when. Guests appear as arrivals too — *a
...
- **Disestablish** is *"that's not who they say"*: clears the establishment, the record drops to
...
- **Phase 4 — warrants and the kill-the-lighthouse drill**: a LAN join completing with the VPS
  down, and the real escrow finally exported before the second minting door is reduced.
...
- Where the owner may wish to loosen: the E4 provenance rule (whether a typed name from a pure remote
