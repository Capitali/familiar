...
- **Status:** accepted (the owner, 2026-08-20 — "Build q1-q4 and the rest. Go!" on the conduct
  dialogue's decided design; this ADR is the closure that plan owed)
- **Date:** 2026-08-20
- **Relates to:** [ADR-0040](0040-the-reasoning-engine-grows-honest.md) (D2 closed
  here — questions carry stakes; D3/D4 completed here), ADR-0037 §1 (the persona seam),
  [ADR-0035] (the game exclusion, held structurally), and the conduct dialogue whose
  decided blocks this ADR records durably
...
ADR-0040's D4 asked for the epistemic rule and never got it written down. This
is that document.
...
canonical words. A model-authored paraphrase of a Law cannot reach a human —
...
screened (`corrupting_intent`, refusing without writing the corruption ledger — the
owner's ruling), floor-grounded (the registry leads the prompt), typed (`ReplyDraft`, nine shape
...
`MachineryFinding` routed to a human-visible development inbox, never a
household question and never task authority. An offering to a partner AI
...
same class as the Asimov recital. A structural test enforces the first half;
review enforces the second until it can be typed.

**7. The residual law-quotation gap is labelled, not guessed shut** (the owner, 2026-08-20:
...
- `admission::check_cites` is the one admission function: the reply act and the theorize
  anchors both pass through it (that composition is done); `TheoryDraft` and `AskDraft`
...
- Questions carry stakes (`because` / `turns_on` / `stake ∈ {continues, changes, stops}`
  — deliberately no `none`): ADR-0040's D2 is closed.
- The activity feed counts what was durably answered (the persisted nouns), not what a
  queue held.
- The remaining work closes against this ADR when its device-shell half lands (one Laws
  source for daemon and shells), and its paired task closes with it: the two organisms are
  one, by construction.
