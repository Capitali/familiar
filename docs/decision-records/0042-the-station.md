...
- **Status:** **proposed** (the owner's direction 2026-08-15, recorded here for their decision)
- **Date:** 2026-08-15
...
The owner, testing build 90 (2026-08-15): a spare iPhone was given a station name and its
human set to "shared".

> "I set the name to shared. **This is not the solution to this device.** This device is
> going to be in the shared space at [the table], we already use it to control music to
...
There is precedent already in the household: a second fixed station is an old iPhone
mounted on the wall serving as weather station, power-system remote control, and a
packet-radio beacon. Two instances of one thing the model has no name for.
...
person named "shared" is sitting at the table. The familiar would be manufacturing a
...
the leading predictor. The shared table is where that observation happens. A permanent phantom
...
associated, current and past"* — **plural, by design**. The owner's own 2026-08-14 direction
...
whoever is present. The owner's framing is the right one — *"like a light or a lamp"* — it is
...
This is the decision that shapes the station's character, and it is the owner's:
...
not-knowing serve it.** An unnamed person at the station is a *known unknown the familiar
...
2. **A carried device on the same network or in BLE range.** If a personal device bound
   to a particular person is present at the station, that is real evidence they are. This costs no new
...
human happens to be; **a station's are about its own place, always.** It is the familiar's
...
- **A permanent network vantage.** Always on the LAN, so discovery of the shared things
  (the Bluetooth speaker, the lights, the power system) stops depending on whether somebody happened
  to walk through with a phone.
- **A stable presence anchor for the place** — who is at the station, over time, which is
  the input the shared-lighting consensus has been missing.
- **A control surface for shared things**, which is what the household already uses it for: music,
...
- Presence at the station becomes a real signal, which the shared-lighting work (ADR-0041)
  can actually use.
...
1. **Place.** A station is bound to a place, and the model has no places — only lat/lon and
   the device's own name. Both existing station names carry their place in the name, which
   may be enough. A real place registry would be the larger move, and is worth
...
3. **Guests at a station.** A visitor at the station will be observed and cannot be
   ...
   architecture, and it is the owner's to answer.
