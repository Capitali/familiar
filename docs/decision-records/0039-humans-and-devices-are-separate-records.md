...
- **Status:** **accepted** (the owner, 2026-08-14 — direction given in prose and binding here;
...
their *human*, while the Macs were named as *machines*. 2026-08-13 spent a whole night on
the consequences — a roster that read as a wall of one name, a rename dance that could mint
ghosts, and a display format
...
The owner's direction (2026-08-14), which is the decision:
...
```
HumanRecord {
  handle                    // the identity slug — the establishment reference
  name                      // how they are addressed
...
DeviceRecord {
  ...
  name                      // the SystemName, a machine name — deliberate, human-given
...
current association ("Atlas : iPad : <name>", "Meridian : MacIntel : <name>"). A device with
...
- **Migration** (one, per ADR-0026's lesson): existing establishments whose handle names
  a machine move that name to `DeviceRecord.name` and re-point the
