...
- **Status:** accepted — implemented and **inert** (2026-08-21: the queue is drained
...
or rate-limited (Anthropic $0, gemini/cerebras 429), and the host Mac is a 2019
Intel MBP whose CPU-only ollama manages ~6s/consult on a 3B model. Meanwhile the
fleet's own devices — an iPhone 16 Pro Max and an M5 iPad Pro — are
first-class Apple Intelligence hardware whose inference is already paid for.
iOS 26's `FoundationModels` framework (in that Mac's Xcode 26.5 SDK) exposes
...
- 2026-07-25 — proposed; design settled in-session (fleet verified capable:
  ...
  deferred to a fresh session by the owner's call.
...
from**. Away from the home LAN the iPad reads from the lighthouse, while the campaign
