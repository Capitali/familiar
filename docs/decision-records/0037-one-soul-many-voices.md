...
- **Status:** proposed — **revised 2026-08-16** on the owner's direction: the wire contract
...
A ship's stores, a cargo manifest, and a crew of cats are **fiction**. The household's lights,
a person's presence, and a household member's wellbeing are **real**. If both enter one observation log,
...
The owner, 2026-08-16: *"identity management needs to be part of this as well so that a
...
- **The memory between them is the dossier**, scoped to that ship's world. What the
  ...
  machinery that remembers which light the owner prefers dimmed, partitioned per §B.
...
The owner supplied the endpoint, a publicly reachable MCP server. It answers `initialize`
...
**Settled 2026-08-16 (the owner):** *"for now ship control stays in-game — may change for fleet
