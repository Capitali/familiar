...
are shared and change hands — a second household member's iPad, iPhone and watch sit on
the same mesh as the owner's. The identity model already supported this in principle: an
...
But the served human was **hard-coded to a single creator handle** wherever a device
attributed activity, so every device's observations claimed to be that one person's —
mislabelling everyone else's devices — and there was no per-human attribution or scoping
in the worldview.
...
1. **One shared worldview, per-human attribution.** The mesh is a shared worldview by
   ...
   human suffix — `phone:<human>`, `watch:<human>`). We do not split into private per-human
   consoles.
...
- **Dynamic served-human handle.** iOS `DeviceActor` (`ios/App/Sources/NetworkDiscovery.swift`)
  ...
  seed (`crates/cli/src/main.rs`) attribute to the current human, not a baked-in one.
...
- A second household member's devices tag with their own handle, not the owner's; the roster
  attributes correctly; their heart-rate/location never appear in another node's federated
  worldview.
