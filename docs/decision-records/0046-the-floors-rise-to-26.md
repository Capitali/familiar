...
- **Status:** **accepted** — the owner, 2026-08-24, deciding the fork verbatim, option (a):
  "raise ALL deployment floors to 26," made with the cost stated ("every pre-26 device
  loses the app"). Grounded in the same day's target-audience ruling (verbatim: "lets
...
- **Relates to:** the adoption sweep this floor unblocks, the shell work that depends on this
  landing first ("or this is written twice"), the Envoy (the other FoundationModels
  consumer), [ADR-0038](0038-thoughts-travel-by-leave.md) (`allow_llm`/`allow_llm_cloud`
  still bind every thought-path), and one bench Mac's external-boot ineligibility —
...
and adopt behind guards forever, dropping nobody but carrying two worlds. The owner chose (a).
...
**Stated narrowly (an external review's correction, absorbed): what the floor guarantees is
...
- **Guard dissolution** — the `#if canImport(FoundationModels)` + `@available(26)` guards
  ...
  (the adoption sweep proper, through its own review dialogue), not this record's.
- **PCC posture** — unchanged by this ADR: reopened everywhere eligible (decided 2026-08-24),
...
- The shells build against one honest floor; the survey/BLE work is written once.
- The watch can gain `Shared/Sources` parity without carrying a
  pre-26 code path.
- "We can't easily test today" (no 26-capable bench until the bench Mac returns or new
  hardware appears) is accepted as a known cost; the discipline stands — availability
