...
- **Status:** **accepted** (the owner, 2026-08-21: *"start ADR-0044 (use [an external
  review] partner if needed)"* — the build begins with rung 2; design decided in the conduct
  dialogue, with the external review's proposal absorbed nearly whole)
- **Date:** 2026-08-20
- **Relates to:** [ADR-0043](0043-one-typed-source-per-kind-of-truth.md) (kinds of truth
  ...
  server), the MCP door as deployed (as of 2026-08-18: covenant-gated, fails closed three
  ways), Recipe v1 (declaration never grants authority), ADR-0005 (no companion
  may open a gate), viewer-scoped naming (the sibling privacy decision)

## The owner's direction (2026-08-20, verbatim)
...
The familiar's learned control surface is five classes (traced live):
...
**7. Partners never receive a worldview** (the `Partner` audience): whatever network
...
## Acceptance tests (the external review's list, verbatim commitments)
...
## Build order (after the owner's word — no code before it)
...
3. `observe`, then `invoke`, each behind its own gate and the owner's per-grant word, with the
   revert discipline and bounds enforced kernel-side.

## What this ADR deliberately does not do

It opens no gate (ADR-0005 stands: only the owner opens). It ships no household record, masked
