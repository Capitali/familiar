...
needs to ask a particular person something, it must know that they are present, and at
which device, or the question goes to everyone and is therefore addressed to no one.
...
| Confirm-or-correct prompt ("Is this <name>?" / "Who is it?") | `FaceIdentifyPrompt`, `proposedHandle`, `needsIdentification` |
...
3. **Presence has no confidence and no expiry.** `last_seen` is a timestamp.
   "this person was at this iPad forty minutes ago" and "this person is here now" are
...
is a particular person" would begin unlocking that person's body data to whoever happens to be
standing there. It must not.

Corollaries:

- Confidence **travels with** the attribution. An observation attributed at 0.6 is
  recorded as such, never flattened into "this person did X."
...
2. **Verify the prior (1:1).** If face consent is on and a signature exists, check
   the face *against the bound owner* — "the device says this should be its owner; does
   the face agree?" Agreement raises confidence. Contradiction drops to rung 3. It
   never searches the registry.
3. **Ask.** The existing confirm-or-correct prompt. With a prior: "Is this <name>?"
...
is what lets the mesh answer *"where is a given person right now, and how sure are we?"*
...
## Follow-on work
- Surface `confidence`/`via` in the roster, so "PRESENT <name>" reads with how it was established.
...
confidence, cheap to correct, unlocking nothing. "The camera believes this is a given person"
still opens none of that person's doors, and the sensitive-personal rules (a face signature never federates; a
