...
- Status: **accepted** (the owner and the familiar, 2026-08-07). Every finding below was caught
  live during the first true two-human riddle session (one person on three devices against a
  second person on one, both doors active) and fixed the same afternoon. ADR-0028 said
  "testers playing IS the test"; this record is that sentence paying out.
...
no freshness window. A weeks-old row from a since-retired relay path tagged a Mac door as a
watch at the lighthouse — forever. Door and watch merged into one dedup lineage, and the
roster row flipped between the Mac's name and "Apple Watch" every time their `last_seen`s
leapfrogged.
...
Two humans acting through two different doors (one person's iPad through the lighthouse, the
other's phone through a Mac) watched the holder seesaw for ~30s after every act, as each door's
...
words, door named ("door 127.0.0.1 · not your turn — the ember is with <holder>"). The silent
...
gap stretched to minutes — the second player answered the same riddle three times and each
duplicate became a recorded guess.
...
- The lighthouse keeps its globe dot and node dive but no longer takes a roster card — it is
  infrastructure, not a housemate (the owner's call).
