...
3. **The mesh survived losing the home hub entirely.** On 2026-07-28 the network
   moved to a new site and the canonical "home hub" stayed behind, unreachable
   and not even resolving in DNS, until late September 2026.
...
door (the lighthouse and a Mac)"*. So the group secret lives in at least two
...
phone, watch, iPad, Mac, the old home hub, a future Linux box — is a peer among equals.**

Stated at full strength, because the weaker version invites exactly the drift this ADR exists to
correct: **everything except the lighthouse is transient.** No device is permanent — the oldest
Mac here will be replaced, and the design must treat that as an ordinary
...
1. **One online minting door.** The lighthouse is the only node that holds the
   ...
   The second minting door is to be reduced to a covenant credential like any other peer.
...
- The old home hub returning to power in late September 2026 rejoins as a **peer**. It must
...
- Reduce the second minting door to a covenant credential; confirm `can_mint()` is `false`.
  **Ordering matters and was initially recorded backwards:** as of 2026-07-30 that door still
...
exported first** (the ordering warning in Follow-on work stands: the second door is not
reduced until a real escrow exists). Decisions 2–4 stand as written: peers still cannot mint *without a
