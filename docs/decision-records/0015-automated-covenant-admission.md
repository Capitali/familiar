...
on every mint-capable door (the lighthouse and a Mac). A node is admitted the
moment it:
...
- **Follow-up — deferred, sequenced behind facial recognition (decided 2026-07-27):**
...
- *"every mint-capable door (the lighthouse and a Mac)"* is now **the lighthouse alone**.
  [ADR-0018](0018-lighthouse-single-fixture.md) reduced minting to one door; that Mac still holds
