...
- Status: **accepted** (the owner, 2026-08-06) — documented as-built; every mechanism below is
...
- **Sponsorship** (the new-human half): a refused claim naming a *new* handle is also kept,
  and every member's welcome screen asks: *"a device here introduces a new name — nobody in the
  mesh holds it yet. Welcome them?"* One signed member act (`POST /mesh/sponsor`) is
