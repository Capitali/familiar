...
- **Theorize does not yet weigh its own past.** The familiar now *does* act on a theory —
  ...
  (`cycle::pursues_open_threads_into_candidates`) — so this is no longer a
...
- **Proper names are invisible.** The classifier matches a tight marker set
  (`client`, `customer`, `user`, `person`, …) but not a bare personal name.
...
the ancestor: the kernel is ported and each invariant is now encoded as a passing
...
[claim→evidence table](05-validation-and-results.md#the-evolutionary-kernel-ported)
