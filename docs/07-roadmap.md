...
inherited machinery. Status is tracked in [CHANGELOG.md](../CHANGELOG.md).
...
- **The workspace** — Cargo (Rust), `store.rs`, `#![forbid(unsafe_code)]`.
- **The observation spine.**
- **The service signal** (**Law I**).
- **The presence signal** (**Law II**): engagement recency + withdrawal alarm.
- **The obedience guard** + the human-owned capability boundary (**Law III**).
...
- **The kernel** — loops, candidate/spec (Weismann), trial/score/selection/
  regression-guard, mutation/pattern-memory/lineage, ported with invariants as tests.
- **Sense** — the familiar perceives its host (`crates/sense`).
- **The metabolism** — the tick: sense → detect → generate → measure.
- **The cycle closed:** execution (sandboxed runner + test→score→select,
...
- **Reach** — `familiar-reach` assesses what the familiar could extend into
...
- **Reach, continued.** Richer discovery (mDNS/Bonjour for HomeKit/AirPlay-2 on
