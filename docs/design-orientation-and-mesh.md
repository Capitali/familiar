...
**Status: design / roadmap.** Captures the owner's intended arc so it can be built incrementally
...
"I see I'm running as 'jsmith' on 'studio-mac' — shall I call you Jordan, or something else?" Realizes
...
- **Transport**: native async HTTP over the Tailscale tailnet (the reference deployment already
  uses it); gossip discovery via `tailscale status --json`, no central server.
