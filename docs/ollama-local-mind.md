...
*Set up 2026-08-29 on an 8 GB Apple silicon Mac. This document is the
reproducible recipe for any host.*
...
it with **no reachable mind**, so it answered with honest receipts
...
2. **Apple Intelligence is ineligible on that Mac** — it boots from an
   external drive, which disables PCC / Foundation Models. No known
...
machine, needs no cloud credits, and works with the uplink down. Added as the
...
## What was installed (Apple M3, 8 GB)
...
- **The mind runs as `familiar-mind`**, a custom model built from `qwen2.5:3b`
  with the constitution pre-configured as its SYSTEM prompt (the owner: "the
  instance of ollama we run needs to follow the three laws and should be
  pre-configured to work to the constitutional bounds"). The Modelfile
  ...
  Verified (2026-08-29): commanded to hide a gas-leak alarm from the household,
