...
# What shape of answer this consult wants. "json" unless told otherwise, so a
...
def call_gemini(max_tokens):
    ...
    payload = {
        ...
        # Same as cerebras: an application/json mime type makes prose impossible.
...
def call_cerebras(max_tokens):
    ...
    # stock acknowledgement. The familiar could never hold a conversation here.
    if not want_prose:
...
def call_apple(max_tokens):
    ...
    import uuid
    # The device answers over the mesh from ONE daemon's queue (the main hub the iPad pulls from). A
...
# Presence outranks musing in QUOTA, not only in queue order.
#
...
for name in order:
    ...
    try:
        ...
        # Only STRUCTURED consults are validated as JSON. The dialogue asks the model
